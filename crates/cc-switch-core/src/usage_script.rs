...
static USAGE_SCRIPT_CLIENT: Lazy<reqwest::Client> = Lazy::new(|| {
    reqwest::Client::builder()
        .redirect(reqwest::redirect::Policy::limited(5))
        .build()
        .expect("usage script http client should build")
});
...
pub async fn execute_usage_script(
    ...
    let is_custom_template = template_type.is_some_and(|value| value == "custom");
...
async fn send_http_request(config: &RequestConfig, timeout_secs: u64) -> Result<String, AppError> {
    ...
    let mut request = USAGE_SCRIPT_CLIENT
        ...
        for (index, item) in items.iter().enumerate() {
            validate_single_usage(item).map_err(|e| {
                AppError::localized(
                    ...
                    format!("数组索引[{index}]验证失败: {e}"),
                    format!("Validation failed at index [{index}]: {e}"),
                )
            })?;
        }
        return Ok(());
...
fn validate_single_usage(result: &Value) -> Result<(), AppError> {
    ...
    if object.contains_key("isValid")
    ...
    if object.contains_key("invalidMessage")
    ...
    if object.contains_key("remaining")
    ...
    if object.contains_key("unit") && !result["unit"].is_null() && !result["unit"].is_string() {
        return Err(AppError::localized(
    ...
    if object.contains_key("total") && !result["total"].is_null() && !result["total"].is_number() {
        return Err(AppError::localized(
    ...
    if object.contains_key("used") && !result["used"].is_null() && !result["used"].is_number() {
        return Err(AppError::localized(
    ...
    if object.contains_key("planName")
    ...
    if object.contains_key("extra") && !result["extra"].is_null() && !result["extra"].is_string() {
        return Err(AppError::localized(
...
fn is_suspicious_hostname(hostname: &str) -> bool {
    ...
    hostname.chars().any(|c| suspicious_chars.contains(&c))
}
...
mod tests {
    ...
    #[test]
    fn private_ip_validation_distinguishes_public_172_range() {
    ...
    #[test]
    fn request_validation_rejects_cross_origin_and_private_ips() {
        let cross_origin = validate_request_url(
        ...
        let private_ip = validate_request_url("https://10.0.0.1/usage", "", true);
        assert!(private_ip.is_err());
    }

    #[test]
    fn request_validation_accepts_matching_default_https_ports() {
        assert!(validate_request_url(
            "https://api.example.com:443/usage",
            "https://api.example.com",
            false
        )
        .is_ok());
    ...
    #[test]
    fn result_validation_supports_object_and_array() {
        assert!(validate_result(&json!({
            "planName": "pro",
            "used": 1.2,
            "remaining": 4.8,
            "unit": "USD"
        ...
        assert!(validate_result(&json!([
            { "planName": "a", "used": 1.0 },
            { "planName": "b", "remaining": 2.0 }
        ]))
        .is_ok());
    }

    #[test]
    fn result_validation_rejects_invalid_field_types() {
        let result = validate_result(&json!({
            "planName": 123,
            "used": "oops"
        }));
