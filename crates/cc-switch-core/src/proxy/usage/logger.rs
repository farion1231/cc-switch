...
impl<'a> UsageLogger<'a> {
    ...
    pub async fn resolve_pricing_config(
        ...
        let default_multiplier_raw = match self.db.get_default_cost_multiplier(app_type) {
            Ok(value) => value,
            Err(e) => {
        ...
        let default_pricing_source_raw = match self.db.get_pricing_model_source(app_type) {
            Ok(value) => value,
            Err(e) => {
