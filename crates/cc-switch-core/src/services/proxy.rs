...
impl ProxyService {
    ...
    pub async fn start_with_takeover(&self) -> Result<ProxyServerInfo, String> {
        ...
        if let Err(e) = self.sync_live_to_providers().await {
            ...
            if let Err(clean_err) = self.db.delete_all_live_backups() {
        ...
        if let Err(e) = self.db.set_live_takeover_active(true) {
            if let Err(clean_err) = self.db.delete_all_live_backups() {
                log::warn!("清理 Live 备份失败: {clean_err}");
            }
        ...
        if let Err(e) = self.takeover_live_configs().await {
            ...
            match self.restore_live_configs().await {
                Ok(()) => {
                    let _ = self.db.set_live_takeover_active(false);
                    let _ = self.db.delete_all_live_backups();
                }
                Err(restore_err) => {
    ...
    pub async fn set_takeover_for_app(&self, app_type: &str, enabled: bool) -> Result<(), String> {
        ...
        if enabled {
            ...
            if current_config.enabled {
                let has_backup = match self.db.get_live_backup(app_type_str) {
                    Ok(v) => v.is_some(),
                    Err(e) => {
            ...
            if let Err(e) = self.sync_live_to_provider(&app).await {
                let _ = self.db.delete_live_backup(app_type_str);
                return Err(e);
            }
            ...
            let _ = self.db.set_live_takeover_active(true);
            return Ok(());
        }
    ...
    pub async fn stop(&self) -> Result<(), String> {
        if let Some(server) = self.server.write().await.take() {
            ...
            if global_config.proxy_enabled {
                ...
                if let Err(e) = self.db.update_global_proxy_config(global_config) {
    ...
    pub async fn stop_with_restore(&self) -> Result<(), String> {
        ...
        for app_type in ["claude", "codex", "gemini"] {
            if let Ok(mut config) = self.db.get_proxy_config_for_app(app_type) {
                if config.enabled {
                    config.enabled = false;
                    if let Err(e) = self.db.update_proxy_config_for_app(config) {
    ...
    pub async fn stop_with_restore_keep_state(&self) -> Result<(), String> {
        ...
        if let Ok(mut config) = self.db.get_proxy_config("claude") {
            config.live_takeover_active = false;
            let _ = self.db.update_proxy_config(config);
        }
    ...
    async fn restore_live_config_for_app(&self, app_type: &AppType) -> Result<(), String> {
        match app_type {
            AppType::Claude => {
                if let Ok(Some(backup)) = self.db.get_live_backup("claude") {
            ...
            AppType::Codex => {
                if let Ok(Some(backup)) = self.db.get_live_backup("codex") {
                    let config: Value = serde_json::from_str(&backup.original_config)
            ...
            AppType::Gemini => {
                if let Ok(Some(backup)) = self.db.get_live_backup("gemini") {
                    let config: Value = serde_json::from_str(&backup.original_config)
    ...
    fn restore_live_from_ssot_for_app(&self, app_type: &AppType) -> Result<bool, String> {
        ...
        ProviderService::write_live_snapshot(app_type, provider)
