...
- 不跨 Target 合并 Windows 与 WSL 历史（两端历史永远独立）。
- 不在普通 Provider 切换时隐式改写会话标签；显式“历史归一/恢复”由用户在设置页按 Target 触发（见 ADR-0002）。
...
### 9.3 按 Target 显式历史归一（ADR-0002）

首个竖切已采用与上游一致的策略：**每个 Target 内**把旧会话标签显式迁移到稳定的 `custom` 桶，以便 Codex 原生历史列表统一可见。规则：

- 仅用户在设置页对 **Managed** Target 显式触发；不在普通 Provider 切换时隐式改写。
- 迁移前要求该 Target live `model_provider = "custom"`；否则跳过（`live_not_unified`），避免迁移后历史被当前路由隐藏。
- 修改前写入按 Target 隔离的备份账本；支持按账本精确恢复原始标签。
- **禁止**跨 Target（Windows ↔ WSL）合并历史。
- 统一的是历史分类标签，不是跨后端无损续聊；`encrypted_content` 失败时应切回原 Provider 或新建会话。
- 未来如需来源展示，应建立 CC Switch 自己的 provenance 账本，而不是依赖 Codex 原生 `model_provider` 区分渠道。
