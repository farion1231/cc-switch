...
所有认证、模型、文件路径、加载顺序和传输行为，必须由 pinned Pi oracle 或 request capture 验证。源码阅读和经验判断只能用于提出假设，不能作为产品状态的依据。当前 pin、证据和实现取舍见 [Pi 原生契约与实现边界](./pi-native-contract-zh.md)。

前端必须遵守以下规则：

- Pi 原生文件和目录采用 `exists = active`，不得再增加一个 `enabled` 字段。
- Pi 全局保存的当前供应商和模型仅由后端在写操作前读取并用于安全校验，不作为供应商页面的展示状态，也不从数据库元数据猜测。
- 后端无法读取 `models.json` 时，涉及 Pi 供应商成员关系的写操作必须失败关闭。无法读取当前选择时，只阻止可能破坏默认供应商或默认模型的操作，不得牵连新增、启用或不删除模型的普通编辑。
...
供应商标识是成员身份。Pi `models.json` 中存在该标识即表示已启用，不再比较整份 JSON 来制造“漂移”或“所有权”状态。Pi 中符合 CC Switch 支持范围的自定义 API Key 供应商应自动同步到供应商列表；原生修改同步回已保存档案，原生移除只改变启用状态。Pi 内置供应商和 OAuth 项始终跳过。
...
- 移除或删除时，后端必须重新读取 Pi 全局保存的当前选择；目标仍被 Pi 使用时拒绝操作并引导用户先使用 `/model` 切换。
- 编辑全局当前供应商时，当前模型不能被移除或改名。用户应先在 Pi 中使用 `/model` 切换。
- `models.json` 状态未知时，启用、移除和删除操作应禁用。Pi 当前选择读取失败时，后端只阻止移除、删除或删掉当前模型等破坏性操作。

不得用“列表第一个模型”推导默认模型，也不得在保存供应商时偷偷修改 Pi 默认选择。

项目级 `.pi/settings.json` 依赖启动 Pi 时的工作目录，供应商页没有权威项目上下文，不扫描目录或猜测活动会话。项目级默认项的精确保护应等待 Pi 提供权威状态接口，或由带项目上下文的入口承载。
...
- 未选择预设或“自定义配置”前，不展示后续表单，底部保存按钮保持禁用。
- 选择预设后直接填入供应商知识，例如请求地址、接口格式和模型；用户通常只需填写 API Key。
- 自定义配置显示供应商标识，并在创建后锁定。
- 不使用 `1、2、3` 编号步骤，不增加向导专属页面。
- 预设必须直接消费 `piProviderPresets`，不得手写一套 Anthropic、OpenAI、xAI 等固定入口。
...
| 模型 `models` | 直接展示 | 供应商必须声明可用模型 |
| 请求头 `headers` | 直接展示，默认只有“添加请求头” | 常见兼容需求，且是 Pi 原生字段 |
| 配置 JSON | 只读、默认展开 | 方便确认最终结构 |
| `oauth`、`authHeader`、`compat`、`modelOverrides` 等罕见字段 | 不做结构化入口，原样透传 | 不属于普通创建路径 |
...
选择或填写模型 ID 后，显示名称和已知能力自动预填。用户可以覆盖自动值，也可以恢复自动值。展开区只放 Pi 原生且常用的能力：
...
未知模型不能凭名称猜测能力。没有可靠元数据时保持未定义，并提示用户手动确认。未选择开关不等于强行禁用模型自身能力；只有用户覆盖或可靠预填时才写入相应字段。

Pi 支持的少见模型级 `api`、`baseUrl`、`cost`、`thinkingLevelMap` 等字段暂不做结构化入口，但编辑保存必须保留。

### 4.6 请求头

请求头使用 Pi 原生 `headers`，不得增加“请求头身份”“Claude Code”“Codex”等模板选择器，也不得自动把 API Key 塞进某个 Header。

- 没有请求头时，只显示标题和“添加请求头”按钮。
- 添加后显示名称和值两列。
...
配置 JSON 默认展开、只读，并跟随结构化字段更新。它展示将要保存的结构，但 API Key、Authorization、Token、Secret、Password 等敏感值必须遮罩。遮罩只作用于显示，实际保存值不得改变。
...
- 系统提示：管理 `APPEND_SYSTEM.md` 和 `SYSTEM.md`。
- 提示词模板：管理 Pi 原生模板。
...
- 状态未知时，不用猜测值恢复写操作。
- 删除和移除必须有确认；后端在写入前保护 Pi 正在使用的供应商。
- 保存失败后保留用户输入，显示具体错误。
- 表单错误就地显示并聚焦对应字段。
- 预览、日志、toast 和测试输出不得泄露 API Key 或 Header 中的凭证。
- 外部原生内容发生变化时先重新读取或进入冲突状态，不执行最后写入者覆盖。
...
- 配置 JSON 同步、敏感值遮罩和实际保存不受遮罩影响。
- 已有未知字段、缺失可选字段和精确模型 ID 的无损往返。
- Pi 供应商配置加载、错误、外部修改和当前选择的后端保护。
...
1. UX 审查：小白路径、家族一致性、信息层级、文案和可访问性。
2. Pi 原生语义审查：字段覆盖、无损往返、认证所有权、默认模型所有权和失败关闭。

两路审查独立给出 P0/P1/P2。P0、P1 清零后才能视为通过；不得用一份审查替代另一份。
