...
- 第一次使用的小白能看懂当前在用什么，知道从哪里添加供应商，并能完成 API Key、请求地址和模型配置。
...
- Pi 当前供应商、当前模型和原生目录状态只从后端快照或目录读取，不从数据库元数据猜测。
- 后端状态读取失败时，涉及 Pi 配置的写操作必须失败关闭。不得回退到 `liveConfigManaged` 等旧元数据继续操作。
...
1. 紧凑的当前默认摘要。
2. CC Switch 中保存的供应商列表。
3. 页面右上角唯一的“+”添加入口。

当前摘要只读展示 Pi 当前供应商、模型归属和所有权。它不是操作面板，不放刷新、登录、切换默认、路由诊断等按钮。
...
CC Switch 不设置 Pi 的默认供应商，也不提供默认模型字段。

- 默认供应商和模型完全交给 Pi 原生 `/model`。
- 前端只读显示 Pi 当前选择。
- Pi 中执行 `/model` 后，CC Switch 下次读取快照时必须同步显示。
- 当前默认供应商不能移除或删除。
- 编辑当前供应商时，当前模型不能被移除或改名。用户应先在 Pi 中使用 `/model` 切换。
- 当前状态未知时，启用、移除和删除操作都应禁用；编辑表单可以查看，但不能保存。
