...
- Pi 全局保存的当前供应商和模型只由后端读取：默认供应商用于移除或删除前的条件提醒，默认模型用于防止编辑时直接移除当前模型。它们不作为供应商页面的展示状态，也不从数据库元数据猜测。
- 后端无法读取 `models.json` 时，涉及 Pi 供应商成员关系的写操作必须失败关闭。无法读取全局默认项时不阻止成员关系操作；删除模型等可能破坏当前选择的编辑仍应失败关闭。
