...
供应商标识是成员身份。Pi `models.json` 中存在该标识即表示已启用，不再比较整份 JSON 来制造“漂移”或“所有权”状态。凡是显式存在于 `models.json.providers` 的供应商都应同步到供应商列表，包括与 Pi 内置供应商同名的 ID；原生修改同步回已保存档案，原生移除只改变启用状态。只存在于 `auth.json`、且实际通过 `/login` 配置的 provider 生成 Pi 管理的只读卡片；环境变量中的认证状态不生成卡片。详细规则见 [Pi 显式供应商与登录渠道同步需求](./pi-live-provider-sync-requirements-zh.md)。
