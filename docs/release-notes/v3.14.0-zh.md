...
- **应用级窗口控件**：可选让 CC Doctor 自绘 min/max/close，显著改善 Linux Wayland 体验
...
CC Doctor 首次支持 Hermes Agent 作为一等受管应用，与 Claude / Codex / Gemini / OpenCode / OpenClaw 并列。
...
- **用户 `providers:` dict 只读呈现**：用户在 YAML 里手写的 providers 条目在 CC Doctor 中以只读卡片展示，深度配置跳转到 Hermes Web UI
...
- 新增可选设置，让 CC Doctor 自绘最小化 / 切换最大化 / 关闭按钮，代替系统装饰 (#1119, 感谢 @git1677967754)
...
- 新增设置项，允许把 CC Doctor 指向自定义的 `openclaw.json` 位置 (#1518, 感谢 @mrFranklin)

### Hermes 配置目录自定义

- 新增设置项，允许把 CC Doctor 指向自定义的 `~/.hermes/config.yaml` 位置，底层通过数据驱动 dispatch
...
- 深度 YAML 配置不再在 CC Doctor 表单里重复，直接通过"启动 Hermes Web UI"按钮交给 Web UI
...
- 托盘标识符从硬编码字符串 `"main"` 改为 `TRAY_ID` 常量（`"cc-doctor"`），所有调用点同步 (#1978, 感谢 @lidaxian121)
...
- 经 CC Doctor 往返写入不再丢失 OAuth MCP `auth` 块、也不污染不相关的 YAML 键
...
用户启用上述功能即表示**自行承担所有风险**。CC Doctor 不对因使用这些功能而导致的任何账号限制、警告或服务暂停承担责任。

---

## 下载与安装

访问 [Releases](https://github.com/diaojz/cc-doctor/releases/latest) 下载对应版本。
...
| 文件                                     | 说明                                |
| ---------------------------------------- | ----------------------------------- |
| `CC-Doctor-v3.14.0-Windows.msi`          | **推荐** - MSI 安装包，支持自动更新 |
| `CC-Doctor-v3.14.0-Windows-Portable.zip` | 便携版，解压即用，不写入注册表      |

### macOS

| 文件                             | 说明                                          |
| -------------------------------- | --------------------------------------------- |
| `CC-Doctor-v3.14.0-macOS.dmg`    | **推荐** - DMG 安装包，拖入 Applications 即可 |
| `CC-Doctor-v3.14.0-macOS.zip`    | 解压后拖入 Applications，Universal Binary     |
| `CC-Doctor-v3.14.0-macOS.tar.gz` | 用于 Homebrew 安装和自动更新                  |
...
| 发行版                                  | 推荐格式    | 安装方式                                                               |
| --------------------------------------- | ----------- | ---------------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`      | `sudo dpkg -i CC-Doctor-*.deb` 或 `sudo apt install ./CC-Doctor-*.deb` |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`      | `sudo rpm -i CC-Doctor-*.rpm` 或 `sudo dnf install ./CC-Doctor-*.rpm`  |
| openSUSE                                | `.rpm`      | `sudo zypper install ./CC-Doctor-*.rpm`                                |
| Arch Linux / Manjaro                    | `.AppImage` | 添加执行权限后直接运行，或使用 AUR                                     |
| 其他发行版 / 不确定                     | `.AppImage` | `chmod +x CC-Doctor-*.AppImage && ./CC-Doctor-*.AppImage`              |
