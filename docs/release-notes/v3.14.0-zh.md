...
- **应用级窗口控件**：可选让 EC Switch 自绘 min/max/close，显著改善 Linux Wayland 体验
...
EC Switch 首次支持 Hermes Agent 作为一等受管应用，与 Claude / Codex / Gemini / OpenCode / OpenClaw 并列。
...
- **用户 `providers:` dict 只读呈现**：用户在 YAML 里手写的 providers 条目在 EC Switch 中以只读卡片展示，深度配置跳转到 Hermes Web UI
...
- 新增可选设置，让 EC Switch 自绘最小化 / 切换最大化 / 关闭按钮，代替系统装饰 (#1119, 感谢 @git1677967754)
...
- 新增设置项，允许把 EC Switch 指向自定义的 `openclaw.json` 位置 (#1518, 感谢 @mrFranklin)

### Hermes 配置目录自定义

- 新增设置项，允许把 EC Switch 指向自定义的 `~/.hermes/config.yaml` 位置，底层通过数据驱动 dispatch
...
- 深度 YAML 配置不再在 EC Switch 表单里重复，直接通过"启动 Hermes Web UI"按钮交给 Web UI
...
- 托盘标识符从硬编码字符串 `"main"` 改为 `TRAY_ID` 常量（`"ec-switch"`），所有调用点同步 (#1978, 感谢 @lidaxian121)
...
- 经 EC Switch 往返写入不再丢失 OAuth MCP `auth` 块、也不污染不相关的 YAML 键
