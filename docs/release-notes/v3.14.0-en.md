...
- **Application-Level Window Controls**: Opt-in setting to render EC Switch's own min/max/close buttons, materially improving Linux Wayland experience
- **Hermes in Unified Skills Management**: Skill install, enable, and filter now cover Hermes
- **Hermes / OpenClaw Config Directory Override**: Point EC Switch at a custom `~/.hermes/config.yaml` or `openclaw.json` location
...
EC Switch now treats Hermes Agent as a first-class managed app alongside Claude / Codex / Gemini / OpenCode / OpenClaw.
...
- **User `providers:` Dict Read-Only Rendering**: User-authored providers in the YAML appear as read-only cards in EC Switch; deep configuration delegates to the Hermes Web UI
...
- Opt-in setting to render EC Switch's own minimize / toggle-maximize / close buttons instead of system decorations (#1119, thanks @git1677967754)
...
- New settings option to point EC Switch at a custom `openclaw.json` location (#1518, thanks @mrFranklin)

### Hermes Config Directory Override

- New settings option to point EC Switch at a custom `~/.hermes/config.yaml` location, backed by data-driven dispatch
...
- Deep YAML knobs are no longer duplicated in the EC Switch form — they are delegated to the Hermes Web UI via a direct launch action
...
- The tray identifier moved from the hardcoded string `"main"` to a `TRAY_ID` constant (`"ec-switch"`) across all call sites (#1978, thanks @lidaxian121)
...
- Round-tripping through EC Switch no longer drops OAuth MCP `auth` blocks or pollutes unrelated YAML keys
