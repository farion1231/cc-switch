...
- **Application-Level Window Controls**: Opt-in setting to render CC Doctor's own min/max/close buttons, materially improving Linux Wayland experience
- **Hermes in Unified Skills Management**: Skill install, enable, and filter now cover Hermes
- **Hermes / OpenClaw Config Directory Override**: Point CC Doctor at a custom `~/.hermes/config.yaml` or `openclaw.json` location
...
CC Doctor now treats Hermes Agent as a first-class managed app alongside Claude / Codex / Gemini / OpenCode / OpenClaw.
...
- **User `providers:` Dict Read-Only Rendering**: User-authored providers in the YAML appear as read-only cards in CC Doctor; deep configuration delegates to the Hermes Web UI
...
- Opt-in setting to render CC Doctor's own minimize / toggle-maximize / close buttons instead of system decorations (#1119, thanks @git1677967754)
...
- New settings option to point CC Doctor at a custom `openclaw.json` location (#1518, thanks @mrFranklin)

### Hermes Config Directory Override

- New settings option to point CC Doctor at a custom `~/.hermes/config.yaml` location, backed by data-driven dispatch
...
- Deep YAML knobs are no longer duplicated in the CC Doctor form — they are delegated to the Hermes Web UI via a direct launch action
...
- The tray identifier moved from the hardcoded string `"main"` to a `TRAY_ID` constant (`"cc-doctor"`) across all call sites (#1978, thanks @lidaxian121)
...
- Round-tripping through CC Doctor no longer drops OAuth MCP `auth` blocks or pollutes unrelated YAML keys
...
By enabling these features, users **accept all associated risks**. CC Doctor is not responsible for any account restrictions, warnings, or service suspensions that result from using these features.
...
| File                                     | Description                                     |
| ---------------------------------------- | ----------------------------------------------- |
| `CC-Doctor-v3.14.0-Windows.msi`          | **Recommended** - MSI installer, supports auto-update |
| `CC-Doctor-v3.14.0-Windows-Portable.zip` | Portable, extract and run, no registry writes   |

### macOS

| File                             | Description                                              |
| -------------------------------- | -------------------------------------------------------- |
| `CC-Doctor-v3.14.0-macOS.dmg`    | **Recommended** - DMG installer, drag into Applications  |
| `CC-Doctor-v3.14.0-macOS.zip`    | Extract and drag into Applications, Universal Binary     |
| `CC-Doctor-v3.14.0-macOS.tar.gz` | For Homebrew installation and auto-update                |
...
| Other distros / not sure                | `.AppImage` | `chmod +x CC-Doctor-*.AppImage && ./CC-Doctor-*.AppImage`              |
