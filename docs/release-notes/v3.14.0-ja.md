...
- **アプリケーションレベルのウィンドウコントロール**: EC Switch が自前で最小化 / 最大化トグル / 閉じるボタンを描画するオプション設定。Linux Wayland での体験を大きく改善
...
EC Switch は Hermes Agent を Claude / Codex / Gemini / OpenCode / OpenClaw と並ぶ一等の管理対象アプリとして初めてサポートします。
...
- **ユーザー `providers:` dict の読み取り専用表示**: YAML に手書きされたプロバイダーエントリは EC Switch で読み取り専用カードとして表示され、深い設定は Hermes Web UI に委譲
...
- EC Switch が自前で最小化 / 最大化トグル / 閉じるボタンを描画するオプション設定を追加。システム装飾の代わりに使用 (#1119, 感謝 @git1677967754)
...
- EC Switch が参照する `openclaw.json` のカスタム位置を設定できるオプションを追加 (#1518, 感謝 @mrFranklin)

### Hermes 設定ディレクトリのカスタマイズ

- EC Switch が参照する `~/.hermes/config.yaml` のカスタム位置を設定できるオプションを追加。データ駆動 dispatch でサポート
...
- 深い YAML 設定は EC Switch フォームで重複させず、「Hermes Web UI を起動」ボタン経由で Web UI に直接委譲
...
- トレイ識別子をハードコーディング文字列 `"main"` から `TRAY_ID` 定数（`"ec-switch"`）に移行。すべての呼び出し箇所で同期 (#1978, 感謝 @lidaxian121)
...
- EC Switch 経由でラウンドトリップしても OAuth MCP `auth` ブロックが消失したり、無関係な YAML キーが汚染されたりしなくなった
...
これらの機能を有効にすることで、ユーザーは**すべての関連リスクを自己責任で受諾**したものとみなされます。EC Switch はこれらの機能の使用に起因するアカウントの制限、警告、サービス停止について一切の責任を負いません。
