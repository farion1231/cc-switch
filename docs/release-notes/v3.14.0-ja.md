...
- **アプリケーションレベルのウィンドウコントロール**: CC Doctor が自前で最小化 / 最大化トグル / 閉じるボタンを描画するオプション設定。Linux Wayland での体験を大きく改善
...
CC Doctor は Hermes Agent を Claude / Codex / Gemini / OpenCode / OpenClaw と並ぶ一等の管理対象アプリとして初めてサポートします。
...
- **ユーザー `providers:` dict の読み取り専用表示**: YAML に手書きされたプロバイダーエントリは CC Doctor で読み取り専用カードとして表示され、深い設定は Hermes Web UI に委譲
...
- CC Doctor が自前で最小化 / 最大化トグル / 閉じるボタンを描画するオプション設定を追加。システム装飾の代わりに使用 (#1119, 感謝 @git1677967754)
...
- CC Doctor が参照する `openclaw.json` のカスタム位置を設定できるオプションを追加 (#1518, 感謝 @mrFranklin)

### Hermes 設定ディレクトリのカスタマイズ

- CC Doctor が参照する `~/.hermes/config.yaml` のカスタム位置を設定できるオプションを追加。データ駆動 dispatch でサポート
...
- 深い YAML 設定は CC Doctor フォームで重複させず、「Hermes Web UI を起動」ボタン経由で Web UI に直接委譲
...
- トレイ識別子をハードコーディング文字列 `"main"` から `TRAY_ID` 定数（`"cc-doctor"`）に移行。すべての呼び出し箇所で同期 (#1978, 感謝 @lidaxian121)
...
- CC Doctor 経由でラウンドトリップしても OAuth MCP `auth` ブロックが消失したり、無関係な YAML キーが汚染されたりしなくなった
...
これらの機能を有効にすることで、ユーザーは**すべての関連リスクを自己責任で受諾**したものとみなされます。CC Doctor はこれらの機能の使用に起因するアカウントの制限、警告、サービス停止について一切の責任を負いません。

---

## ダウンロード・インストール

[Releases](https://github.com/diaojz/cc-doctor/releases/latest) から対応バージョンをダウンロードしてください。
...
| ファイル                                 | 説明                                        |
| ---------------------------------------- | ------------------------------------------- |
| `CC-Doctor-v3.14.0-Windows.msi`          | **推奨** - MSI インストーラー、自動更新対応 |
| `CC-Doctor-v3.14.0-Windows-Portable.zip` | ポータブル版、解凍して実行、レジストリ不要  |

### macOS

| ファイル                         | 説明                                                     |
| -------------------------------- | -------------------------------------------------------- |
| `CC-Doctor-v3.14.0-macOS.dmg`    | **推奨** - DMG インストーラー、Applications にドラッグ   |
| `CC-Doctor-v3.14.0-macOS.zip`    | 解凍して Applications にドラッグ、Universal Binary       |
| `CC-Doctor-v3.14.0-macOS.tar.gz` | Homebrew インストールと自動更新用                        |
...
| --------------------------------------- | ----------- | ---------------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`      | `sudo dpkg -i CC-Doctor-*.deb` または `sudo apt install ./CC-Doctor-*.deb` |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`      | `sudo rpm -i CC-Doctor-*.rpm` または `sudo dnf install ./CC-Doctor-*.rpm`  |
| openSUSE                                | `.rpm`      | `sudo zypper install ./CC-Doctor-*.rpm`                                |
| Arch Linux / Manjaro                    | `.AppImage` | 実行権限を付与して実行、または AUR を使用                              |
| その他のディストリビューション / 不明   | `.AppImage` | `chmod +x CC-Doctor-*.AppImage && ./CC-Doctor-*.AppImage`              |
