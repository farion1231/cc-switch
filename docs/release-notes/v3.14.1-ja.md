...
CC Doctor v3.14.1 は v3.14.0 に続くパッチリリースで、**Codex OAuth リバースプロキシの安定性**、**トレイでの用量可視化**、**Skills インポート / インストールの信頼性**、**Gemini セッション復元パス**、および **Hermes 設定ヘルス処理の簡素化**を中心に据えています。
...
さらに、CC Doctor 内蔵の **Hermes 設定ヘルススキャナー**と警告バナー（および対応する `scan_hermes_config_health` コマンド、`HermesHealthWarning` 型、`HermesWriteOutcome.warnings` ペイロード）を削除し、Hermes サーフェスをアクティブプロバイダー表示、デフォルト切り替え、Memory 編集、および Hermes Web UI の起動に再フォーカスしました — 深い設定ヘルスは Hermes 自身の責任になります。
...
- CC Doctor の Hermes サーフェスは本来の役割に回帰: アクティブプロバイダー表示、デフォルトプロバイダー切り替え、Memory 編集、および深い設定用の Hermes Web UI 起動
...
- CC Doctor が非ストリーミング変換を実行する前に上流 SSE イベントを集約
...
- **Hermes ヘルススキャナー削除済み**: Hermes YAML の深い設定の問題提示を CC Doctor に頼っていた場合は、ツールバーの「Hermes Web UI を起動」ボタンから Hermes 自身のパネルで確認してください。日常のプロバイダー管理、切り替え、Memory 編集、MCP / Skills 同期は引き続き CC Doctor が担います。
...
| ファイル                                 | 説明                                        |
| ---------------------------------------- | ------------------------------------------- |
| `CC-Doctor-v3.14.1-Windows.msi`          | **推奨** - MSI インストーラー、自動更新対応 |
| `CC-Doctor-v3.14.1-Windows-Portable.zip` | ポータブル版、解凍して実行、レジストリ不要  |

### macOS

| ファイル                         | 説明                                                   |
| -------------------------------- | ------------------------------------------------------ |
| `CC-Doctor-v3.14.1-macOS.dmg`    | **推奨** - DMG インストーラー、Applications にドラッグ |
| `CC-Doctor-v3.14.1-macOS.zip`    | 解凍して Applications にドラッグ、Universal Binary     |
| `CC-Doctor-v3.14.1-macOS.tar.gz` | Homebrew インストールと自動更新用                      |
