...
Added full reverse proxy support for GitHub Copilot, enabling Copilot-authenticated requests to be forwarded through EC Switch.
...
- Backups stored in `~/.ec-switch/skill-backups/` with all skill files and a `meta.json` containing original metadata
...
Users enable this feature **at their own risk**. EC Switch is not responsible for any account restrictions, warnings, or service suspensions resulting from the use of this feature.
