...
CC Doctor v3.14.1 is a patch release following v3.14.0, focused on **Codex OAuth reverse-proxy stability**, **tray usage visibility**, **Skills import / install reliability**, **Gemini session restore paths**, and **simplifying Hermes configuration health handling**.
...
- The CC Doctor Hermes surface now focuses on its core job: active provider display, default provider switching, memory editing, and launching the Hermes Web UI for deep configuration
...
- CC Doctor aggregates the upstream SSE events before running the non-streaming transform
...
- **Hermes Health Scanner Removed**: If you were relying on CC Doctor to surface deep Hermes YAML configuration issues, switch to the "Launch Hermes Web UI" toolbar button and inspect them in Hermes's own panel. Day-to-day provider management, switching, memory editing, and MCP / Skills sync continue to be handled by CC Doctor.
...
| File                                     | Description                                           |
| ---------------------------------------- | ----------------------------------------------------- |
| `CC-Doctor-v3.14.1-Windows.msi`          | **Recommended** - MSI installer, supports auto-update |
| `CC-Doctor-v3.14.1-Windows-Portable.zip` | Portable, extract and run, no registry writes         |

### macOS

| File                             | Description                                             |
| -------------------------------- | ------------------------------------------------------- |
| `CC-Doctor-v3.14.1-macOS.dmg`    | **Recommended** - DMG installer, drag into Applications |
| `CC-Doctor-v3.14.1-macOS.zip`    | Extract and drag into Applications, Universal Binary    |
| `CC-Doctor-v3.14.1-macOS.tar.gz` | For Homebrew installation and auto-update               |
