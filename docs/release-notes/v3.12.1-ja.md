...
CC Doctor v3.12.1 は、安定性の改善とバグ修正に焦点を当てたパッチリリースです。共通設定モーダルの無限再オープンループ、WebDAV 同期時の外部キー制約エラー、複数の i18n 補間問題を修正しました。また、**StepFun（阶跃星辰）** プロバイダープリセットの追加、OpenClaw の**入力タイプ選択**と **authHeader** サポート、デフォルト Gemini モデルの **3.1-pro** へのアップグレード、4 つの新スポンサーパートナーの追加が含まれます。
...
| ファイル                                   | 説明                                                 |
| ------------------------------------------ | ---------------------------------------------------- |
| `CC-Doctor-v3.12.1-Windows.msi`            | **推奨** - MSI インストーラー、自動更新対応          |
| `CC-Doctor-v3.12.1-Windows-Portable.zip`   | ポータブル版、解凍して実行、レジストリ書き込みなし   |

### macOS

| ファイル                           | 説明                                                              |
| ---------------------------------- | ----------------------------------------------------------------- |
| `CC-Doctor-v3.12.1-macOS.zip`      | **推奨** - 解凍して Applications にドラッグ、Universal Binary     |
| `CC-Doctor-v3.12.1-macOS.tar.gz`   | Homebrew インストールと自動更新用                                 |
