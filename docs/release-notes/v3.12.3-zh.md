...
- 备份存储在 `~/.ec-switch/skill-backups/`，包含所有 skill 文件和记录原始元数据的 `meta.json`
- 旧备份自动清理，最多保留 20 个
- 备份路径返回前端并在成功提示中显示
...
用户启用此功能即表示**自行承担所有风险**。EC Switch 不对因使用此功能而导致的任何账号限制、警告或服务暂停承担责任。
