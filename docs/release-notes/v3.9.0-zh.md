...
CC Doctor v3.9.0 是 v3.9 测试版序列（`3.9.0-1`、`3.9.0-2`、`3.9.0-3`）的稳定版。
...
- 为降低导入风险，SQL 导入被限制为仅允许导入 CC Doctor 自己导出的备份。
- Proxy 接管会修改 CLI 的 live 配置；CC Doctor 会在重定向前自动备份 live 配置。如需回退，可关闭接管/停止代理，并在必要时从备份恢复。
...
| 文件                                    | 说明                                |
| --------------------------------------- | ----------------------------------- |
| `CC-Doctor-v3.9.0-Windows.msi`          | **推荐** - MSI 安装包，支持自动更新 |
| `CC-Doctor-v3.9.0-Windows-Portable.zip` | 便携版，解压即用，不写入注册表      |

### macOS

| 文件                            | 说明                                                      |
| ------------------------------- | --------------------------------------------------------- |
| `CC-Doctor-v3.9.0-macOS.zip`    | **推荐** - 解压后拖入 Applications 即可，Universal Binary |
| `CC-Doctor-v3.9.0-macOS.tar.gz` | 用于 Homebrew 安装和自动更新                              |
...
| 沙箱隔离需求                            | `.flatpak`  | `flatpak install CC-Doctor-*.flatpak`                                  |
