...
EC Switch v3.9.0 是 v3.9 测试版序列（`3.9.0-1`、`3.9.0-2`、`3.9.0-3`）的稳定版。
...
- 为降低导入风险，SQL 导入被限制为仅允许导入 EC Switch 自己导出的备份。
- Proxy 接管会修改 CLI 的 live 配置；EC Switch 会在重定向前自动备份 live 配置。如需回退，可关闭接管/停止代理，并在必要时从备份恢复。
