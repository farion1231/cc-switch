...
- **組み込み Web 検索は上流の対応状況に依存**: ルーターは Claude Code のホスト型 WebSearch ツールを Responses API のホスト型 `web_search` ツールへ変換します。このツールを実装する Codex OAuth および互換 Responses ゲートウェイで利用できます。`allowed_domains` は維持されます。Responses には同等の拒否リストがないため、空でない `blocked_domains` を含むリクエストは、制限を黙って無視せず明示的に失敗します。ローカルで実行される WebFetch は影響を受けません。
