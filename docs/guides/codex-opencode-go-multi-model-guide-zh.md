...
> 适用版本:CC Switch 3.16.x 附近。本文根据实践整理,补全官方预设里缺失的 **OpenCode Go × Codex** 适配方法。示例使用去敏数据,请勿泄露真实 API Key。
...
| 显示名 | 模型 ID(填这个) | 上下文 | 多模态输入 |
|---|---|---|---|
| DeepSeek V4 Flash | `deepseek-v4-flash` | 1M | 纯文本 |
| DeepSeek V4 Pro | `deepseek-v4-pro` | 1M | 纯文本 |
| Qwen3.7 Plus | `qwen3.7-plus` | 1M | 图 + 视频 |
| Qwen3.7 Max | `qwen3.7-max` | 1M | 纯文本 |
| Qwen3.6 Plus | `qwen3.6-plus` | 1M | 图 + 视频 |
| GLM-5.1 | `glm-5.1` | 200k | 纯文本 |
| GLM-5 | `glm-5` | 200k | 纯文本 |
| Kimi K2.6 | `kimi-k2.6` | 262k | 图 + 视频 |
| Kimi K2.5 | `kimi-k2.5` | 262k | 图 + 视频 |
| MiniMax M3 | `minimax-m3` | 512k | 图 + 视频 |
| MiniMax M2.7 / M2.5 | `minimax-m2.7` / `minimax-m2.5` | 204k | 纯文本 |
| MiMo V2.5 | `mimo-v2.5` | 1M | 图 + 音 + 视频 |
| MiMo V2.5 Pro | `mimo-v2.5-pro` | 1M | 纯文本 |
...
### 坑 4:MiniMax(M2/M3)在 chat 通路上的特殊性
...
- 结论:MiniMax 系列在 Codex 的 chat 通路上**不适合长 agent 循环**;需要连续工具调用的 agent 任务,建议用 **DeepSeek / Qwen / GLM**。MiniMax 更适合一次性/读图场景。
...
| 回一句就停、需手动继续 | MiniMax 关闭 thinking 后失去交错思考 | agent 任务改用 DeepSeek/Qwen/GLM |

---

*本指南由社区实践整理,欢迎补充 English 版本与更多模型的实测数据。*
