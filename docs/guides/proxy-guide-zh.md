...
## 会话级定向供应商（x-cc-provider）

代理默认按「当前供应商 / 故障转移队列」路由。如果你想让**某一个会话**固定走指定供应商（其他会话不受影响），在请求头里带上 `x-cc-provider` 即可：

```bash
# 值可以是供应商名称（忽略大小写），也可以是 provider id
ANTHROPIC_CUSTOM_HEADERS="x-cc-provider: my-backup-vendor" claude
```

（`ANTHROPIC_CUSTOM_HEADERS` 用 `Name: Value` 格式，多个头用换行分隔，需要 Claude Code v2.1.227 及以上。）

> **供应商名里有中文时请改用 provider id。** Claude Code 会拒发含 U+00FF 以上字符的请求头（报 `Invalid ANTHROPIC_CUSTOM_HEADERS`），中文名根本出不了客户端这一关。代理侧本身能正确解析中文名（UTF-8 / latin-1 都吃），curl 等能直传原始字节的客户端不受这条限制。

Codex / Gemini / Grok 等其他应用同样生效，只要客户端能自定义请求头。

要点：

- **不带这个头 = 行为完全不变**，仍走默认路由
- 钉住的会话不做故障转移、不查熔断器。**例外**：Claude Code 的辅助请求（`x-claude-code-request-class: auxiliary`，含 Auto Mode 安全分类器）仍按辅助请求队列分流（分流优先于钉住），队列不可用时才回落到被钉的供应商
- **不会改写首页的当前供应商**，也不会动供应商卡片上的「正在使用」标记——它是这一个会话的定向，不是全局切换
- 名称拼错或匹配到多个同名供应商时，代理直接返回 400 并说明原因，而不是悄悄回落到默认供应商（否则额度会记到另一个账号上而你毫不知情）
- 该请求头只在代理内部消费，不会转发给上游
