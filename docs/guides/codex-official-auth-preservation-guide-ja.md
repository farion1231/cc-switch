...
### 古いスレッドは公式 `api.openai.com` に残り得る（新規チャットは成功）

サードパーティプロバイダーへ切り替えた後:

- **新規の Codex App チャット**は通常 `config.toml` のサードパーティ `base_url` を使い、正常に動作します。
- **ChatGPT アカウント / 公式モデル時代に作った古いスレッド**を続けると、リクエストが `https://api.openai.com/v1/responses` に送られることがあります。
- そのとき `auth.json` / bearer がサードパーティ API Key だと、公式側は `401 invalid_api_key` を返します。同じ key は新規チャットや CLI では成功することがあります。

これは多くの場合「key が壊れている」のではなく、**古いスレッドの実行時 auth/endpoint が公式チャネルに張り付いたまま**で、CC Switch が書いたデフォルトの `model_provider=custom` と一致しない状態です。

回避策:

1. サードパーティ利用中は **新規チャット**を優先する。
2. 古い文脈が必要なら、新規チャットへ要約を貼る / ツールで旧 thread を読む。古いアカウント時代スレッドで連打しない。
3. その古いスレッド自体を続ける必要がある場合は `OpenAI Official` に戻し、有効な ChatGPT ログインを復旧してから続行する。

追跡: https://github.com/farion1231/cc-switch/issues/5672
...
**新規チャットは成功するのに、古いスレッドを続けると 401（`api.openai.com` + invalid_api_key）になるのはなぜ？**

古いスレッドが公式 Responses ホストに残っている一方で、現在の key がサードパーティ key であることが多いです。新規チャットを優先してください。上記の副作用セクションと issue #5672 を参照。
