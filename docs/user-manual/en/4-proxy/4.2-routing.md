...
App routing means letting EC Switch route a specific application's API requests through the local routing service.
...
When routing is enabled, EC Switch modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, EC Switch will:
...
Before enabling routing, EC Switch backs up the original configuration and restores it when disabled.
