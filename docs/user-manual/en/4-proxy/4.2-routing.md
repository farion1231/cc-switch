...
App routing means letting CC Doctor route a specific application's API requests through the local routing service.
...
When routing is enabled, CC Doctor modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, CC Doctor will:
...
Before enabling routing, CC Doctor backs up the original configuration and restores it when disabled.
