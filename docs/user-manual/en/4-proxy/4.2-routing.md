...
### Auto-resume Routing on Startup

In Settings > Local Routing you can enable "Auto-resume Routing on Startup":

- **Enabled**: The routing takeover state is preserved when the app exits and automatically restored the next time CC Switch starts. This requires "Launch on Startup" to be enabled; otherwise takeover resumes only when you manually open CC Switch.
- **Disabled (default)**: Exiting the app restores the original client configurations and clears the takeover state; the next launch does not re-take-over automatically. Even if the app exits abnormally (e.g. a direct shutdown or power loss), the next CC Switch launch automatically restores any leftover routing address to the original configuration.
...
Solutions:
- Reopen CC Switch: on launch it automatically detects leftover takeover state and restores the original configuration (when "Auto-resume Routing on Startup" is disabled)
