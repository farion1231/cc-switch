...
The Claude Code Gateway exposes models from multiple router-enabled Claude providers through one local endpoint:

```text
http://127.0.0.1:15721/claude-router
```

Enable **Enabled for Router** on each Claude provider and add its model alias, display name, and upstream model. Router enablement is independent from **Switch**, the failover queue, and **Launch Claude (Isolated)**. The public model IDs use `ccswitch/claude/<provider>/<alias>` with Base64 URL-safe encoded segments, so Claude Code discovers them while provider identity remains unambiguous.

Open **Settings > Advanced > Proxy Service > Claude Code Gateway > Setup**, then merge this snippet into VS Code `settings.json`:
  ...
  "claudeCode.environmentVariables": [
    {
      ...
      "value": "http://127.0.0.1:15721/claude-router"
    },
    {
...
If `claudeCode.environmentVariables` already exists, merge these entries into the existing array; do not replace it. Start a new Claude session after changing settings. Run **Developer: Reload Window** only if the model list remains cached.

The gateway is intended for local use. `PROXY_MANAGED` is a sentinel, not an upstream credential; CC Switch selects and injects the configured provider credential per request. Keep the proxy bound to `127.0.0.1` unless LAN exposure is explicitly required and trusted. The gateway adds model discovery and Messages routing only; legacy takeover, active-provider routing, failover, and provider **Switch** behavior remain unchanged.
