...
The Usage Trends card shows token volume and estimated cost together in one time-series chart. Token series use the left axis and estimated cost uses the right axis.

### Time Granularity

Choose **Auto** or a fixed interval from the granularity menu. Common minute, hour, and calendar-day presets are available, and **Custom** accepts any positive value in seconds, minutes, hours, or days. The badge next to the selector always shows the interval actually returned by the server.

To keep the chart responsive, a request that would produce more than 2,000 points is automatically adjusted to a coarser interval. Request details are retained for roughly 30 days; older records are stored as local-calendar-day rollups. When a range includes that older history, the chart switches to calendar-day precision and explains that sub-day detail cannot be reconstructed.

### Zoom and Navigation

- Pinch with two fingers (trackpad) to zoom around the pointer.
- Scroll vertically over the plot to scroll the page normally.
- Scroll horizontally (two fingers) to pan the time window.
- Drag with a mouse to pan through the visible window.
- Use the overview handles below the chart to select a range.
- Use the toolbar to zoom, pan left or right, or reset the full range.
- With the chart focused, press `+`/`-` to zoom, Left/Right to pan, Home/End to jump to either edge, and `0` to reset.

### Token and Cost Trends

The chart plots Input, Output, Cache Creation, and Cache Hit tokens as areas on the left axis, with estimated cost as a line on the right axis.

The tooltip shows the bucket start and end time, request count, token breakdown, cost, and actual granularity.
