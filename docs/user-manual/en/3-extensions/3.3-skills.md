...
CC Doctor comes pre-configured with the following GitHub repositories:
...
CC Doctor provides powerful search and filter features:
...
- **Automatic backup**: Before deletion, the skill is backed up to `~/.cc-doctor/skill-backups/`
- Removes the skill from all app directories (Claude, Codex, Gemini, OpenCode)
- Removes the skill from the SSOT directory (`~/.cc-doctor/skills/`)
...
CC Doctor compares installed skills with the remote repository version using **SHA-256 content hashes**. Whenever the remote has any content changes, the corresponding local skill card automatically shows an "Update available" indicator.
...
2. CC Doctor batch-downloads all skills with pending updates
...
| ------------------------ | --------------------------------------------------------------------- |
| **CC Doctor built-in**   | Default location `~/.cc-doctor/skills/`, managed by CC Doctor         |
| **`~/.agents/skills`**   | A shared directory conforming to community agent tool conventions, better for cross-tool collaboration |

### How to Switch

Select the target storage location from the settings or management menu in the Skills panel. The switch **does not lose skill state** — CC Doctor smoothly migrates existing skills to the new location.
...
v3.13.0 integrates **skills.sh** public registry search so you can discover community skills directly inside CC Doctor.
...
- CC Doctor has not finished the latest scan
