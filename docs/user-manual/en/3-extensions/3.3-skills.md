...
EC Switch comes pre-configured with the following GitHub repositories:
...
EC Switch provides powerful search and filter features:
...
- **Automatic backup**: Before deletion, the skill is backed up to `~/.ec-switch/skill-backups/`
- Removes the skill from all app directories (Claude, Codex, Gemini, OpenCode, Hermes)
- Removes the skill from the SSOT directory (`~/.ec-switch/skills/`)
...
EC Switch compares installed skills with the remote repository version using **SHA-256 content hashes**. Whenever the remote has any content changes, the corresponding local skill card automatically shows an "Update available" indicator.
...
2. EC Switch batch-downloads all skills with pending updates
...
| ------------------------ | --------------------------------------------------------------------- |
| **EC Switch built-in**   | Default location `~/.ec-switch/skills/`, managed by EC Switch         |
| **`~/.agents/skills`**   | A shared directory conforming to community agent tool conventions, better for cross-tool collaboration |

### How to Switch

Select the target storage location from the settings or management menu in the Skills panel. The switch **does not lose skill state** — EC Switch smoothly migrates existing skills to the new location.
...
v3.13.0 integrates **skills.sh** public registry search so you can discover community skills directly inside EC Switch.
...
- EC Switch has not finished the latest scan
