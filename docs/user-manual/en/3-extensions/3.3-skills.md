...
## Filter Installed Skills by Source

In the installed skills list toolbar, a **Source Filter** dropdown appears when skills come from multiple different sources, letting you quickly locate skills by their origin.

### Source Types

| Source | Description |
|--------|-------------|
| `owner/repo` | Skills installed from a GitHub repository, identified by `owner/name` |
| Local | Skills imported from existing apps or installed from ZIP, with no repository info |

### How to Use

1. Click the source dropdown in the toolbar
2. Select a target source (e.g., `anthropics/claude-skills`)
3. The list shows only skills from that source; the top count and per-app enable stats update accordingly
4. Select "All sources" to restore the full list

> **Note**: When all skills share the same source, the dropdown hides automatically.
