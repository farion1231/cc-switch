...
Only download EC Switch from **[ecswitch.io](https://ecswitch.io)**, **[GitHub Releases](https://github.com/farion1231/ec-switch/releases)**, or the project source repository. Any "EC Switch" site or client that asks for payment, top-ups, or login credentials is not official.
...
The CLI tools managed by EC Switch (Claude Code, Codex, Gemini CLI) require a Node.js environment.
...
1. Visit the [Releases page](https://github.com/farion1231/ec-switch/releases)
...
2. Open the DMG, or extract the zip to get `EC Switch.app`
3. Drag it to the Applications folder

### Signed and Notarized

EC Switch for macOS is signed and notarized by Apple. You can install and open it directly — no extra steps needed.
...
After installation, launch EC Switch:

1. The app window displays correctly
2. A EC Switch icon appears in the system tray
3. The app switcher shows enabled managed apps, and you can switch to the target app panel

## Auto Update

EC Switch includes built-in auto-update functionality:
...
- Move `EC Switch.app` to Trash
- Optional: Delete the configuration directory `~/.ec-switch/`

### Linux

```bash
# Debian/Ubuntu
sudo apt remove ec-switch

# ArchLinux
paru -R ec-switch-bin
```
