...
Click the "Refresh" button to re-detect. When an update is available, you can update a single tool or use "Update All". Missing tools show an install button; installed tools show an "Uninstall" button. Uninstall prefers each tool's official self-uninstall when available — `claude uninstall`, `codex uninstall`, `hermes uninstall --yes` — with the package manager (`npm uninstall -g` / `brew uninstall` / `volta uninstall` / `bun remove -g` / `pnpm remove -g`) as a fallback for package-manager-owned installs; Grok Build uses the package manager directly. Native installs that no package manager can be proven to own (for example grok or opencode installed by their own installer) prompt for manual removal instead, so a bare `npm uninstall -g` never deletes a different copy. Everything is anchored to the same install the command line actually uses. It asks for confirmation before removing anything. Installs, updates, and uninstalls run silently in the background—the button shows progress and versions refresh automatically when done.
