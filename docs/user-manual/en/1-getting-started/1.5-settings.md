...
Click the "Refresh" button to re-detect. When an update is available, you can update a single tool or use "Update All". Missing tools show an install button; installed tools show an "Uninstall" button. Automatic uninstall runs only when CC Switch can prove which installer owns the command-line default: Hermes uses its verified `hermes uninstall --yes` command, while package-manager installs use the matching anchored manager (`npm uninstall -g`, `brew uninstall` / `brew uninstall --cask`, `volta uninstall`, `bun remove -g`, or `pnpm remove -g`). Claude and Codex do not provide CLI uninstall subcommands. Native, broken, WSL, missing, or ambiguous installs whose ownership cannot be proven show manual-removal guidance; uninstall never falls back to a bare package-manager command that could delete another copy. CC Switch always asks for confirmation before removing anything. If another installation remains afterward, click "Uninstall" again to review and remove it. Installs, updates, and supported uninstalls run silently in the background—the button shows progress and versions refresh automatically when done.
