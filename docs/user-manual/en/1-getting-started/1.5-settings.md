...
- **Follow system proxy** (on by default): when no proxy URL is set, CC Switch uses the `HTTP_PROXY` / `HTTPS_PROXY` environment variables and the OS proxy settings. If requests keep failing after your proxy app quit or changed port, turn it off to connect directly. When a proxy URL is set, the system proxy and environment variables are ignored and the switch is disabled. Checking for app updates follows the same policy. Local-routing forwarding to Claude (Anthropic-format) providers ignores the system proxy: it connects directly unless a proxy URL is set here.
- The card shows the proxy actually in effect, its source (environment variables or OS settings) and whether the port is reachable, resolved for the Anthropic API host (bypass rules such as `NO_PROXY` apply per destination). If the system proxy settings change after startup, a notice appears and **Re-apply** picks them up without restarting.
