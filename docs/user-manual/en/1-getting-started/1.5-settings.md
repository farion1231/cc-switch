...
### Claude Background Notifications (Windows)

When enabled, CC Switch can receive local Claude Code background events and show Windows Toast notifications in the lower-right corner.

Phase 1 supports:

- **Notify on confirmation required**: shown when Claude Code needs a permission confirmation
- **Notify on round completion**: shown when Claude Code finishes the current round or enters `idle_prompt`

How to use:

1. Enable **Claude background notifications** in Settings
2. Keep CC Switch running
3. Click **Apply hooks** to write the configuration into `~/.claude/settings.json`

Notes:

- Windows only
- If CC Switch is not running, notifications will not appear, but Claude Code continues to work normally
- Hooks are written only to the user-level `~/.claude/settings.json`, not to project repository settings
- If the local listening port changes, re-apply the hooks
