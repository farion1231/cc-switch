...
This section describes how to configure CC Doctor according to your preferences.
...
CC Doctor supports three languages:
...
When enabled, CC Doctor automatically runs when the system starts.
...
Starting from v3.13.0, CC Doctor adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger CC Doctor via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If CC Doctor runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, CC Doctor automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that CC Doctor uses when opening a terminal.
...
The storage location for CC Doctor's own data, defaulting to `~/.cc-doctor/`.
...
Configure CC Doctor's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, CC Doctor automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
| Remote Directory | Remote storage path (default: `cc-doctor-sync`) |
...
- CC Doctor automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, CC Doctor displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current CC Doctor version number, with support for:
