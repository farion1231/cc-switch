...
This section describes how to configure EC Switch according to your preferences.
...
EC Switch supports four languages:
...
When enabled, EC Switch automatically runs when the system starts.
...
Starting from v3.13.0, EC Switch adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger EC Switch via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If EC Switch runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, EC Switch automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that EC Switch uses when opening a terminal.
...
The storage location for EC Switch's own data, defaulting to `~/.ec-switch/`.
...
The exported file name format is `ec-switch-export-{timestamp}.sql`.
...
Configure EC Switch's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, EC Switch automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
| Remote Directory | Remote storage path (default: `ec-switch-sync`) |
...
- EC Switch automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, EC Switch displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current EC Switch version number, with support for:
