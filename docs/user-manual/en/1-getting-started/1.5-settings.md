...
Click the "Refresh" button to re-detect. When an update is available, you can update a single tool or use "Update All". Missing tools show an install button; installed tools show an "Uninstall" button. Uninstall prefers each tool's official self-uninstall when available — `claude uninstall`, `grok uninstall`, `hermes uninstall --yes` — and falls back to the package manager (`npm uninstall -g` / `brew uninstall` / `volta uninstall` / `bun remove -g` / `pnpm remove -g`) for the rest, always anchored to the same install the command line actually uses. It asks for confirmation before removing anything. Installs, updates, and uninstalls run silently in the background—the button shows progress and versions refresh automatically when done.
