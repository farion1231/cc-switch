...
Click the "Refresh" button to re-detect. When an update is available, you can update a single tool or use "Update All". Missing tools show an install button; installed tools show an "Uninstall" button. Uninstall anchors to the same install the command line actually uses (npm/Homebrew/Volta/Bun/pnpm, or `claude uninstall` for native Claude) and asks for confirmation before removing anything. Installs, updates, and uninstalls run silently in the background—the button shows progress and versions refresh automatically when done.
