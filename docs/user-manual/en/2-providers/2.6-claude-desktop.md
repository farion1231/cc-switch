...
Model mapping mode is for providers that expose non-Claude models, such as DeepSeek and Kimi, or when EC Switch needs to convert the API format.
...
The reason is that Claude Desktop now rejects non-`claude-*` models, so EC Switch local routing performs a model mapping pass.
...
Not in model mapping mode. The Claude Desktop profile only stores safe `claude-*` routes and display names. Real upstream model names stay in the EC Switch provider configuration and are mapped when requests pass through the local gateway.
