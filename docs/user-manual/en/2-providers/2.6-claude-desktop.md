...
| Flatpak note | The Flatpak build of CC Switch always uses the host `~/.config` and does not follow a custom `XDG_CONFIG_HOME` |
...
## Migrate Official (1P) Data to a Third-Party (3P) Account

### Why history "disappears" after switching

Claude Desktop's official (1P) mode and third-party (3P) mode use **two completely separate data directories**:

- Official (1P): `~/Library/Application Support/Claude`
- Third-party (3P): `~/Library/Application Support/Claude-3p`

When you switch providers, CC Switch only rewrites the sign-in method and profile configuration — it **does not move data**. So after switching from official to third-party mode, the Code sessions and Cowork tasks in your official account are not deleted: they are still in the `Claude` directory, but third-party mode reads from `Claude-3p`, so they appear to be gone.

### The data migration feature

CC Switch provides an **opt-in, safe, reversible** local data migration that copies data from the official (1P) account into the third-party (3P) account.

On the Claude Desktop panel, click **Review & migrate…** on the "Migrate official (1P) data to this account" card to open the migration wizard.

The flow:

1. **Read-only scan**: lists the 1P / 3P data directories and the number of Code sessions and Cowork tasks. Nothing changes at this stage.
2. **Migration plan**: shows how many new records each component will copy, how many already exist in 3P (and will be skipped), and the estimated copy / backup sizes. If a problem is found (for example multiple candidate data directories, or the 3P directory is not initialized yet), it is listed first and you must resolve it before continuing.
3. **Explicit confirmation**: nothing runs until you tick the confirmation box. A **full backup of the target** is created first, and you are asked to quit Claude Desktop completely.
4. **Apply & verify**: records are copied one by one (existing ones are never overwritten). Afterwards you can **run verification** with one click.
5. **Reversible**: every migration records a ledger. **Undo migration** removes only what this migration copied in — it never deletes sessions or tasks you created after migrating.

Safety guarantees:

- The source (official 1P) directory is **read-only** and never modified
- Existing records in the target (3P) are **never overwritten**; duplicates are always skipped
- A backup is taken before any write; installs use a "stage → verify → atomic install" sequence
- Migration is refused while Claude Desktop is running
- When multiple candidate data directories exist it **never guesses** — it stops and asks you to confirm
- Chat bodies, audit logs, and upload / output files are copied **byte-for-byte**; only necessary structural path fields are rewritten
- The backup and ledger **do not** store chat bodies or any credentials

### What is migrated

| Component         | Notes                                                                     |
| ----------------- | ------------------------------------------------------------------------- |
| Code sessions     | Session index (transcript files are shared between 1P / 3P, no copy needed) |
| Cowork tasks      | Metadata + the matching session folder (audit, uploads, outputs)          |
| Scheduled tasks   | Imported **disabled**; enable them manually when needed                   |
| Project cache     | Only projects missing in 3P are copied                                    |
| Artifact registry | Merged by ID; existing entries are skipped                                |

### What is not migrated

- Cloud Chat conversation history
- Account preferences and credentials (Cookie / Token / Keychain / OAuth / API Key)
- Connectors / OAuth grants
- Skill / Plugin packaging and runtime authentication
- Various caches (Cache / Code Cache / GPUCache), `vm_bundles`, etc.

These stay signed in or configured per account.

> Data migration currently supports macOS only.
