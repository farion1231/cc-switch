...
### Web Account API Template

For proxy/self-hosted sites whose balance is returned by a "web account info" endpoint, typically `/api/v1/auth/me`. These sites usually accept only the **web login token** (Access Token), not the model-calling API Key:
...
({
  ...
  extractor: function(response) {
    if (response && typeof response.balance !== "undefined") {
      return {
        isValid: true,
        planName: response.email || "Default Plan",
...
> **Note**: The web login token and the model API Key are two separate credentials. If the `sk-...` Key used for model calls cannot query the balance (e.g. returns 401/403), grab the Access Token from the site's web session (DevTools → Network → `Authorization` header of the account-info request) and fill it in below.

**Configuration parameters**:
| Parameter | Description |
|-----------|-------------|
| Base URL | Site root address (without `/api/v1`-style prefixes) |
| Access Token | Web login token (not the model API Key) |

If the site endpoint is not `/api/v1/auth/me`, edit the URL in the script directly; add query parameters (e.g. `?timezone=Asia/Shanghai`) when the endpoint requires them.
