...
### Claude Aggregate Providers

An aggregate provider is a virtual Claude provider with no upstream endpoint or credentials of its own. It routes Haiku, Sonnet, Opus, and Fable requests to selected models from existing third-party providers. For example, DeepSeek can serve Haiku while Kimi serves Fable.

To configure one:

1. Add and verify the regular Claude providers that will be used as route targets.
2. Add another Claude provider and enable **Aggregate provider**.
3. Select a target provider and enter its upstream model ID for each tier you need. At least one tier is required.
4. Start [Local Routing](../4-proxy/4.1-service.md), enable Claude takeover, and then switch to the aggregate provider.

Notes:

- Aggregate providers require Local Routing and cannot be selected without takeover.
- Targets must be regular third-party providers in the same app. Self references, nested aggregates, and official providers that are blocked from Local Routing are not allowed.
- Remove a target from every aggregate route before deleting that provider.
- With automatic failover enabled, aggregate targets participate in the existing circuit-breaker and queue fallback behavior.
- An unconfigured tier does not automatically reuse another tier; configure every tier that the client will request.
