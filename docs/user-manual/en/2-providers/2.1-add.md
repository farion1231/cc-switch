...
| Azure OpenAI    | Azure OpenAI in Foundry Models (v1 Responses API) |
| AiHubMix        | AiHubMix aggregation service                      |
| DMXAPI          | DMXAPI proxy service                              |
| PackyCode       | PackyCode proxy service                           |
| Cubence         | Cubence service                                   |
...
### Azure OpenAI (Foundry v1) Recommended Example

```toml
model_provider = "azure"
model = "gpt-5.3-codex"  # replace with your actual deployment name
...
- Use `/openai/v1` in `base_url`.
- For the v1 API, you no longer need `query_params` with `api-version`.
- Set `AZURE_OPENAI_API_KEY` in your environment, and use `env_key = "AZURE_OPENAI_API_KEY"`.
- Official Azure Codex guide: https://learn.microsoft.com/en-us/azure/foundry/openai/how-to/codex?tabs=npm
