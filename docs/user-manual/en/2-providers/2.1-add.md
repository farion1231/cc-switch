...
Some third-party providers only support the **OpenAI Chat Completions** protocol or use non-GPT model names (such as Baidu Qianfan Coding Plan, StepFun Step Plan, or SiliconFlow). Codex natively understands only the OpenAI Responses API and GPT-series models, so these providers need CC Switch to convert the protocol and models locally.
...
> 💡 When you pick a Chat-format preset such as StepFun (Step Plan) or SiliconFlow, this toggle is enabled by default — no manual setup needed.
