...
| `requires_openai_auth` | No | Whether Codex forwards the auth.json API key to this provider (see note below) |

> **About `requires_openai_auth`**: CC Switch includes `requires_openai_auth = true` in newly generated third-party Codex configs by default (presets, the custom template, universal-provider sync, and deeplink imports). You can change this default with **Settings → Codex App Enhancements → Include requires_openai_auth in new Codex configs**. The toggle only affects newly generated configs — already-saved providers keep their stored TOML, and the field remains editable per provider in the config editor (removing it there now persists, including for universal-provider derived entries). Warning: without the field, Codex does not send the auth.json API key to a custom provider unless `env_key` is configured (local proxy takeover is unaffected). The unified-session-history and official proxy takeover injections always keep the field because it routes the ChatGPT login in auth.json.
...
Some third-party providers only support the **OpenAI Chat Completions** protocol or use non-GPT model names (such as Baidu Qianfan, StepFun, or SiliconFlow). Codex natively understands only the OpenAI Responses API and GPT-series models, so these providers need CC Switch to convert the protocol and models locally.
...
> 💡 When you pick a Chat-format preset such as StepFun or SiliconFlow, this toggle is enabled by default — no manual setup needed.
...
| **On/off switch only** | Reasoning can only be turned on or off; **levels have no effect** | Zhipu GLM, Qwen, MiniMax, Xiaomi MiMo, SiliconFlow |
