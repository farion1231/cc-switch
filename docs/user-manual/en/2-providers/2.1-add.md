...
- **App-specific Provider**: Only for the currently selected app (Claude Code / Claude Desktop / Codex / Gemini / OpenCode / OpenClaw / Hermes / Pi Agent)
...
Pi Agent provider configuration writes to `~/.pi/agent/models.json`.

In the Pi Agent panel:

1. Select a provider template at the top of the form.
2. Fill in provider information below the template selector.
3. Add at least one model ID. The model name defaults to the model ID and can be edited.
4. Preview the JSON diff.
5. Apply the change. cc-switch creates a backup before writing the file.

cc-switch does not modify `~/.pi/agent/auth.json` or `~/.pi/agent/settings.json` for this feature.
