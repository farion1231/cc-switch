...
| `goals` | No | Enables Codex `/goal` mode when set under `[features]`; without it, some API-key or custom endpoint setups may show "unable to set goal" |
...
If Codex recognizes `/goal` but cannot set a goal after switching to an API-key or custom endpoint provider, check that `goals = true` is present under `[features]` in `~/.codex/config.toml`, then restart Codex. If Goal mode asks for approval more often than expected, refresh the Codex settings page and check that `approval_policy` and `sandbox_mode` still match your intended permission level.
