...
## EC Switch Data Storage

### Storage Directory

Default location: `~/.ec-switch/`
...
```
~/.ec-switch/
├── ec-switch.db      # SQLite database (SSOT)
...
`ec-switch.db` is a SQLite database that stores:
...
Hermes uses YAML configuration. EC Switch writes MCP servers to `mcp_servers`, writes editable provider entries to `custom_providers`, reads read-only entries from Hermes' `providers` dict, and updates `model.provider` / `model.default` when switching providers.
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to EC Switch's "providers") |
...
EC Switch's priority when modifying configurations:

1. **EC Switch Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by EC Switch)
- EC Switch's `settings.json`

### Not Recommended to Edit Manually

- `ec-switch.db` database file
- Backup files
...
EC Switch v3.7.0 migrated from JSON files to SQLite:
