...
├── AGENTS.md         # System prompt
## DevEco Code Configuration

### Configuration Directory

Default: `~/.config/deveco/` (overridable via the `DEVECO_CONFIG_DIR` environment variable)

### Key Files

```
~/.config/deveco/
├── deveco.jsonc      # Main configuration file (JSON5; falls back to deveco.json, config.json)
├── AGENTS.md         # System prompt
└── skills/           # Skills directory
    └── ...
```
...
DevEco Code is built on OpenCode, so its configuration schema matches OpenCode's. The main sections are:

| Field | Description |
|-------|-------------|
| `provider` | Model provider configuration (mapped to CC Switch's "providers") |
| `mcp` | MCP server configuration (`local` / `remote`) |
| `model` / `small_model` | Default model references in `<provider>/<model>` form |
| `agent` | Per-agent model overrides (e.g. `agent.ui_verification.model`) |

The session database lives at `~/.local/share/deveco/deveco.db` (honours `XDG_DATA_HOME`; overridable via `DEVECO_DB`).
