...
## CC Doctor Data Storage

### Storage Directory

Default location: `~/.cc-doctor/`
...
```
~/.cc-doctor/
├── cc-doctor.db      # SQLite database (SSOT)
...
`cc-doctor.db` is a SQLite database that stores:
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to CC Doctor's "providers") |
...
CC Doctor's priority when modifying configurations:

1. **CC Doctor Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by CC Doctor)
- CC Doctor's `settings.json`

### Not Recommended to Edit Manually

- `cc-doctor.db` database file
- Backup files
...
CC Doctor v3.7.0 migrated from JSON files to SQLite:
