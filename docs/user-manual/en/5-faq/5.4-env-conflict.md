...
EC Switch automatically detects conflicts between system environment variables and app configurations, preventing configurations from being unexpectedly overridden.
...
Found X environment variables that may conflict with EC Switch configuration
[Expand] [Dismiss]
```
...
3. EC Switch will automatically back up and remove the selected variables

### Automatic Backup

A backup is automatically created before removal:

- Backup location: `~/.ec-switch/env-backups/`
...
If you prefer not to use EC Switch to remove variables, you can handle them manually:
...
- EC Switch provider configurations being overridden
- API requests being sent to the wrong endpoint
- Using the wrong API key

## Best Practices

1. **Use EC Switch to manage configurations**: Avoid setting API keys in system environment variables
...
1. Find the backup file: `~/.ec-switch/env-backups/`
