...
CC Doctor automatically detects conflicts between system environment variables and app configurations, preventing configurations from being unexpectedly overridden.
...
Found X environment variables that may conflict with CC Doctor configuration
[Expand] [Dismiss]
```
...
3. CC Doctor will automatically back up and remove the selected variables

### Automatic Backup

A backup is automatically created before removal:

- Backup location: `~/.cc-doctor/env-backups/`
...
If you prefer not to use CC Doctor to remove variables, you can handle them manually:
...
- CC Doctor provider configurations being overridden
- API requests being sent to the wrong endpoint
- Using the wrong API key

## Best Practices

1. **Use CC Doctor to manage configurations**: Avoid setting API keys in system environment variables
...
1. Find the backup file: `~/.cc-doctor/env-backups/`
