...
### Codex Memories / Chronicle errors with "model not found"

**Symptom**: At Codex startup, errors like `model "MiniMax-M3" not found` or `[memories].extract_model unavailable` — the memory feature is effectively disabled.

**Cause**: The `[memories]` table in `~/.codex/config.toml` hard-codes a specific model that the currently active custom provider does not serve.

**Fix**: Edit the Codex provider and check the **Enable Codex memories** toggle (leftmost of the four checkboxes in the `config.toml (TOML)` row). cc-switch will sync the active `model` into `[memories].extract_model` / `[memories].consolidation_model` and **auto-write** `[features].memories = true` (the Codex feature flag). Unchecking only removes those three keys; other `[memories]` / `[features]` fields are preserved. See [2.7 Codex Memories / Chronicle](../2-providers/2.7-codex-memories.md).
