...
### Linux title-bar buttons (close / minimize / maximize) do not respond

**Cause**: On Linux (especially Wayland), the first time the window is shown — or after `hide()` to the tray and `show()` again — GTK's client-side decorations may not finish negotiating their input surfaces with the compositor. Typical symptom: the page still accepts clicks, but the native title-bar buttons do nothing.

CC Switch automatically reactivates focus and rebinds the title-bar buttons on startup, tray restore, deep links, and single-instance activation. If the buttons are still dead, try:

1. Enable **Settings → Window Behavior → Enable app-level window controls**;
2. Quit from the system tray menu;
3. AppImage users can set `CC_SWITCH_GDK_BACKEND=x11` to switch the window backend.
