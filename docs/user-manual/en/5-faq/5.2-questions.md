...
CC Doctor for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/diaojz/cc-doctor/releases).
...
2. Add CC Doctor to your antivirus software's whitelist
...
chmod +x CC-Doctor-*.AppImage

# If it still fails, try
./CC-Doctor-*.AppImage --no-sandbox
```
...
**Solutions**:
1. Check if the `~/.cc-doctor/` directory exists
2. Restore from backup: `~/.cc-doctor/backups/`
...
**Solutions**:
1. Confirm the file was exported by CC Doctor
...
**Windows**:
- Check taskbar settings to ensure the CC Doctor icon is not hidden
...
2. Restart the app
3. Delete `~/.cc-doctor/settings.json` to reset settings
...
3. If using Homebrew: `brew upgrade --cask cc-doctor`

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and CC Doctor runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, CC Doctor covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `ccdoctor://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/diaojz/cc-doctor/issues)
...
- macOS/Linux: `~/.cc-doctor/logs/`
- Windows: `%APPDATA%\cc-doctor\logs\`
