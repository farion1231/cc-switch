...
EC Switch for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/farion1231/ec-switch/releases).
...
2. Add EC Switch to your antivirus software's whitelist
...
**Solutions**:
1. Check if the `~/.ec-switch/` directory exists
2. Restore from backup: `~/.ec-switch/backups/`
...
**Solutions**:
1. Confirm the file was exported by EC Switch
...
**Windows**:
- Check taskbar settings to ensure the EC Switch icon is not hidden
...
2. Restart the app
3. Delete `~/.ec-switch/settings.json` to reset settings
...
3. If using Homebrew: `brew upgrade --cask ec-switch`

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and EC Switch runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, EC Switch covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `ecswitch://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/farion1231/ec-switch/issues)
...
- macOS/Linux: `~/.ec-switch/logs/`
- Windows: `%APPDATA%\ec-switch\logs\`
