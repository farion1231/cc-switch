...
| `app` | Yes | App type: `claude` / `claude-desktop` / `codex` / `gemini` / `grokbuild` / `opencode` / `openclaw` / `hermes` |
| `name` | Yes | Name |
...
| `haikuModel` | No | Haiku model (Claude / Claude Desktop only) |
| `sonnetModel` | No | Sonnet model (Claude / Claude Desktop only) |
| `opusModel` | No | Opus model (Claude / Claude Desktop only) |
| `claudeDesktopMode` | No | Claude Desktop write mode: `direct` (default) or `proxy` (local routing + model mapping). Only applies when `app=claude-desktop`; under `proxy` the three model params above are read as upstream model mappings |
| `notes` | No | Notes |
| `icon` | No | Icon |
...
By default the provider is written in direct mode, which requires the upstream to expose the native Claude Messages API and to use `claude-*` role model names:

```
ccswitch://v1/import?resource=provider&app=claude-desktop&name=My%20Desktop&endpoint=https%3A%2F%2Fapi.example.com&apiKey=sk-xxx&model=claude-sonnet-5
```

For non-Claude upstreams (grok, deepseek, ...), add `claudeDesktopMode=proxy` and use `sonnetModel` / `opusModel` / `haikuModel` to declare the upstream model each role should request:

```
ccswitch://v1/import?resource=provider&app=claude-desktop&name=My%20Desktop&endpoint=https%3A%2F%2Fapi.example.com&apiKey=sk-xxx&claudeDesktopMode=proxy&sonnetModel=grok-4.6&opusModel=deepseek-v4.1&haikuModel=grok-4.5-mini
```

`proxy` mode requires CC Switch local routing to stay running, and Claude Desktop must be restarted after switching providers.
