...
For Claude Desktop provider imports, use `app=claude-desktop`. The aliases `claude_desktop` and `claudedesktop` are also accepted and normalized to `claude-desktop`. URL parameters and Base64-encoded inline Claude `env` configuration are supported; URL parameters take precedence. Claude Desktop is not supported for prompt or MCP imports. Desktop imports are saved without activation unless `enabled=true`. Model parameters use the existing Desktop route conversion: Desktop-compatible model IDs use direct mode; other upstream model IDs use proxy mappings.
