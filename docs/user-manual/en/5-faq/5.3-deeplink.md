...
EC Switch supports the `ecswitch://` deep link protocol, enabling one-click configuration import via links.
...
EC Switch provides an online deep link generator tool:

**URL**: [https://farion1231.github.io/ec-switch/deplink.html](https://farion1231.github.io/ec-switch/deplink.html)
...
```
ecswitch://v1/import?resource={type}&app={app}&name={name}&...
```
...
**Example**:
```
ecswitch://v1/import?resource=provider&app=claude&name=My%20Provider&endpoint=https%3A%2F%2Fapi.example.com&apiKey=sk-xxx
```
...
```
ecswitch://v1/import?resource=mcp&apps=claude,codex&config=%7B%22command%22%3A%22uvx%22%2C%22args%22%3A%5B%22mcp-server-fetch%22%5D%7D&name=mcp-fetch
```

### Import Prompt Preset

```
ecswitch://v1/import?resource=prompt&app=claude&name=%E4%BB%A3%E7%A0%81%E5%AE%A1%E6%9F%A5&content=%23%20%E8%A7%92%E8%89%B2%0A%E4%BD%A0%E6%98%AF%E4%B8%80%E4%B8%AA%E4%B8%93%E4%B8%9A%E7%9A%84%E4%BB%A3%E7%A0%81%E5%AE%A1%E6%9F%A5%E4%B8%93%E5%AE%B6
```

### Import Skill

```
ecswitch://v1/import?resource=skill&name=my-skill&repo=owner/repo&directory=skills/my-skill&branch=main
```
...
const url = `ecswitch://v1/import?${params.toString()}`;
```

### Online Tool

Using EC Switch's official online deep link generator tool is more convenient.
...
1. The system asks whether to open EC Switch
2. After confirming, EC Switch opens
...
EC Switch automatically registers the `ecswitch://` protocol during installation.
...
```bash
/usr/bin/open -a "EC Switch" --args --register-protocol
```

**Windows**:
Reinstall the app, or check the registry:
```
HKEY_CLASSES_ROOT\ecswitch
```
...
```
ecswitch://v1/import?resource=provider&app=claude&name=Test%20Provider&apiKey=sk-xxx&endpoint=https%3A%2F%2Fapi.example.com
```

### Example: Import MCP Server

```
ecswitch://v1/import?resource=mcp&name=mcp-fetch&apps=claude,codex,gemini&config=%7B%22command%22%3A%22uvx%22%2C%22args%22%3A%5B%22mcp-server-fetch%22%5D%7D
```
