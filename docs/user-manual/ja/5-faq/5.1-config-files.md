...
```
~/.cc-doctor/
├── cc-doctor.db      # SQLite データベース（SSOT）
...
`cc-doctor.db` は SQLite データベースで、以下を保存しています：
...
| フィールド | 説明 |
|------|------|
| `models.providers` | プロバイダー設定（CC Doctor の「プロバイダー」にマッピング） |
| `env` | 環境変数設定 |
...
CC Doctor が設定を変更する際の優先順位：

1. **CC Doctor データベース** - 単一事実源 (SSOT)
...
- CLI ツールの設定ファイル（CC Doctor がバックフィルする）
- CC Doctor の `settings.json`

### 手動編集を推奨しないもの

- `cc-doctor.db` データベースファイル
- バックアップファイル
...
CC Doctor v3.7.0 で JSON ファイルから SQLite に移行しました：
