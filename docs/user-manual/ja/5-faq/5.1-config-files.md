...
デフォルト: `~/.config/deveco/`（環境変数 `DEVECO_CONFIG_DIR` で上書き可能）

### 主要ファイル

```
~/.config/deveco/
├── deveco.jsonc      # メイン設定ファイル（JSON5。deveco.json、config.json の順にフォールバック）
├── AGENTS.md         # システムプロンプト
└── skills/           # スキルディレクトリ
    └── ...
```
...
DevEco Code は OpenCode をベースにしているため、設定スキーマは OpenCode と同じです。主なセクションは次のとおりです。

| フィールド | 説明 |
|-------|-------------|
| `provider` | モデルプロバイダの設定（CC Switch の「プロバイダ」に対応） |
| `mcp` | MCP サーバーの設定（`local` / `remote`） |
| `model` / `small_model` | `<provider>/<model>` 形式のデフォルトモデル参照 |
| `agent` | エージェント単位のモデル上書き（例: `agent.ui_verification.model`） |

セッションデータベースは `~/.local/share/deveco/deveco.db` にあります（`XDG_DATA_HOME` に従います。`DEVECO_DB` で上書き可能）。
