...
```
~/.ec-switch/
├── ec-switch.db      # SQLite データベース（SSOT）
...
`ec-switch.db` は SQLite データベースで、以下を保存しています：
...
Hermes は YAML 設定を使用します。EC Switch は MCP サーバーを `mcp_servers` に書き込み、編集可能なプロバイダーエントリを `custom_providers` に書き込み、Hermes の `providers` dict にある読み取り専用エントリを読み取り、プロバイダー切り替え時に `model.provider` / `model.default` を更新します。
...
| フィールド | 説明 |
|------|------|
| `models.providers` | プロバイダー設定（EC Switch の「プロバイダー」にマッピング） |
| `env` | 環境変数設定 |
...
EC Switch が設定を変更する際の優先順位：

1. **EC Switch データベース** - 単一事実源 (SSOT)
...
- CLI ツールの設定ファイル（EC Switch がバックフィルする）
- EC Switch の `settings.json`

### 手動編集を推奨しないもの

- `ec-switch.db` データベースファイル
- バックアップファイル
...
EC Switch v3.7.0 で JSON ファイルから SQLite に移行しました：
