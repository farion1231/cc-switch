...
| `app` | はい | アプリタイプ（provider、prompt で必要）：`claude` / `codex` / `gemini` / `grokbuild` / `opencode` / `openclaw` / `hermes`。prompt ではさらに `pi` に対応 |
| `name` | provider、prompt で必須 | 名前（MCP のサーバー ID は `config` 内の `mcpServers` のキーから取得。Skill では不要） |

Claude Desktop のプロバイダーをインポートする場合は、`app=claude-desktop` を使用します。`claude_desktop` と `claudedesktop` も受け付け、`claude-desktop` に正規化します。URL パラメータと Base64 エンコードされた Claude の `env` インライン設定に対応し、URL パラメータが優先されます。Claude Desktop の Prompt と MCP のインポートには対応していません。 Desktop のインポートは設定の保存のみを行い、`enabled=true` の場合に有効化します。モデルパラメータは既存の Desktop ルート変換を利用し、Desktop 対応のモデル ID は直接接続、それ以外はプロキシマッピングを使用します。
...
| パラメータ | 必須 | 説明 |
|------|------|------|
| `content` | はい | プロンプト内容。Base64 エンコードが必要（さらに URL エスケープ） |
| `description` | いいえ | 説明 |
| `enabled` | いいえ | 有効にするかどうか（ブール値） |
...
| パラメータ | 必須 | 説明 |
|------|------|------|
| `apps` | はい | アプリリスト（カンマ区切り、例：`claude,codex,gemini,grokbuild`）。指定可能な値：`claude` / `codex` / `gemini` / `grokbuild` / `opencode` / `hermes`。MiniMax Code は現在非対応 |
| `config` | はい | MCP 設定の JSON。形式は `{"mcpServers": {"<サーバー ID>": {...}}}` で、Base64 エンコードが必要（さらに URL エスケープ） |
| `enabled` | いいえ | 有効にするかどうか（ブール値） |
...
ここで `config` は `{"mcpServers":{"mcp-fetch":{"command":"uvx","args":["mcp-server-fetch"]}}}` を Base64 エンコードしたものです（末尾の `=` は `%3D` にエスケープ）。
...
ここで `content` は、中国語のサンプルプロンプト（見出し `# 角色` の後に、プロのコードレビュー担当者という役割を指定する 1 文が続くもの）を Base64 エンコードしたものです（`+` は `%2B` にエスケープ）。
