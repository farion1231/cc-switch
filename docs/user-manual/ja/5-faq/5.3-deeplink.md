...
Claude Desktop のプロバイダーをインポートする場合は、`app=claude-desktop` を使用します。`claude_desktop` と `claudedesktop` も受け付け、`claude-desktop` に正規化します。URL パラメータと Base64 エンコードされた Claude の `env` インライン設定に対応し、URL パラメータが優先されます。Claude Desktop の Prompt と MCP のインポートには対応していません。
