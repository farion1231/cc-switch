...
| `app` | はい | アプリタイプ：`claude` / `claude-desktop` / `codex` / `gemini` / `grokbuild` / `opencode` / `openclaw` / `hermes` |
| `name` | はい | 名前 |
...
| `model` | いいえ | デフォルトモデル |
| `haikuModel` | いいえ | Haiku モデル（Claude / Claude Desktop のみ） |
| `sonnetModel` | いいえ | Sonnet モデル（Claude / Claude Desktop のみ） |
| `opusModel` | いいえ | Opus モデル（Claude / Claude Desktop のみ） |
| `claudeDesktopMode` | いいえ | Claude Desktop の書き込みモード：`direct`（デフォルト、直接接続）または `proxy`（ローカルルーティング + モデルマッピング）。`app=claude-desktop` の場合のみ有効。`proxy` モードでは上記 3 つのモデルパラメータがアップストリームモデルのマッピングとして解釈されます |
| `notes` | いいえ | メモ |
| `icon` | いいえ | アイコン |
...
### Claude Desktop プロバイダーのインポート

デフォルトでは直接接続モードで書き込まれます。アップストリームが Claude ネイティブの Messages API を公開しており、モデル名が `claude-*` のロール名に準拠している必要があります：
...
アップストリームが非 Claude モデル（grok、deepseek など）の場合は `claudeDesktopMode=proxy` を付け、`sonnetModel` / `opusModel` / `haikuModel` で各ロールが実際に要求するアップストリームモデルを指定します：
...
`proxy` モードでは CC Switch のローカルルーティングを実行したままにする必要があり、プロバイダーを切り替えた後は Claude Desktop を再起動しないと反映されません。
