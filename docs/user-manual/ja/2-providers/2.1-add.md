...
| Azure OpenAI | Foundry Models の Azure OpenAI（v1 Responses API） |
| AiHubMix     | AiHubMix 統合サービス                              |
| DMXAPI       | DMXAPI 中継サービス                                |
| PackyCode    | PackyCode 中継サービス                             |
| Cubence      | Cubence サービス                                   |
...
### Azure OpenAI（Foundry v1）推奨例

```toml
model_provider = "azure"
model = "gpt-5.3-codex"  # 実際のデプロイ名に置き換えてください
...
- `base_url` には `/openai/v1` を含めてください。
- v1 API では `query_params` による `api-version` 指定は不要です。
- 環境変数 `OPENAI_API_KEY` を設定し、`env_key = "OPENAI_API_KEY"` を使用してください。
- Azure 公式 Codex ガイド: https://learn.microsoft.com/en-us/azure/foundry/openai/how-to/codex?tabs=npm
