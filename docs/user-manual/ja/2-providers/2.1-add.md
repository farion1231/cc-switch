...
| `requires_openai_auth` | いいえ | Codex が auth.json の API キーをこのプロバイダーに送信するかどうか（下記の説明を参照） |

> **`requires_openai_auth` について**：CC Switch は既定で、新しく生成されるサードパーティ Codex 設定（プリセット、カスタムテンプレート、統一プロバイダー同期、ディープリンク取り込み）に `requires_openai_auth = true` を含めます。この既定値は **設定 → Codex アプリ拡張 → 新規 Codex 設定に requires_openai_auth を含める** で変更できます。トグルは新規生成される設定にのみ影響し、保存済みプロバイダーの TOML は変更されません。このフィールドはプロバイダーごとに設定エディタで追加・削除でき、削除は永続化されます（統一プロバイダー派生エントリも同様）。警告：このフィールドがない場合、`env_key` を設定しない限り Codex は auth.json の API キーをカスタムプロバイダーに送信しません（ローカルプロキシ接管には影響しません）。セッション履歴統一および公式プロキシ接管の注入では、auth.json の ChatGPT ログインをルーティングするために常にこのフィールドが保持されます。
...
一部の第三者プロバイダーは **OpenAI Chat Completions** プロトコルのみをサポートするか、非 GPT モデル名（Baidu Qianfan、StepFun、SiliconFlow など）を使用します。Codex はネイティブには OpenAI Responses API と GPT 系列モデルのみを認識するため、これらのプロバイダーでは CC Switch がローカルでプロトコルとモデルを変換する必要があります。
...
> 💡 StepFun や SiliconFlow などの Chat 形式プリセットを選択すると、このトグルはデフォルトで有効になっており、手動設定は不要です。
...
| **オン / オフ切替のみ** | 思考のオン / オフのみ、**レベルは無効** | Zhipu GLM、Qwen、MiniMax、Xiaomi MiMo、SiliconFlow |
