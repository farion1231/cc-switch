...
集約プロバイダーは、独自のアップストリーム URL や認証情報を持たない仮想 Claude プロバイダーです。Haiku、Sonnet、Opus、Fable の各リクエストを、既存のサードパーティプロバイダーで指定したモデルへ振り分けます。たとえば、Haiku を DeepSeek、Fable を Kimi に担当させることができます。

設定手順：

1. ルート先に使用する通常の Claude プロバイダーを追加し、接続を確認します。
2. 新しい Claude プロバイダーを追加して **集約プロバイダー** を有効にします。
3. 必要な各ティアで対象プロバイダーとアップストリームのモデル ID を指定します。少なくとも1つのティアが必要です。
4. [ローカルルーティング](../4-proxy/4.1-service.md)を起動して Claude の引き継ぎを有効にし、集約プロバイダーへ切り替えます。

注意事項：

- 集約プロバイダーはローカルルーティングが必須で、引き継ぎなしでは選択できません。
- 対象には同じアプリ内の通常のサードパーティプロバイダーを使用します。自己参照、集約プロバイダーの入れ子、ローカルルーティングが禁止されている公式プロバイダーは指定できません。
- 対象プロバイダーを削除する前に、すべての集約ルートから参照を削除してください。
- 自動フェイルオーバー有効時、集約ルートの対象にも既存のサーキットブレーカーとキューのフォールバックが適用されます。
- 未設定のティアが別のティアを自動利用することはありません。クライアントが使用する各ティアを設定してください。
