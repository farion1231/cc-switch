...
| `goals` | いいえ | `[features]` 配下で `true` にすると Codex の `/goal` モードを有効化します。一部の API Key またはカスタムエンドポイント設定では、これがないと「目標を設定できない」エラーになる場合があります |
| `base_url` | はい | API エンドポイントアドレス |
| `wire_api` | いいえ | API プロトコルタイプ（通常 `responses`） |
| `requires_openai_auth` | いいえ | OpenAI 認証方式を使用するかどうか |

API Key またはカスタムエンドポイントのプロバイダーへ切り替えた後、Codex が `/goal` を認識しているのに目標を設定できない場合は、`~/.codex/config.toml` の `[features]` に `goals = true` があることを確認し、Codex を再起動してください。Goal mode 有効化後に想定より頻繁に承認を求められる場合は、Codex の設定画面を再読み込みし、`approval_policy` と `sandbox_mode` が意図した権限と一致しているか確認してください。
