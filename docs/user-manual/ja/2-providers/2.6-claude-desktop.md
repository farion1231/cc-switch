...
| Flatpak 注記 | Flatpak 版 CC Switch はホストの `~/.config` を固定で使用し、独自の `XDG_CONFIG_HOME` には追従しません |
...
## 公式 (1P) データをサードパーティ (3P) アカウントへ移行する

### 切り替え後に履歴が「消える」理由

Claude Desktop の公式 (1P) モードとサードパーティ (3P) モードは、**完全に別々のデータディレクトリ**を使用します。

- 公式 (1P)：`~/Library/Application Support/Claude`
- サードパーティ (3P)：`~/Library/Application Support/Claude-3p`

プロバイダーを切り替えても、CC Switch はサインイン方法と profile 設定を書き換えるだけで、**データは移動しません**。そのため公式モードからサードパーティモードへ切り替えると、公式アカウントにあった Code セッションや Cowork タスクは削除されていませんが、`Claude` ディレクトリに残ったままになります。サードパーティモードは `Claude-3p` を読むため、見えなくなってしまうのです。

### データ移行機能

CC Switch は、公式 (1P) アカウントのデータをサードパーティ (3P) アカウントへコピーする、**オプトイン・安全・元に戻せる**ローカルデータ移行を提供します。

Claude Desktop パネル上部の「公式 (1P) データをこのアカウントへ移行」カードで **確認して移行…** をクリックすると、移行ウィザードが開きます。

流れ：

1. **読み取り専用スキャン**：1P / 3P のデータディレクトリと、Code セッション・Cowork タスクの件数を表示します。この段階では何も変更しません。
2. **移行プラン**：各コンポーネントがコピーする新規レコード数、3P に既に存在する件数（スキップされる）、推定コピー / バックアップサイズを表示します。問題が見つかった場合（複数の候補ディレクトリがある、3P ディレクトリが未初期化など）は先に一覧表示され、解決するまで続行できません。
3. **明示的な確認**：確認ボックスにチェックするまで実行されません。最初に**ターゲットの完全バックアップ**を作成し、Claude Desktop を完全に終了するよう求められます。
4. **実行と検証**：レコードを 1 件ずつコピーします（既存のものは上書きしません）。完了後、ワンクリックで**検証を実行**できます。
5. **元に戻せる**：毎回の移行は台帳に記録されます。**移行を元に戻す**と、この移行でコピーした内容だけを削除し、移行後に作成したセッションやタスクは決して削除しません。

安全保証：

- ソース（公式 1P）ディレクトリは**読み取り専用**で、一切変更しません
- ターゲット (3P) の既存レコードは**上書きせず**、重複は常にスキップします
- 書き込み前にバックアップを作成し、「ステージ → 検証 → アトミックにインストール」の手順で実行します
- Claude Desktop 起動中は移行を拒否します
- 候補ディレクトリが複数ある場合は**推測せず**、停止して確認を求めます
- チャット本文・audit ログ・アップロード / 出力ファイルは**バイト単位でそのまま**コピーし、必要な構造化パスフィールドのみ書き換えます
- バックアップと台帳には、チャット本文や認証情報を**一切保存しません**

### 移行されるもの

| コンポーネント   | 説明                                                                        |
| ---------------- | --------------------------------------------------------------------------- |
| Code セッション  | セッションインデックス（トランスクリプトは 1P / 3P で共有のためコピー不要） |
| Cowork タスク    | メタデータ + 対応するセッションフォルダ（audit、uploads、outputs）          |
| Scheduled タスク | **無効のまま**インポートされます。必要に応じて手動で有効化してください      |
| Project キャッシュ | 3P に不足しているプロジェクトのみコピーします                             |
| Artifact レジストリ | ID でマージし、既存のものはスキップします                                |

### 移行されないもの

- クラウド Chat 会話履歴
- アカウント設定と認証情報（Cookie / Token / Keychain / OAuth / API Key）
- Connectors / OAuth 認可
- Skill / Plugin のパッケージングとランタイム認証
- 各種キャッシュ（Cache / Code Cache / GPUCache）、`vm_bundles` など

これらはアカウントごとにサインイン・設定されたままです。

> データ移行は現在 macOS のみサポートしています。
