...
> **注意**：OpenClaw と Claude Desktop は現在 EC Switch MCP 同期に対応していません。MCP 機能は Claude、Codex、Gemini、OpenCode、Hermes に対応しています。

### スイッチの動作

あるアプリのスイッチをオンにすると、EC Switch は以下を実行します：
...
あるアプリのスイッチをオフにすると、EC Switch は以下を実行します：
...
CLI ツールで既に MCP サーバーを設定している場合、EC Switch にインポートできます：

1. 「インポート」ボタンをクリック
2. インポートするアプリを選択（Claude/Codex/Gemini/OpenCode/Hermes）
3. EC Switch が既存の設定を読み取ってインポート
