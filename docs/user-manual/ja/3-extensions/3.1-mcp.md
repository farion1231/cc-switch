...
あるアプリのスイッチをオンにすると、CC Doctor は以下を実行します：
...
あるアプリのスイッチをオフにすると、CC Doctor は以下を実行します：
...
CLI ツールで既に MCP サーバーを設定している場合、CC Doctor にインポートできます：

1. 「インポート」ボタンをクリック
2. インポートするアプリを選択（Claude/Codex/Gemini/OpenCode）
3. CC Doctor が既存の設定を読み取ってインポート
