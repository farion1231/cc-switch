...
Claude Code 网关通过一个本地端点公开多个已启用路由的 Claude 供应商模型：

```text
http://127.0.0.1:15721/claude-router
```

在每个 Claude 供应商中开启 **启用于路由**，并添加模型别名、显示名称和上游模型。路由启用状态独立于 **切换**、故障转移队列和 **启动 Claude（隔离）**。公开模型 ID 使用 `ccswitch/claude/<provider>/<alias>` 格式，其中两个动态段采用 Base64 URL-safe 编码；这既满足 Claude Code 的模型发现过滤规则，也能无歧义地标识供应商。

打开 **设置 > 高级 > 代理服务 > Claude Code 网关 > 设置**，将以下片段合并到 VS Code `settings.json`：
...
如果 `claudeCode.environmentVariables` 已存在，请把这些条目合并到现有数组中，不要替换整个数组。修改设置后新建一个 Claude 会话；仅当模型列表仍被缓存时，再运行 **Developer: Reload Window**。

该网关面向本机使用。`PROXY_MANAGED` 是哨兵值，不是上游凭据；CC Switch 会为每个请求选择指定供应商并注入其凭据。除非明确需要且信任局域网访问，否则请让代理绑定 `127.0.0.1`。网关只新增模型发现和 Messages 路由；原有代理接管、当前供应商路由、故障转移和供应商 **切换** 语义保持不变。
