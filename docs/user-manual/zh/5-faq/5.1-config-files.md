...
默认：`~/.config/deveco/`（可通过 `DEVECO_CONFIG_DIR` 环境变量覆盖）

### 主要文件

```
~/.config/deveco/
├── deveco.jsonc      # 主配置文件（JSON5；回退顺序 deveco.json、config.json）
├── AGENTS.md         # 系统提示词
└── skills/           # 技能目录
    └── ...
```
...
DevEco Code 基于 OpenCode 二次开发，配置结构与 OpenCode 一致。主要字段如下：

| 字段 | 说明 |
|-------|------|
| `provider` | 模型供应商配置（对应 CC Switch 的「供应商」） |
| `mcp` | MCP 服务器配置（`local` / `remote`） |
| `model` / `small_model` | 默认模型引用，格式为 `<provider>/<model>` |
| `agent` | 各 Agent 的模型覆盖（如 `agent.ui_verification.model`） |

会话数据库位于 `~/.local/share/deveco/deveco.db`（遵循 `XDG_DATA_HOME`，可通过 `DEVECO_DB` 覆盖）。
