...
Hermes 使用 YAML 配置。EC Switch 将 MCP 服务器写入 `mcp_servers`，将可编辑的供应商条目写入 `custom_providers`，读取 Hermes `providers` 字典中的只读条目，并在切换供应商时更新 `model.provider` / `model.default`。
...
| 字段 | 说明 |
|------|------|
| `models.providers` | 供应商配置（映射为 EC Switch 的"供应商"） |
| `env` | 环境变量配置 |
...
- CLI 工具的配置文件（会被 EC Switch 回填）
- EC Switch 的 `settings.json`
...
EC Switch v3.7.0 从 JSON 文件迁移到 SQLite：
