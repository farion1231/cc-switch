...
| 字段 | 说明 |
|------|------|
| `models.providers` | 供应商配置（映射为 CC Doctor 的"供应商"） |
| `env` | 环境变量配置 |
...
- CLI 工具的配置文件（会被 CC Doctor 回填）
- CC Doctor 的 `settings.json`
...
CC Doctor v3.7.0 从 JSON 文件迁移到 SQLite：
