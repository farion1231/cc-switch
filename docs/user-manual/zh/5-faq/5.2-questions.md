...
### Codex 记忆/Chronicle 功能报错「model not found」

**现象**：启动 Codex 时提示类似 `model "MiniMax-M3" not found` 或 `[memories].extract_model` 不可用，记忆功能无法启用。

**原因**：`~/.codex/config.toml` 的 `[memories]` 段里硬编码了某个具体模型，但当前激活的自定义供应商不提供该模型。

**解决**：编辑 Codex 供应商，勾选「**启用 Codex 记忆功能**」开关（位于 4 个复选框的最左侧）。cc-switch 会同时把活动 `model` 同步到 `[memories].extract_model` / `[memories].consolidation_model`，并自动写入 `[features].memories = true` 这个 Codex feature flag。取消勾选时只删这三处键/段，其他 `[memories]` / `[features]` 字段保留。详见 [2.7 Codex 记忆/Chronicle 功能](../2-providers/2.7-codex-memories.md)。
