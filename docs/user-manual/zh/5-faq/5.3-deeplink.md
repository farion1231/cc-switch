...
| `app` | 是 | 应用类型（provider、prompt 需要）：`claude` / `codex` / `gemini` / `grokbuild` / `opencode` / `openclaw` / `hermes`；prompt 另支持 `pi` |
| `name` | provider、prompt 必填 | 名称（MCP 的服务器 ID 取自 `config` 中 `mcpServers` 的 key；Skill 不需要） |

导入 Claude Desktop 供应商时，使用 `app=claude-desktop`。同时兼容 `claude_desktop` 和 `claudedesktop`，并统一为 `claude-desktop`。支持 URL 参数和 Base64 编码的 Claude `env` 内嵌配置，URL 参数优先。Prompt 和 MCP 导入不支持 Claude Desktop。 Desktop 导入默认只保存配置，仅在 `enabled=true` 时启用。模型参数复用现有 Desktop 路由转换：Desktop 可识别的模型 ID 使用直连模式，其他上游模型 ID 使用代理映射。
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `content` | 是 | 提示词内容，须 Base64 编码（再做 URL 转义） |
| `description` | 否 | 描述 |
| `enabled` | 否 | 是否启用（布尔值） |
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `apps` | 是 | 应用列表（逗号分隔，如 `claude,codex,gemini,grokbuild`）。可用值：`claude` / `codex` / `gemini` / `grokbuild` / `opencode` / `hermes`；MiniMax Code 暂不支持 |
| `config` | 是 | MCP 配置 JSON，格式为 `{"mcpServers": {"<服务器 ID>": {...}}}`，须 Base64 编码（再做 URL 转义） |
| `enabled` | 否 | 是否启用（布尔值） |
...
其中 `config` 是 `{"mcpServers":{"mcp-fetch":{"command":"uvx","args":["mcp-server-fetch"]}}}` 的 Base64 编码（末尾的 `=` 转义为 `%3D`）。
...
其中 `content` 是提示词 `# 角色\n你是一个专业的代码审查专家` 的 Base64 编码（`+` 转义为 `%2B`）。
