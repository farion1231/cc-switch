...
导入 Claude Desktop 供应商时，使用 `app=claude-desktop`。同时兼容 `claude_desktop` 和 `claudedesktop`，并统一为 `claude-desktop`。支持 URL 参数和 Base64 编码的 Claude `env` 内嵌配置，URL 参数优先。Prompt 和 MCP 导入不支持 Claude Desktop。
