...
| `app` | 是 | 应用类型：`claude` / `claude-desktop` / `codex` / `gemini` / `grokbuild` / `opencode` / `openclaw` / `hermes` |
| `name` | 是 | 名称 |
...
| `homepage` | 否 | 供应商官网 |
| `model` | 否 | 默认模型 |
| `haikuModel` | 否 | Haiku 模型（仅 Claude / Claude Desktop） |
| `sonnetModel` | 否 | Sonnet 模型（仅 Claude / Claude Desktop） |
| `opusModel` | 否 | Opus 模型（仅 Claude / Claude Desktop） |
| `claudeDesktopMode` | 否 | Claude Desktop 写入模式：`direct`（默认，直连）或 `proxy`（本地路由 + 模型映射）。仅 `app=claude-desktop` 时生效；`proxy` 模式下上面三个模型参数会被解读为上游模型映射 |
| `notes` | 否 | 备注 |
| `icon` | 否 | 图标 |
...
默认写入直连模式，要求上游暴露 Claude 原生 Messages API，且模型名符合 `claude-*` 角色名：
...
如果上游是非 Claude 模型（grok、deepseek 等），加上 `claudeDesktopMode=proxy`，并用 `sonnetModel` / `opusModel` / `haikuModel` 指定各角色实际请求的上游模型：
...
`proxy` 模式需要 CC Switch 的本地路由保持运行，且切换供应商后需要重启 Claude Desktop 才生效。
