...
适用于余额通过「网页账户信息接口」返回的中转站/自建站，典型路径如 `/api/v1/auth/me`。此类站点通常只接受**网页登录 Token**（Access Token），不接受模型调用用的 API Key：
...
> **提示**：网页登录 Token 与模型 API Key 是两套凭据。若模型调用用的 `sk-...` Key 无法查询余额（如返回 401/403），请从站点网页登录态的开发者工具（Network → 账户接口请求的 `Authorization` 头）中获取 Access Token，填入下方配置。

**配置参数**：
| 参数 | 说明 |
|------|------|
| Base URL | 站点根地址（不含 `/api/v1` 等前缀） |
| Access Token | 网页登录 Token（非模型 API Key） |

如果站点接口路径不是 `/api/v1/auth/me`，可直接在脚本中把 URL 改为实际路径；如接口带时区参数（如 `?timezone=Asia/Shanghai`）也一并加上。
