...
面板分为两个 Tab：
- **应用专属供应商**：仅用于当前选中的应用（Claude Code / Claude Desktop / Codex / Gemini / OpenCode / OpenClaw / Hermes / Pi Agent）
- **统一供应商**：跨应用共享的配置
...
Pi Agent 的 Provider 配置会写入 `~/.pi/agent/models.json`。

在 Pi Agent 面板中：

1. 在表单上方选择 Provider 模板。
2. 在下方填写 Provider 配置信息。
3. 添加至少一个模型 ID。模型名称默认等于模型 ID，也可以手动修改。
4. 预览 JSON diff。
5. 应用配置。cc-switch 会在写入前自动创建备份。

此功能不会修改 `~/.pi/agent/auth.json` 或 `~/.pi/agent/settings.json`。
