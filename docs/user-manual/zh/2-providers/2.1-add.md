...
| `requires_openai_auth` | 否 | Codex 是否把 auth.json 中的 API Key 发送给该供应商（见下方说明） |

> **关于 `requires_openai_auth`**：CC Switch 默认在新生成的第三方 Codex 配置中包含 `requires_openai_auth = true`（预设、自定义模板、统一供应商同步、深链导入）。可在 **设置 → Codex 应用增强 → 新 Codex 配置默认包含 requires_openai_auth** 中修改该默认值。开关仅影响新生成的配置——已保存的供应商保持原有 TOML，且该字段始终可在供应商配置编辑器中逐供应商增删（在编辑器中删除后会持久保留，统一供应商派生条目也一样）。警告：缺少该字段时，除非配置了 `env_key`，Codex 不会向自定义供应商发送 auth.json 的 API Key（本地代理接管不受影响）。统一会话历史与官方代理接管的注入始终保留该字段，因为它负责路由 auth.json 中的 ChatGPT 登录。
