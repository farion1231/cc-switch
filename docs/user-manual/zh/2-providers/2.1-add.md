...
聚合供应商是一个不包含上游地址或密钥的虚拟 Claude 供应商。它按照请求的模型档位，将 Haiku、Sonnet、Opus 和 Fable 分别转发给已有第三方供应商中的指定模型。例如，可以让 DeepSeek 承担 Haiku、Kimi 承担 Fable。

配置方法：

1. 先添加并验证需要作为目标的普通 Claude 供应商。
2. 添加新的 Claude 供应商并开启 **聚合供应商**。
3. 为需要的档位选择目标供应商并填写上游模型 ID；至少配置一个档位。
4. 开启[本地路由服务](../4-proxy/4.1-service.md)和 Claude 接管，然后切换到该聚合供应商。

注意事项：

- 聚合供应商必须通过本地路由使用，未接管时不能切换。
- 目标必须是同一应用中的普通第三方供应商；不能指向自身、其它聚合供应商或不允许经本地路由使用的官方供应商。
- 删除目标供应商前，需要先从所有聚合路由中移除其引用。
- 开启自动故障转移后，聚合路由目标会沿用现有熔断和队列回退机制。
- 未配置的档位不会自动借用其它档位；调用前应为实际使用的模型档位配置路由。
