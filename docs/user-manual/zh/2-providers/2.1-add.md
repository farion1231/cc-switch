...
部分第三方供应商只支持 **OpenAI Chat Completions** 协议，或使用非 GPT 模型名（如百度千帆 Coding Plan、阶跃 Step Plan、SiliconFlow）。Codex 原生只认 OpenAI Responses API 与 GPT 系列模型，因此这类供应商需要 CC Switch 在本地做协议与模型转换。
...
> 💡 选择 StepFun（Step Plan）、SiliconFlow 等 Chat 格式预设时，此开关默认已打开，无需手动设置。
