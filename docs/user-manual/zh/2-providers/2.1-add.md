...
| `goals` | 否 | 放在 `[features]` 下并设为 `true` 后启用 Codex `/goal` 模式；部分 API Key 或自定义端点配置缺少它时会提示“无法设置目标” |
| `base_url` | 是 | API 端点地址 |
| `wire_api` | 否 | API 协议类型（通常为 `responses`） |
| `requires_openai_auth` | 否 | 是否使用 OpenAI 认证方式 |

如果切换到 API Key 或自定义端点供应商后，Codex 能识别 `/goal`，但无法设置目标，请确认 `~/.codex/config.toml` 的 `[features]` 下包含 `goals = true`，然后重启 Codex。如果启用 Goal mode 后比预期更频繁地请求批准，请刷新 Codex 设置页，并确认 `approval_policy` 与 `sandbox_mode` 仍符合你的权限预期。
