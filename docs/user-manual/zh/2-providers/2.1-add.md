...
| Azure OpenAI | Foundry Models 中的 Azure OpenAI（v1 Responses API） |
| AiHubMix     | AiHubMix 聚合服务                                    |
| DMXAPI       | DMXAPI 中转服务                                      |
| PackyCode    | PackyCode 中转服务                                   |
| Cubence      | Cubence 服务                                         |
...
### Azure OpenAI（Foundry v1）推荐示例

```toml
model_provider = "azure"
model = "gpt-5.3-codex"  # 替换为你的实际部署名
...
- `base_url` 需要包含 `/openai/v1`。
- 在 v1 API 下，不再需要通过 `query_params` 传 `api-version`。
- 请在环境变量中设置 `OPENAI_API_KEY`，并使用 `env_key = "OPENAI_API_KEY"`。
- Azure 官方 Codex 文档：https://learn.microsoft.com/en-us/azure/foundry/openai/how-to/codex?tabs=npm
