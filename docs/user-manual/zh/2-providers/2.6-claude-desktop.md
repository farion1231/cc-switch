...
| Flatpak 说明 | Flatpak 版 CC Switch 固定使用宿主 `~/.config`，不跟随自定义 `XDG_CONFIG_HOME` |
...
## 迁移官方 (1P) 数据到第三方 (3P) 账号

### 为什么切换后历史会话“不见了”

Claude Desktop 的官方 (1P) 模式和第三方 (3P) 模式使用**两个完全独立的数据目录**：

- 官方 (1P)：`~/Library/Application Support/Claude`
- 第三方 (3P)：`~/Library/Application Support/Claude-3p`

切换供应商时，CC Switch 只改写登录方式和 profile 配置，**不会搬移数据**。所以从官方模式切到第三方模式后，之前在官方账号里的 Code 会话和 Cowork 任务并没有被删除——它们仍在 `Claude` 目录里，只是第三方模式读取的是 `Claude-3p` 目录，因此看起来“消失了”。

### 数据迁移功能

CC Switch 提供了一套**可选、安全、可回滚**的本地数据迁移，把官方 (1P) 账号里的数据复制到第三方 (3P) 账号。

在 Claude Desktop 面板顶部的「迁移官方 (1P) 数据到当前账号」卡片上，点击 **预览并迁移…** 打开迁移向导。

整个流程：

1. **只读扫描**：列出 1P / 3P 数据目录、Code 会话与 Cowork 任务数量。此阶段不做任何改动。
2. **迁移方案**：展示每个组件将复制多少条新记录、有多少条 3P 已存在（会被跳过）、预计复制与备份大小。若发现问题（例如存在多个候选数据目录、3P 目录尚未初始化）会先列出并要求你处理，否则无法继续。
3. **明确确认**：勾选确认后才开始。迁移前会**自动完整备份**目标目录，并要求你先完全退出 Claude Desktop。
4. **执行与校验**：逐条复制（已存在的绝不覆盖），完成后可一键**运行校验**。
5. **可撤销**：每次迁移都会记录账本，**撤销迁移**只会移除本次复制进去的内容，绝不会删除你在迁移之后新建的会话或任务。

安全保障：

- 源（官方 1P）目录**只读**，绝不修改
- 目标（3P）已有记录**绝不覆盖**，重名一律跳过
- 写入前自动备份；采用“先暂存、校验、再原子落盘”的方式
- 迁移时若 Claude Desktop 正在运行会拒绝执行
- 遇到多个候选数据目录时**绝不猜测**，会停下来让你确认
- 聊天正文、audit 日志、上传 / 输出文件**逐字节复制**，只改写必要的结构化路径字段
- 备份与账本**不会**保存聊天正文或任何凭据

### 会迁移什么

| 组件          | 说明                                                       |
| ------------- | ---------------------------------------------------------- |
| Code 会话     | 会话索引（转录文件本身是 1P / 3P 共享的，无需复制）        |
| Cowork 任务   | 元数据 + 同名会话目录（audit、uploads、outputs）           |
| Scheduled 任务 | 导入后**保持禁用**，需要时手动开启                         |
| Project 缓存  | 仅复制 3P 缺失的项目                                       |
| Artifact 注册表 | 按 ID 合并，已存在的跳过                                 |

### 不会迁移什么

- 云端 Chat 会话历史
- 账号偏好与凭据（Cookie / Token / Keychain / OAuth / API Key）
- Connectors / OAuth 授权
- Skill / Plugin 打包与运行时鉴权
- 各类缓存（Cache / Code Cache / GPUCache）、`vm_bundles` 等
