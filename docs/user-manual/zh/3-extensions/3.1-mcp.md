...
> ⚠️ **注意**：OpenClaw 和 Claude Desktop 暂不支持 EC Switch MCP 同步。MCP 功能支持 Claude、Codex、Gemini、OpenCode 和 Hermes。

### 开关实现机制

当开启某个应用的开关时，EC Switch 会：
...
如果你已经在 CLI 工具中配置了 MCP 服务器，可以导入到 EC Switch：

1. 点击「导入」按钮
2. 选择要导入的应用（Claude/Codex/Gemini/OpenCode/Hermes）
3. EC Switch 会读取现有配置并导入
