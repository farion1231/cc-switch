...
- Codex：补全 catalog 字段，`truncation_policy.limit` 跟随 `context_window` 联动，恢复隐藏的压缩配置 UI
...
单位规则：`K/k` = 1000，`M/m` = 1000000，纯数字直接用。

### 2.2 写入约定

- **统一小写写入**（参考 issue #3679）：`[1m]`、`[200k]`，不用 `[1M]`、`[200K]`
- 解析侧大小写不敏感（已有 `.toLowerCase()` / `eq_ignore_ascii_case`）
...
4. 无后缀或解析失败 → `{ slug: 原值, window: undefined }`
...
- placeholder：`1M`（灰色提示）
- 值为空 = 不声明窗口
- 输入校验：只允许数字 + K/k/M/m，非法时标红

兜底模型行同样改造。

### 3.3 数据存储

存储方式不变——后缀拼到模型名上写入 env：
...
- 现有：只剥离 `[1m]` / `[1M]`
- 改后：剥离任意 `[数字+单位]` 后缀（`[200k]`、`[500k]`、`[1m]` 等）

参考 issue #5177：后缀泄漏到上游会导致 400 错误。

`services/proxy.rs` 的 `has_claude_one_m_marker` / `strip_claude_one_m_marker` / `CLAUDE_ONE_M_MARKER_FOR_CLIENT` 同步泛化。代理在 live takeover 模式下给客户端模型名追加后缀时，也要用新格式。

## 5. Codex catalog 修复（codex_config.rs）

### 5.1 补全 catalog 字段

`codex_catalog_model_entry` 函数（当前 `:452`）补两个字段：
...
- `effective_context_window_percent = 100`：显示真实窗口（默认 95 会让 1M 显示为 950K）
- `auto_compact_token_limit = null`：让 Codex 按内置比例自动计算压缩点

### 5.2 truncation_policy.limit 联动

`codex_native_responses_template.json` 中硬编码的 `truncation_policy.limit = 10000`（issue #4832）改为动态生成：

- 在 catalog 生成时，`truncation_policy.limit` 取 `context_window` 的值
- 如果 `context_window` 为 0 或缺失，回退到现有 10000

### 5.3 恢复隐藏 UI

`CodexConfigSections.tsx:1` 的注释取消，恢复 `model_context_window` / `model_auto_compact_token_limit` 两个输入框：

- `model_context_window`：上下文大小（如 1000000）
- `model_auto_compact_token_limit`：压缩阈值（如 900000）
- 留空 = 不改写（保持现有行为）

### 5.4 覆盖保护

已有的 `preserves_user_model_catalog_json` 逻辑保持不动（issue #5110 问题4）。用户手动改的 catalog 不被覆盖。
...
- `parse_context_window_suffix` 对称测试
...
- Claude Code：配 sonnet=1M + opus=200K → 检查 `settings.json` 的 env
- Claude Code：`/context` 显示正确窗口
- Codex：配 deepseek-v4-pro contextWindow=1000000 → 检查 catalog JSON
- Codex：长对话触发自动压缩
...
| Codex truncation 改动影响现有行为 | 留空 = 不改写，保持现有 10000 回退 |
| 代理剥离遗漏新格式导致上游 400 | 泛化 `strip_context_window_suffix_for_upstream`，覆盖所有 `[数字+单位]` |
| 用户手动改 catalog 被覆盖 | 已有 `preserves_user_model_catalog_json` 保护，不动 |
...
### Rust 后端
- `src-tauri/src/claude_desktop_config.rs` — 新增 `parse_context_window_suffix`
...
- `src-tauri/src/codex_config.rs` — 补 catalog 字段 + truncation 联动
- `src-tauri/src/resources/codex_native_responses_template.json` — truncation 动态化（或在 Rust 中覆盖）
