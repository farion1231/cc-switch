...
CC Doctor 的代理功能是一个本地 HTTP 代理服务器，可以统一管理 Claude Code、Codex 和 Gemini CLI 的 API 请求。主要特性包括：
...
在 CC Doctor 主界面，点击右上角的 **Proxy** 按钮，可以看到代理控制面板。
...
> **注意**：启用接管后，CC Doctor 会自动修改对应应用的配置文件，将 API 端点指向本地代理。原始配置会被安全备份。

### 3. 正常使用 CLI

启用接管后，你可以正常使用各个 CLI 工具。所有请求都会经过 CC Doctor 代理转发到配置的供应商。

### 4. 停止代理

当你不再需要代理时，点击 **停止代理** 按钮。CC Doctor 会：
...
CC Doctor 通过检测配置备份来判断接管状态：

- 存在备份 = 已接管
- 无备份 = 未接管

这确保了即使 CC Doctor 异常退出，重新启动后也能正确识别状态。
...
A: 点击 **停止代理** 按钮，CC Doctor 会自动恢复所有应用的原始配置。

如果 CC Doctor 异常退出，重新启动后会检测到之前的备份，你可以：
- 点击停止代理来恢复配置
- 或继续使用代理功能
...
原始配置备份在 CC Doctor 数据库中，停止代理时自动恢复。
