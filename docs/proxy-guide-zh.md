...
## connectionOverride（连接覆写地址）使用说明

`connectionOverride` 用于把实际网络连接改为指定的 `IP:端口`（或 `[IPv6]:端口`），同时仍保留原始 API URL 的域名语义（TLS SNI / HTTP Host 不变）。

### 生效前提（必须满足）

1. 已启动 CC Switch 内置代理（Proxy）
2. 已为目标应用启用“接管”

> 仅保存 `connectionOverride` 不会立即生效；未开启代理接管时，请求不会走覆写链路。

### 配置步骤

1. 在供应商编辑页的高级设置中填写 `connectionOverride`（示例：`1.2.3.4:443` 或 `[2001:db8::1]:443`）
2. 保存供应商配置
3. 打开 **Proxy** 面板并启动代理
4. 对目标应用（Claude/Codex/Gemini）开启接管
5. 正常使用 CLI，请求将通过内置代理按覆写目标转发

### 常见排查

- 配置了 `connectionOverride` 但未生效：优先检查代理是否已启动、目标应用是否已开启接管
- 证书或握手错误：确认 API URL 的原始域名与上游证书匹配，且覆写地址端口可达
