...
This release centers on the terminal workbench and provider-switching reliability. The embedded terminal learns alternate-scroll — wheel events are bridged to arrow keys while a TUI such as opencode owns the alternate buffer, so the wheel finally scrolls — and the bottom prompt now follows the selected terminal tab instead of writing into the previous session's PTY. The three-column layout's right-hand model list becomes collapsible (a narrow strip with a vertical label, persisted like the left panel) and loses its redundant title, while the terminal header stats chip finally names the model it is measuring. Switching providers gets two reliability fixes: the card's primary click switches again (the fork's usage-pin rewrite had left only the context menu), and context-menu clicks no longer bubble into an accidental switch that made the clicked provider current — which then blocked its deletion as "current provider cannot be deleted". A stale `live_backup` row surviving a crash no longer pins every switch to a dead proxy's hot-switch path. The rate statistics write the model name they previously never stored and expire stale live bursts left by streams that ended without a usage record. New this release is a factory reset: the settings page gains a type-RESET-gated section that stops the proxy, restores every taken-over live config, snapshots the database into backups/, wipes user data, resets app settings and restarts. This release does not change the database schema.

**Stats**: 5 commits | 16 files changed | +524 insertions | -45 deletions

### Added

- **Factory Reset (Restore All Configuration to Defaults)**: a new section in Settings → Advanced, behind a type-RESET confirmation dialog, runs the new `factory_reset` command: stop the local proxy and restore every taken-over live config (Claude/Codex/Gemini/GrokBuild clients keep working), snapshot the database into the backups directory (recoverable later from Backup & Restore), wipe user data — providers, profiles, prompts, MCP servers, custom endpoints, proxy takeover state and live backups, proxy request logs, session sync/dedup records, stream checks, usage rollups and the DB settings key-values — inside a single transaction, re-seed the per-app proxy_config default rows, reset settings.json to its defaults, and restart the app automatically. Model pricing seeds and skill content tables are preserved.
- **Collapsible Model List Column**: the terminal workbench's right column (model list) collapses into a 32px strip with a vertical label, mirroring the left panel's interaction, with the state persisted to localStorage; the redundant 模型列表 header title is removed and the row keeps the usage display clear of the hover-revealed collapse button.
- **Alternate-Scroll Wheel Bridging in the Embedded Terminal**: while a full-screen TUI (opencode and friends) owns the xterm alternate buffer there is no scrollback to scroll, and with mouse tracking off the wheel did nothing; wheel deltas are now translated into arrow-key sequences (SS3 under DECCKM) written to the PTY, matching Windows Terminal's default behavior. Normal-buffer scrolling and mouse-aware TUIs such as htop are untouched.
- **Model Name in the Terminal Stats Chip**: the header chip shows the resolved model of the last request (backend statistics previously read a model field nothing ever wrote), with long names truncated, date suffixes stripped, and speeds formatted to avoid width jitter above 100 tok/s.
- **Terminal Workbench Section in the READMEs**: the three-column workbench (projects, terminal instances, session resume, git changes panel, live stats, HUD floating window) is now documented.

### Fixed

- **Clicking a Provider Card Switches Again**: the fork's card rewrite made the primary click pin the usage bar for three seconds, so the only way to switch was the context menu — read under proxy takeover as "switching has no effect". The primary click now mirrors the context menu's primary action: switch, or toggle failover-queue membership in failover mode; OMO and additive cards keep the usage pin so a stray click cannot disable or remove them.
- **Context-Menu Clicks No Longer Switch the Provider**: the context menu is a DOM child of the card, so clicking 删除 also fired the card's switch handler — the clicked provider became current, which then blocked its own deletion with "current provider cannot be deleted". Menu clicks are now contained.
- **The Bottom Prompt Follows the Selected Terminal**: switching terminal tabs left the prompt holding the previous session's PTY id, silently writing input into the other session; the reference is cleared on switch and reports unavailable until the new session connects.
- **Stale Takeover Backup No Longer Pins Switching to a Dead Proxy**: a `live_backup` row is only deleted by a graceful proxy stop, so a crash mid-takeover left one behind and forced every subsequent switch into hot-switch mode that never wrote real endpoints into the live file; when a backup exists without a live takeover placeholder and the proxy is not running, the row is cleared and the switch performs a normal write.
- **Rate Statistics: Model Name and Stale Live Bursts**: `record_output` now stores the outbound model so the tooltip stops showing 未知模型, and a stream that ends without a usage record (unparseable usage) no longer leaves its live-burst counters dirty, which had dragged the next generation's displayed tok/s down and stuck it there.
