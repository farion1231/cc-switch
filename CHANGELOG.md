...
- **Large Skills Collections Now Round-Trip Through Cloud Sync**: WebDAV and S3 snapshots now support up to 100,000 Skills archive file/directory entries while retaining the 512 MB compressed-download and extracted-content safety limits. Upload performs the same entry and uncompressed-size preflight as restore, so a snapshot accepted by the current version can be restored by that version instead of uploading successfully and failing on another device. New manifests include optional entry-count and estimated-extracted-size metadata shown in the restore confirmation; existing manifests without these fields remain directly restorable and are preflighted from the ZIP central directory after download.
