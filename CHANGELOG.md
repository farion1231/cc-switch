...
- **Session Manager Now Actually Lists Hermes Sessions**: Hermes' SessionManager provider was already registered on the Rust side (`session_manager::providers::hermes`) but two real bugs kept every Hermes conversation invisible in the UI. `load_messages_sqlite` queried a non-existent `created_at` column — Hermes' `messages` table uses `timestamp` (REAL epoch seconds) — so opening any Hermes session detail raised a SQL `no such column` error and the transcript rendered empty. `scan_sessions_sqlite` then had no title fallback, so even sessions that *did* appear in the list showed only an 8-character id. The list-view provider dropdown also had no Hermes entry, so users could not filter to Hermes alone. Fixes: query the real `timestamp`/`tool_calls`/`tool_name`/`active` columns, skip `active = 0` rows, render assistant tool invocations as `[Tool: <name>]` and tool results as `[Result: <name>]`, use the first user message as the session title (in a single grouped query so 750 sessions scan without N+1), add Hermes to the provider filter dropdown and to the `getProviderIconName` registry. Covered by four new Rust unit tests against an in-memory SQLite using Hermes' real schema, two new frontend tests pinning the icon map and i18n fallback, and an end-to-end smoke test that loads 1038 messages from a real Hermes session and verifies 465 `[Tool: …]` / 464 `[Result: …]` lines render correctly.
