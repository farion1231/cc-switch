...
- **Optional `requires_openai_auth` Default for Generated Codex Configs**: A new "Include requires_openai_auth in new Codex configs" toggle (Settings → Codex App Enhancements, `codexDefaultRequiresOpenaiAuth`, default on) controls whether newly generated third-party Codex configs include `requires_openai_auth = true` — covering the frontend presets/custom template, universal-provider sync regeneration, the universal-modal preview, and deeplink imports. The default stays on because without the field Codex does not forward the auth.json `OPENAI_API_KEY` to a custom provider unless `env_key` is set (the toggle description carries this warning; local proxy takeover is unaffected since it authenticates via the bearer-token placeholder). The unified-session-history injection and the official proxy takeover route keep writing the field unconditionally — it is load-bearing there (routes the ChatGPT login in auth.json) and the strip round-trip depends on the injected table's exact shape. Localized in all four locales (zh/en/ja/zh-TW).

### Fixed

- **Universal-Provider Sync No Longer Clobbers Manual Codex TOML Edits**: `sync_universal_to_apps` used to regenerate the derived `universal-codex-*` provider's whole `config.toml` string and overlay it via JSON merge, silently resurrecting `requires_openai_auth = true` (and discarding any other manual TOML edit) on every save-and-sync. The Codex branch now patches the existing config field-surgically with `update_codex_toml_field` (base_url with the same `/v1` origin-only normalization as generation, top-level `model`, and a newly supported top-level `model_reasoning_effort`), so removals and custom keys survive re-syncs while universal-form edits still propagate; the full regeneration path remains as a fallback when the stored config is missing or fails TOML parse, preserving the old self-healing behavior for corrupted configs. The universal edit modal's Codex preview now also bases itself on the derived provider's actual config (same field patches) instead of a hardcoded template.
