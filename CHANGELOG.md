...
- **Codex Streaming Idle Timeout Raised to 300s for Long-Reasoning Models**: Long-thinking providers such as MiMo / DeepSeek-reasoner / Kimi often exceed the old 120s default between reasoning bursts, which used to trip the idle timeout and cut the stream mid-response. The default `streaming_idle_timeout` is now 300s (5 min) for Codex, with a one-shot `v10 → v11` migration that promotes existing rows only when they still hold the old default (so any user override is preserved). `ProxyConfig::default()` and `AppProxyConfig` fallbacks in the DAO also use 300s to match. Frontend and docs updated to reflect the new default.
- **Codex MiMo / MiMo-Pro Multi-Round Hang** (continued): Following the `content: null` and consecutive-assistant-message fix in `transform_codex_chat.rs` and `streaming_codex_chat.rs`, the streaming converter now also short-circuits the inline think-block state machine when reasoning was already captured via `reasoning_content`, so MiMo content deltas with embedded `<think>` tags no longer get buffered until the closing tag arrives. The `mimo_reasoning_then_tool_call_stream` and `reasoning_content_then_think_block_skips_detection` regression tests guard the new path.
