        ...
        if (result.status === "operational") {
          ...
          if (isApiError) {
            if (
              ...
              result.message.toLowerCase().includes("key")
            ) {
              hint = t("streamCheck.authErrorHint", {
                defaultValue: "认证失败，请检查您的 API 密钥 (API Key) 配置。",
              });
            } else if (
              ...
              hint = t("streamCheck.quotaErrorHint", {
                defaultValue: "频控受限或额度用尽，请检查账户可用额度。",
              });
            } else if (
              result.message.includes("404") ||
              result.message.toLowerCase().includes("model")
            ) {
              hint = t("streamCheck.modelErrorHint", {
                defaultValue:
                  "请求模型不存在，或当前渠道未开通该模型访问权限。",
              });
            } else {
              hint = t("streamCheck.apiErrorHint", {
                defaultValue:
                  "大模型接口调用失败，请根据具体返回的 HTTP 错误进行排查。",
              });
            }
          }
