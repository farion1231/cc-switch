...
> = {
  ...
  ohmypi: { key: "ohmypi", defaultFolder: ".omp/agent" },
};
    ...
    const load = async () => {
      try {
        const [
          ...
          settingsApi.getConfigDir("ohmypi"),
          ...
          computeDefaultConfigDir("ohmypi"),
        ]);
        ...
        setResolvedDirs({
          ...
          ohmypi: ohmypiDir || defaultsRef.current.ohmypi,
        });
      } catch (error) {
        console.error(
  ...
  const resetAllDirectories = useCallback(
    (overrides?: ResolvedAppDirectoryOverrides) => {
      ...
      setResolvedDirs({
        ...
        ohmypi: overrides?.ohmypi ?? defaultsRef.current.ohmypi,
      });
    },
    [],
  );
