...
> = {
  ...
  atomcode: { key: "atomcode", defaultFolder: ".atomcode" },
};
    ...
    const load = async () => {
      try {
        const [
          ...
          settingsApi.getConfigDir("atomcode"),
          ...
          computeDefaultConfigDir("atomcode"),
        ]);
        ...
        setResolvedDirs({
          ...
          atomcode: atomcodeDir || defaultsRef.current.atomcode,
        });
      } catch (error) {
        console.error(
  ...
  const resetAllDirectories = useCallback(
    (overrides?: ResolvedAppDirectoryOverrides) => {
      ...
      setResolvedDirs({
        ...
        atomcode: overrides?.atomcode ?? defaultsRef.current.atomcode,
      });
    },
    [],
  );
