  ...
  const performSubmit = async (
    ...
    if (appId === "codex") {
      try {
        ...
        // Omit the disabled/default state so ordinary and managed official
        // providers keep the upstream auth/config-only shape. The hook derives
        // aggregation=false and official-login=true when these fields are absent.
        if (category === "official" && effectiveAggregation) {
          configObj.enableOfficialLogin = effectiveOfficialLogin;
          configObj.codexAggregationEnabled = true;
        }
