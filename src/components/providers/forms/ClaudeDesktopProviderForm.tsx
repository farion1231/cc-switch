                  ...
                  <p className="text-xs leading-relaxed text-muted-foreground">
                    {needsModelMapping
                      ? t("claudeDesktop.modelMappingOnHint", {
                          defaultValue:
                            "Claude Desktop 只接受 claude-sonnet-* / claude-opus-* / claude-haiku-* 三档角色 ID。开启后 EC Switch 会把这三档映射到供应商的实际模型，并在使用期间保持本地路由开启。",
                        })
                  ...
                  <div className="space-y-1 border-t border-border-default pt-4">
                    ...
                    <p className="text-xs leading-relaxed text-muted-foreground">
                      {t("claudeDesktop.routeMapHint", {
                        defaultValue:
                          "选择模型角色后，EC Switch 会自动生成 Claude Desktop 兼容路由；菜单显示名可以写 DeepSeek、Kimi 等品牌名，实际请求模型按右侧填写内容发送。留空的档会自动沿用 Sonnet（或第一个已填档）的模型，确保子 agent 调用的 Haiku 始终可用。",
                      })}
                    </p>
                  </div>
