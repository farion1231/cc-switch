...
export function CodexSessionsDialog({
  ...
  const codexProviderIds = useMemo(
    () => codexProviders.map((candidate) => candidate.id),
    [codexProviders],
  );
  ...
  const getSyncVisibilityTarget = (linkedProviderIds: string[]) => {
    const providerIds = linkedProviderIds.length
      ? linkedProviderIds
      : codexProviderIds;
    const providerIdSet = new Set(providerIds);
    const syncToCodex =
      codexProviderIds.length > 0 &&
      providerIdSet.size === codexProviderIds.length &&
      codexProviderIds.every((id) => providerIdSet.has(id));

    return {
      providerIds,
      linkMode: syncToCodex ? "all" : "manual",
      syncToCodex,
    } as const;
  };
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex shrink-0 items-center gap-2">
                        <Button
                          ...
                          onClick={() => {
                            if (!sourcePath) return;
                            const target = getSyncVisibilityTarget(
                              item.linkedProviderIds,
                            );
