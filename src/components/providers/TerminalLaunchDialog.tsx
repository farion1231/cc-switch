...
export const BYPASS_ARG_FOR_APP: Record<AppId, string> = {
...
export function TerminalLaunchDialog({
  ...
  const bypassArg = BYPASS_ARG_FOR_APP[app];
    ...
    >
      <DialogContent className="max-w-md" zIndex="alert">
        <DialogHeader className="space-y-3">
          ...
          <DialogDescription className="space-y-3 pt-2">
            <p className="text-sm leading-relaxed text-foreground/90">
              {t("provider.terminalLaunchMessage", {
                bypassArg,
                defaultValue: `您准备在终端中启动 ${app}。是否启用越权模式 (${bypassArg})？`,
              })}
            </p>
            {isClaude && (
        ...
        <DialogFooter className="flex flex-col gap-2 pt-2 sm:flex-row sm:justify-end">
          ...
          >
            {t("provider.terminalLaunchBypass", "越权启动")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
