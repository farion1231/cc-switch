...
function buildPasswordPreservationKey(values: {
  ...
  return JSON.stringify({
    ...
    remoteRoot: values.remoteRoot ?? "ec-switch-sync",
...
export function WebdavSyncSection({
  ...
  const [form, setForm] = useState(() => ({
    ...
    remoteRoot: config?.remoteRoot ?? "ec-switch-sync",
  ...
  const [s3RemoteRoot, setS3RemoteRoot] = useState(
    s3Config?.remoteRoot ?? "ec-switch-sync",
  );
  ...
  useEffect(() => {
    ...
    setForm(() => {
      ...
      const nextRemoteRoot = config.remoteRoot ?? "ec-switch-sync";
  ...
  useEffect(() => {
    ...
    setS3RemoteRoot(s3Config.remoteRoot ?? "ec-switch-sync");
  ...
  const buildSettings = useCallback((): WebDavSyncSettings | null => {
    ...
    return {
      ...
      remoteRoot: form.remoteRoot.trim() || "ec-switch-sync",
  ...
  const buildS3Settings = useCallback((): S3SyncSettings => {
    return {
      ...
      remoteRoot: s3RemoteRoot.trim() || "ec-switch-sync",
  ...
  const currentRemotePath = `/${form.remoteRoot.trim() || "ec-switch-sync"}/v2/db-v6/${form.profile.trim() || "default"}`;
  const currentS3RemotePath = `${s3Bucket.trim() || "bucket"}/${s3RemoteRoot.trim() || "ec-switch-sync"}/v2/db-v6/${s3Profile.trim() || "default"}`;
