...
// auth 记录里除 auth_mode 外是否还有任何凭据内容（API key、OAuth 字段等）
const authHasCredential = (auth: Record<string, unknown> | null): boolean =>
  !!auth &&
 ...
 *
 * When the live snapshot carries no bearer and no auth material at all
 * (auth.json missing or empty — e.g. a custom provider authenticated via
 * config.toml headers), the live auth resolves to an empty object; letting it
 * win would wipe the stored auth on a plain open-and-save round trip. In that
 * case the stored auth is kept as-is.
 */
export const reconcileCodexLiveAuth = (
  ...
  if (!bearer) {
    if (
      !authHasCredential(asRecord(liveSettings.auth)) &&
      storedAuth &&
      authHasCredential(storedAuth)
    ) {
