...
/**
 * 格式化仓库拉取失败摘要，供“发现/检查更新”这类部分失败提示使用。
 */
export function formatSkillRepoFailure(
  failure: { owner: string; name: string; error: string },
  t: TFunction,
): string {
  return `${failure.owner}/${failure.name}: ${t(
    getSkillRepoFailureReasonKey(failure.error),
  )}`;
}
