...
export const useProvidersQuery = (
  ...
  const { isProxyRunning = false, isRemoteBackend = false } = options || {};
  ...
  return useQuery({
    ...
    refetchInterval: isProxyRunning || isRemoteBackend ? 10000 : false,
    queryFn: async () => {
...
export const useRuntimeQuery = (): UseQueryResult<RuntimeInfo> => {
  return useQuery({
    queryKey: ["runtime"],
    queryFn: async () => runtimeApi.getCached(),
    staleTime: Infinity,
    gcTime: Infinity,
  });
};
