...
const applyBaseUrl = (
  ...
  if (appId === "codex") {
    targetConfig.config = setCodexBaseUrl(stringValue(targetConfig.config) ?? "", baseUrl);
    return;
  }
  if (appId === "gemini") {
...
const applyModelMapping = (
  ...
  const sourceConfig = isRecord(source.settingsConfig)
  ...
  if (appId === "codex" && typeof mapping === "string") {
    ...
    const baseUrl = extractCodexBaseUrl(stringValue(sourceConfig.config));
    if (baseUrl) {
