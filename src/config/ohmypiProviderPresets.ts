 ...
 *
 * The provider presets written into Oh My Pi's `~/.omp/agent/models.yml`.
 * Oh My Pi speaks the API formats below with baseUrl + apiKey + headers +
 * compat + models config shape and OpenAI-compatible provider endpoints, so
 * presets carry real, app-agnostic endpoints.
 *
 * `models.yml` is validated by Oh My Pi's `ModelsConfigSchema`, and only
 * fields the schema defines are allowed on a model. In particular the model
 * field `thinkingLevelMap` and the `compat` keys `forceAdaptiveThinking`,
 * `requiresReasoningContentOnAssistantMessages` and `deferredToolsMode` are
 * not part of that schema, so they are deliberately absent from this catalog
 * — writing them into OMP's config would be a bug.
 *
...
export interface OhMyPiProviderPreset {
  name: string;
  nameKey?: string;
  providerKey: string;
  websiteUrl: string;
