...
export interface Provider {
  ...
  // 新增：是否为商业合作伙伴
  isPartner?: boolean;
  // 可选：供应商元数据（仅存于 ~/.cc-doctor/config.json，不写入 live 配置）
  meta?: ProviderMeta;
  // 图标配置
...
// Skill 存储位置
export type SkillStorageLocation = "cc_doctor" | "unified";
...
// 存储在本地 ~/.cc-doctor/settings.json，不随数据库同步
export interface Settings {
  ...
  // Skill 存储位置：cc_doctor（默认）或 unified（~/.agents/skills/）
