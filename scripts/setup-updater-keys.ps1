...
Write-Host "     TAURI_SIGNING_PRIVATE_KEY_PASSWORD = (leave empty if you used empty -p)"
    Write-Host "3) On a FORK release branch only, point endpoints at this fork's latest.json"
    Write-Host "   NEVER put fork updater endpoint/pubkey into a PR against farion1231/cc-switch"
