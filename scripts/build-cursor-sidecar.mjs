...
const buildWorkspace = mkdtempSync(join(tmpdir(), "cc-switch-cursor-sidecar-"));
const patchedGoproxyDir = join(buildWorkspace, "goproxy-no-default-ca");
const temporaryGoMod = join(buildWorkspace, "cursor-sidecar-build.mod");

function cleanupBuildWorkspace() {
  makeTreeWritable(buildWorkspace);
  rmSync(buildWorkspace, { recursive: true, force: true });
}

process.once("exit", cleanupBuildWorkspace);
