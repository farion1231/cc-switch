...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ca-certificates curl git xz-utils build-essential pkg-config file patchelf \
    libglib2.0-dev libssl-dev libgtk-3-dev libayatana-appindicator3-dev librsvg2-dev \
    libwebkit2gtk-4.1-dev libjavascriptcoregtk-4.1-dev libsoup-3.0-dev libxdo-dev \
    flatpak flatpak-builder bubblewrap xdg-dbus-proxy dbus elfutils \
