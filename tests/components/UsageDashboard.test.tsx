...
const providerResult = (appType: string, status: string, overrides = {}) => ({
  appType,
  status,
  syncResult: syncResult(overrides),
});
...
describe("UsageDashboard", () => {
  ...
  it("keeps the existing controls while exposing the task view", async () => {
    ...
    await userEvent.click(screen.getByRole("tab", { name: "Task Statistics" }));
  ...
  it("calls the generic endpoint with the nested request and invalidates all usage queries", async () => {
    rebuildAgentSessionUsageMock.mockResolvedValue({
      providers: [
        providerResult("codex", "published", { imported: 1, filesScanned: 1 }),
      ],
    });
  ...
  it("reports mixed published and kept-previous results with a warning", async () => {
    rebuildAgentSessionUsageMock.mockResolvedValue({
      providers: [
        providerResult("claude", "keptPrevious", {
          filesScanned: 1,
          deferredFiles: 1,
          errors: ["source unavailable"],
        }),
        providerResult("codex", "published", { imported: 2, filesScanned: 2 }),
      ],
    });
    renderDashboard();
    openRebuildDialog();
  ...
  it("disables rebuild controls while the provider command is in flight", async () => {
    ...
    await waitFor(() =>
      expect(rebuildAgentSessionUsageMock).toHaveBeenCalled(),
    );
    expect(
      screen.getByRole("button", {
        name: "usage.rebuildAgentSessionUsage.action",
