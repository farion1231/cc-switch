...
describe("WebdavSyncSection", () => {
  ...
  it("clears saved WebDAV config after confirmation", async () => {
    renderSection(baseConfig);

    fireEvent.click(
      screen.getByRole("button", {
        name: "settings.webdavSync.clearConfig",
      }),
    );
    expect(
      screen.getByText("settings.webdavSync.clearConfirmTitle"),
    ).toBeInTheDocument();
    fireEvent.click(
      screen.getByRole("button", {
        name: "settings.webdavSync.clearContinue",
      }),
    );
    expect(
      screen.getByText("settings.webdavSync.clearFinalConfirmTitle"),
    ).toBeInTheDocument();

    fireEvent.click(
      screen.getByRole("button", {
        name: "settings.webdavSync.clearFinalConfirm",
      }),
    );
    ...
    expect(settingsApiMock.webdavSyncSaveSettings).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        remoteRoot: "cc-switch-sync",
        profile: "default",
        syncScope: expect.objectContaining({
          mcp: false,
          skills: false,
        }),
      }),
    ...
    expect(
      (
        screen.getByPlaceholderText(
          "settings.webdavSync.baseUrlPlaceholder",
        ) as HTMLInputElement
      ).value,
    ).toBe("");
    expect(toastSuccessMock).toHaveBeenCalledWith(
      "settings.webdavSync.clearSuccess",
    );
  });
