    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化全局出站代理 HTTP 客户端
            {
                ...
                let follow_system_proxy = db.get_global_proxy_follow_system().unwrap_or(true);

                if let Err(e) =
                    crate::proxy::http_client::init(proxy_url.as_deref(), follow_system_proxy)
                {
                    log::error!(
                    ...
                    if let Err(fallback_err) =
                        crate::proxy::http_client::init(None, follow_system_proxy)
                    {
                        log::error!(
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::get_global_proxy_settings,
            commands::get_global_proxy_follow_system,
            commands::set_global_proxy_url,
            commands::set_global_proxy_settings,
            commands::set_global_proxy_follow_system,
