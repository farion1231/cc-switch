...
/// On macOS, calling `window.hide()` on a fullscreen window causes a black screen
/// because the fullscreen presentation layer is not cleaned up. This function
/// instead exits fullscreen first, then hides after the animation finishes.
#[cfg(target_os = "macos")]
