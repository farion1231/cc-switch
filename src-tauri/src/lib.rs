...
fn decide_skill_repo_startup(
    ...
    match pending {
        ...
        SkillRepoInitializationPending::JsonMigration => SkillRepoStartupAction::MigrateJson,
...
pub fn run() {
    ...
    let builder = builder
            ...
            match skill_repo_action {
                ...
                SkillRepoStartupAction::MigrateJson => {
                    if let Some(config) = migration_config {
                        ...
                        log::error!(
                            "Skill repository JSON migration is pending, but config.json is missing"
                        );
                    }
                }
                SkillRepoStartupAction::MarkExisting => {
                    if let Err(error) = db.init_default_skill_repos(false) {
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::get_cached_discoverable_skills,
...
mod tests {
    ...
    #[test]
    fn existing_installation_without_pending_work_preserves_its_repository_list() {
        assert_eq!(
            decide_skill_repo_startup(true, false, SkillRepoInitializationPending::None, false,),
            SkillRepoStartupAction::MarkExisting
        );
    }
