...
fn handle_deeplink_url(
    ...
    if !url_str.starts_with("ecswitch://") {
        return false;
    }
...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.ec-switch/crash.log）
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（单文件输出到 <app_config_dir>/logs/ec-switch.log）
            {
                ...
                let log_file_path = log_dir.join("ec-switch.log");
            ...
            let db_path = app_config_dir.join("ec-switch.db");
            ...
            //
            // 捕获首次运行快照：所有全新装用户都会看到欢迎弹窗介绍 EC Switch 的工作方式。
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    // tauri-plugin-deep-link writes to: ~/.local/share/com.ecswitch.desktop/applications/ec-switch-handler.desktop
                    // Only register if .desktop file doesn't exist to avoid overwriting user customizations
                    let should_register = app
                        .path()
                        .data_dir()
                        .map(|d| !d.join("applications/ec-switch-handler.desktop").exists())
                        .unwrap_or(true);
                ...
                move |event| {
                    ...
                    for (i, url) in urls.iter().enumerate() {
                        ...
                        if handle_deeplink_url(&app_handle, url_str, true, "on_open_url") {
                            break; // Process only first ecswitch:// URL
                        }
                    }
                }
            });
    ...
    app.run(|app_handle, event| {
        ...
        #[cfg(target_os = "macos")]
        {
            match event {
                ...
                // 处理通过自定义 URL 协议触发的打开事件（例如 ecswitch://...）
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Consider rolling back to an older EC Switch version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Suggestions:\n\
            1) Back up the entire config directory (including ec-switch.db)\n\
            2) If you see “database version is newer”, please upgrade EC Switch\n\
