...
impl Database {
    ...
    ///
    /// 启动流程会在失败后重试 JSON 迁移，因此迁移数据与完成标记必须原子提交，
    /// 防止标记写入失败时下次启动重放 `INSERT OR REPLACE`。
        ...
        self.migrate_from_json_transaction(config, |tx| {
            ...
            .map_err(|error| {
                AppError::Database(format!(
                    "Mark default Skill repositories initialized failed: {error}"
                ))
            })?;
            Ok(())
        })
...
mod tests {
    ...
    fn failed_migration_finalizer_rolls_back_imported_data() {
        ...
        let result = db.migrate_from_json_transaction(&config_with_repository(), |_transaction| {
