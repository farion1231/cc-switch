...
fn schema_create_tables_repairs_legacy_proxy_config_singleton_to_per_app() {
    ...
    let claude_xcode_count: i32 = conn
        .query_row(
            "SELECT COUNT(*) FROM proxy_config WHERE app_type = 'claude-xcode'",
            [],
            |r| r.get(0),
        )
        .expect("query claude-xcode by app_type");
    assert_eq!(claude_xcode_count, 1);
}

#[test]
fn proxy_config_supports_claude_xcode_app_type() {
    let conn = Connection::open_in_memory().expect("open memory db");
    Database::create_tables_on_conn(&conn).expect("create tables");
    let db = Database {
    ...
    let mut config = futures::executor::block_on(db.get_proxy_config_for_app("claude-xcode"))
        .expect("read claude-xcode proxy config");
    config.enabled = true;
    futures::executor::block_on(db.update_proxy_config_for_app(config))
        .expect("update claude-xcode proxy config");

    let stored = futures::executor::block_on(db.get_proxy_config_for_app("claude-xcode"))
        .expect("read stored claude-xcode proxy config");
