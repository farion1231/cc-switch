...
impl Database {
    ...
    /// 一次性初始化默认 Skill 仓库。
    ///
    /// `seed_defaults` 只应在确认是全新安装时传入 `true`。旧版本升级仅写入
    /// 初始化标记，不修改已有仓库列表。标记和默认仓库在同一事务中写入，
    /// 确保用户后续删除一个或全部默认仓库后都不会被启动逻辑重新补回。
...
mod default_skill_repo_tests {
    ...
    fn deleted_default_repository_is_not_restored_on_later_startup() {
        ...
        assert_eq!(
            initial_inserted,
            crate::services::skill::SkillStore::default().repos.len()
        );
        db.delete_skill_repo("JimLiu", "baoyu-skills")
        ...
        assert!(
            repositories
                .iter()
                .all(|repo| !(repo.owner == "JimLiu" && repo.name == "baoyu-skills")),
            "a repository explicitly deleted by the user must stay deleted"
        );
    }

    #[test]
    fn deleting_all_default_repositories_does_not_trigger_reinitialization() {
        let db = Database::memory().expect("memory db");
        db.init_default_skill_repos(true)
            .expect("initialize default repositories");

        for repo in db.get_skill_repos().expect("list default repositories") {
            db.delete_skill_repo(&repo.owner, &repo.name)
                .expect("delete default repository");
        }
        assert!(db
            .get_skill_repos()
            .expect("confirm empty repository list")
            .is_empty());
        ...
        assert!(
            db.get_skill_repos()
                .expect("list repositories after restart")
                .is_empty(),
            "an explicitly emptied repository list must stay empty"
        );
    }
    ...
    fn existing_installation_is_marked_without_injecting_default_repositories() {
        ...
        let inserted_after_restart = db
            .init_default_skill_repos(true)
            .expect("repeat initialization after marker");
        assert_eq!(inserted_after_restart, 0);
        assert!(
            db.get_skill_repos()
                .expect("list repositories after restart")
                .is_empty(),
            "an upgraded installation must not receive defaults later"
        );
    }
}
