...
mod tests {
    ...
    #[test]
    fn grouped_mcode_skill_round_trips_without_shifting_metadata() {
        let db = Database::memory().expect("memory db");
        let group = db.create_skill_group("Work", "blue", &[]).unwrap();
        for enabled in [false, true] {
            let mut item = skill("owner/repo:mcode", "mcode", SkillApps::default());
            item.apps.mcode = enabled;
            item.group_id = Some(group.id.clone());
            item.updated_at = 12345;
            db.save_skill(&item).unwrap();
            let single = db.get_installed_skill(&item.id).unwrap().unwrap();
            let all = db.get_all_installed_skills().unwrap();
            for loaded in [&single, all.get(&item.id).unwrap()] {
                assert_eq!(loaded.apps.mcode, enabled);
                assert_eq!(loaded.group_id, item.group_id);
