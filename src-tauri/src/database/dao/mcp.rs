...
const MCP_SERVER_SELECT: &str =
    "SELECT id, name, server_config, description, homepage, docs, tags, enabled_claude, enabled_codex, enabled_gemini, enabled_grokbuild, enabled_opencode, enabled_hermes, enabled_ohmypi FROM mcp_servers";
...
fn row_to_mcp_server(row: &Row<'_>) -> rusqlite::Result<(String, McpServer)> {
    ...
    let enabled_ohmypi: bool = row.get(13)?;
    ...
    pub fn save_mcp_server(&self, server: &McpServer) -> Result<(), AppError> {
        ...
        conn.execute(
            "INSERT OR REPLACE INTO mcp_servers (
                ...
                enabled_claude, enabled_codex, enabled_gemini, enabled_grokbuild, enabled_opencode, enabled_hermes, enabled_ohmypi
