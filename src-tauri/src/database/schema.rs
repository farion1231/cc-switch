...
impl Database {
    ...
    pub(crate) fn create_tables_on_conn(conn: &Connection) -> Result<(), AppError> {
        ...
        conn.execute(
            ...
            enabled_hermes BOOLEAN NOT NULL DEFAULT 0, enabled_pi BOOLEAN NOT NULL DEFAULT 0
        )",
            [],
        )
        .map_err(|e| AppError::Database(e.to_string()))?;
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    10 => {
                        log::info!("迁移数据库从 v10 到 v11（添加 Pi Agent MCP/Skills 支持）");
    ...
    /// v10 -> v11 迁移：添加 Pi Agent MCP/Skills 支持
    fn migrate_v10_to_v11(conn: &Connection) -> Result<(), AppError> {
        ...
        log::info!("v10 -> v11 迁移完成：已添加 Pi Agent MCP/Skills 支持");
        Ok(())
    }
