...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    17 => {
                        log::info!("迁移数据库从 v17 到 v18（添加 Cursor IDE 支持）");
                        Self::migrate_v17_to_v18(conn)?;
                        Self::set_user_version(conn, 18)?;
                    }
                    18 => {
                        log::info!("迁移数据库从 v18 到 v19（proxy_config 添加 Cursor 支持）");
    ...
    /// v17 -> v18: 添加 Cursor IDE 支持（新增 enabled_cursor 列）
    fn migrate_v17_to_v18(conn: &Connection) -> Result<(), AppError> {
            ...
            && !column_exists(conn, "mcp_servers", "enabled_cursor")?
        {
            ...
            log::info!("Migration v18: Added enabled_cursor column to mcp_servers table");
        }
        ...
        if Self::table_exists(conn, "skills")? && !column_exists(conn, "skills", "enabled_cursor")?
        {
            ...
            log::info!("Migration v18: Added enabled_cursor column to skills table");
        }

        Ok(())
    }

    /// v18 -> v19: proxy_config 表添加 Cursor 支持
    fn migrate_v18_to_v19(conn: &Connection) -> Result<(), AppError> {
        ...
        conn.execute_batch(
            "CREATE TABLE IF NOT EXISTS proxy_config_v19 (
                app_type TEXT PRIMARY KEY CHECK (app_type IN ('claude','codex','gemini','grokbuild','cursor')),
                proxy_enabled INTEGER NOT NULL DEFAULT 0,
                listen_address TEXT NOT NULL DEFAULT '127.0.0.1',
                listen_port INTEGER NOT NULL DEFAULT 15721,
            ...
            INSERT OR IGNORE INTO proxy_config_v19 SELECT * FROM proxy_config;

            -- Insert Cursor default row
            INSERT OR IGNORE INTO proxy_config_v19 (
            ...
            ALTER TABLE proxy_config_v19 RENAME TO proxy_config;"
        )
        .map_err(|e| AppError::Database(format!("迁移 proxy_config 支持 Cursor 失败: {e}")))?;

        log::info!("Migration v19: Added Cursor support to proxy_config table");
        Ok(())
    }
