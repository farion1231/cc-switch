...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    16 => {
                        log::info!("迁移数据库从 v16 到 v17（添加 Cursor IDE 支持）");
                        Self::migrate_v16_to_v17(conn)?;
                        Self::set_user_version(conn, 17)?;
                    }
                    17 => {
                        log::info!("迁移数据库从 v17 到 v18（proxy_config 添加 Cursor 支持）");
    ...
    /// 迁移 v16 到 v17：添加 Cursor IDE 支持（新增 enabled_cursor 列）
    fn migrate_v16_to_v17(conn: &Connection) -> Result<(), AppError> {
            ...
            && !column_exists(conn, "mcp_servers", "enabled_cursor")?
        {
            ...
            log::info!("Migration v17: Added enabled_cursor column to mcp_servers table");
        }
        ...
        if Self::table_exists(conn, "skills")? && !column_exists(conn, "skills", "enabled_cursor")?
        {
            ...
            log::info!("Migration v17: Added enabled_cursor column to skills table");
        }

        Ok(())
    }

    /// 迁移 v17 到 v18：proxy_config 表添加 Cursor 支持
    fn migrate_v17_to_v18(conn: &Connection) -> Result<(), AppError> {
        ...
        conn.execute_batch(
            "CREATE TABLE IF NOT EXISTS proxy_config_v18 (
                app_type TEXT PRIMARY KEY CHECK (app_type IN ('claude','codex','gemini','grokbuild','cursor')),
                proxy_enabled INTEGER NOT NULL DEFAULT 0,
                listen_address TEXT NOT NULL DEFAULT '127.0.0.1',
                listen_port INTEGER NOT NULL DEFAULT 15721,
            ...
            INSERT OR IGNORE INTO proxy_config_v18 SELECT * FROM proxy_config;

            -- Insert Cursor default row
            INSERT OR IGNORE INTO proxy_config_v18 (
            ...
            ALTER TABLE proxy_config_v18 RENAME TO proxy_config;"
        )
        .map_err(|e| AppError::Database(format!("迁移 proxy_config 支持 Cursor 失败: {e}")))?;

        log::info!("Migration v18: Added Cursor support to proxy_config table");
        Ok(())
    }
