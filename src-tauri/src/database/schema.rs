...
impl Database {
    ...
    pub(crate) fn create_tables_on_conn(conn: &Connection) -> Result<(), AppError> {
        ...
        Self::create_management_api_tables_on_conn(conn)?;
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    10 => {
                        log::info!("迁移数据库从 v10 到 v11（添加本地管理 API 支持）");
    ...
    /// v10 -> v11 migration: local Management API token/pairing/audit tables.
    fn migrate_v10_to_v11(conn: &Connection) -> Result<(), AppError> {
        Self::create_management_api_tables_on_conn(conn)?;
        log::info!("v10 -> v11 迁移完成：已添加本地管理 API 支持");
        Ok(())
    }
