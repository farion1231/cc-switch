...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    18 => {
                        log::info!("迁移数据库从 v18 到 v19（收敛会话字节游标与 Hermes 聚合表）");
    ...
    /// v18 -> v19: converge the two development v18 lineages.
    ///
    /// Upstream v18 owns the Claude byte cursor columns, while this PR's v18
    /// owns the Hermes aggregate tables. Either shape may already exist on
    /// disk, so every operation here must be idempotent and preserve rows.
...
mod tests {
    ...
    #[test]
    fn migrate_v17_to_v19_adds_hermes_tables_to_upstream_v17_ledger() -> Result<(), AppError> {
        ...
        // Re-running at v19 must be a no-op that retains both structures.
        Database::apply_schema_migrations_on_conn(&conn)?;
        assert_eq!(Database::get_user_version(&conn)?, SCHEMA_VERSION);
        assert!(Database::table_exists(&conn, "session_usage_dedup")?);
        assert!(Database::table_exists(&conn, "hermes_usage_snapshots")?);
    ...
    #[test]
    fn migrate_v17_to_v19_adds_dedup_and_repairs_candidate_v17_hermes_tables(
    ) -> Result<(), AppError> {
