...
const CC_SWITCH_SQL_EXPORT_HEADER: &str = "-- CC Doctor SQLite 导出";
...
impl Database {
    ...
    fn import_sql_string_inner(
        ...
        let sql_content = sql_raw.trim_start_matches('\u{feff}');
        Self::validate_cc_doctor_sql_export(sql_content)?;
    ...
    fn validate_cc_doctor_sql_export(sql: &str) -> Result<(), AppError> {
        ...
        Err(AppError::localized(
            ...
            "仅支持导入由 CC Doctor 导出的 SQL 备份文件。",
            "Only SQL backups exported by CC Doctor are supported.",
        ))
    }
    ...
    pub(crate) fn backup_database_file(&self) -> Result<Option<PathBuf>, AppError> {
        let db_path = get_app_config_dir().join("cc-doctor.db");
        if !db_path.exists() {
            return Ok(None);
        }
...
mod tests {
    ...
    fn periodic_maintenance_runs_even_when_auto_backup_disabled() -> Result<(), AppError> {
        ...
        let test_home =
            std::env::temp_dir().join("cc-doctor-periodic-maintenance-backup-disabled-test");
