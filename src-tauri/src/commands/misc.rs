...
fn github_release_version_from_json(json: &serde_json::Value) -> Option<String> {
    let from_name = json
        ...
        .and_then(|version| release_display_version(&version));
    from_name.or_else(|| {
        json.get("tag_name")
            .and_then(|v| v.as_str())
...
/// 剥掉 OSC（`ESC ] … BEL|ST`）载荷，避免 shell integration 写入的主机地址
/// `198.51.100.23` 抢在真实版本前命中。非 WSL 探测已用私有描述符隔离 rc；这一步只为
/// 仍与 rc 共用 stdout 的 `try_get_version_wsl` 保留。
static OSC_ESCAPE_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\x1b\][^\x07\x1b]*(?:\x07|\x1b\\)").expect("Invalid OSC escape regex")
});
...
fn extract_version(raw: &str) -> Option<String> {
    let cleaned = OSC_ESCAPE_RE.replace_all(raw, "");
...
///
/// `command -v` 的输出走独占临时文件、三条标准描述符全 null，与 `try_get_version` 同一
/// 套理由：rc 与它共用描述符时，OSC 序列会和路径挤在同一行——`zsh -lic 'command -v git'`
/// 得到 `ESC]1337;RemoteHost=…BEL/opt/homebrew/bin/git`，没有一行以 `/` 开头 → None。
#[cfg(not(target_os = "windows"))]
fn resolve_path_default(
    ...
    // 临时文件建不出来时退化成 Ok(None) 而非 Err：调用方 `locate_default_tool` 会继续扫
    // 候选目录，只丢掉 is_path_default 锚点；升成 Err 会让一次 TMPDIR 抖动直接中断定位。
    let Ok(mut out_file) = tempfile::Builder::new()
    ...
    // 用 `>>` 而非 `>`：rc 里一句 `setopt noclobber`（bash 的 `set -C`）会让 `>` 拒绝写已
    // 存在的文件，而 tempfile 已经把它建出来了。新建空文件上追加与截断等价。
    let mut cmd = Command::new(shell);
    cmd.arg(flag)
        .arg(format!("command -v {tool} >>{quoted_out}"))
        // stdin 显式关闭：继承来的 stdin 可能是终端/管道，交互式 rc 里的读操作会永久阻塞。
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    ...
    // 仍走 wait_child_output 承接 deadline：rc 真挂死时，击杀整棵进程树是唯一的出口，
    // 重定向只挡噪音、挡不住阻塞。三条描述符已全 null，这里没有管道要收，超时语义
    // 不再受"后台进程按住写端不放"的影响。
    //
    ...
    let _ = wait_child_output(child, deadline)?;

    // 走手上的句柄回读，而不是拿路径重新 open：句柄不受被删、被换、TMPDIR 卸载的干扰。
    let mut buf = Vec::new();
    if out_file.rewind().is_err() || out_file.read_to_end(&mut buf).is_err() {
        return Ok(None);
    }
...
mod tests {
    ...
    fn test_extract_version() {
        // 受管工具 `--version` 的真实形状（通道已干净，首个匹配即答案）。
        for (raw, expected) in [
    ...
    /// OSC 载荷里的主机地址会被版本正则抢先匹配，故先剥再取。活着的消费者是仍与用户 rc
    /// 共用 stdout 的 `try_get_version_wsl`（地址取自 RFC 5737 文档保留段）。
    #[test]
    fn extract_version_ignores_osc_payloads() {
        assert_eq!(
    ...
    mod version_probe_seam {
        ...
        struct EnvGuard {
            ...
            zdotdir: Option<std::ffi::OsString>,
        ...
        ///
        /// 工具输出若走管道，读端要等**所有**写端关闭才 EOF，这个后台进程会把它
        /// 按住 20 秒。本用例钉的是 `try_get_version`：它这条链（`get_tool_versions`
        /// 内联调用、无 `spawn_blocking`、无 deadline）没有任何超时兼底，一旦回到管道
        /// 就会把关于面板卡死。（`resolve_path_default` 不同：它走 `wait_child_output`，
        /// 超时会击杀整棵进程树，不依赖本用例护着。）
        ///
        ...
        fn survives_rc_setting_noclobber() {
            ...
            assert_eq!(
                super::super::resolve_path_default("claude", None)
                    .expect("resolve_path_default 不应报错"),
        ...
        fn survives_shell_integration_osc_prefix() {
            ...
            let dir = fixture(RC_NOISE_SHELL_INTEGRATION);
            let _env = EnvGuard::set("/bin/zsh", dir.path());
            assert_found_fixture_version(
                try_get_version("claude"),
                "OSC + 版本形状诱饵挤进版本通道",
            );
            assert_eq!(
                super::super::resolve_path_default("claude", None)
                    .expect("resolve_path_default 不应报错"),
                std::fs::canonicalize(dir.path().join("bin/claude")).ok(),
                "OSC 粘在 command -v 路径前时仍应拿到 fixture 入口"
            );
        }
        ...
        ///
        /// 直接换 fd 0 会污染整个多线程 libtest 进程，`#[serial]` 也挡不住未标记测试。
        /// 因此父用例重启当前测试二进制、用 `--exact` 只跑自己；只有那个隔离子进程才把 fd 0
        /// 换成装着已知字节的文件。marker 为空才说明探测没有读到调用方 stdin。
        #[test]
        fn returns_promptly_when_the_rc_reads_from_stdin() {
            const CHILD_ENV: &str = "CC_SWITCH_VERSION_PROBE_STDIN_CHILD";
            if std::env::var_os(CHILD_ENV).is_some() {
                run_stdin_probe_child();
                return;
            }
            if !zsh_available() {
                return;
            ...
            .env(CHILD_ENV, "1")
            .output()
            .expect("spawn isolated stdin probe");
        ...
        fn run_stdin_probe_child() {
            ...
            // fd 0 的置换只发生在这个仅运行本用例的隔离进程中。还原仍走 Drop，确保中途
            // 断言失败时 descriptor 不泄漏，也让 helper 能独立重复调用。
            ...
            let start = std::time::Instant::now();
            let probe = try_get_version("claude");
            // 路径解析必须在 fd 0 仍是 feed 时跑，否则断言就分辨不出守卫在不在。
            let path_default = super::super::resolve_path_default("claude", None)
                .expect("resolve_path_default 不应报错");
            let elapsed = start.elapsed();
