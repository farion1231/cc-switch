...
pub fn emit_profile_apply_events(
    ...
    for app_type in scope.apps().iter() {
        ...
        let (proxy_enabled, auto_failover_enabled) = if app_type.supports_local_proxy() {
            state.db.get_proxy_flags_sync(app_str)
        } else {
            (false, false)
        };
...
pub fn list_profiles(state: State<'_, AppState>) -> Result<ProfilesResponse, String> {
    ...
    let current_ids = CurrentProfileIds {
        ...
        codex_desktop: state
            .db
            .get_current_profile_id(ProfileScope::CodexDesktop.as_str())
            .map_err(|e| e.to_string())?,
    };
    Ok(ProfilesResponse {
