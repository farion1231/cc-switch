...
///
/// 使用 `auth.json` 里 ChatGPT 登录的 access_token 直接拉官方 /models；
/// 未登录或请求失败时返回错误，前端回退到内置插槽列表。
#[tauri::command(rename_all = "camelCase")]
pub async fn get_codex_official_models() -> Result<Vec<String>, String> {
    let token = crate::codex_config::read_codex_auth_access_token()
        .ok_or_else(|| "未登录 ChatGPT（auth.json 缺少 access_token）".to_string())?;
    let models =
        crate::services::codex_oauth_models::fetch_official_models_with_token(&token).await?;
