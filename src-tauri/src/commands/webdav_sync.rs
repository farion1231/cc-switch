...
pub async fn webdav_sync_prepare_download(state: State<'_, AppState>) -> Result<Value, String> {
    ...
    let sync_result =
        run_with_webdav_lock(webdav_sync_service::prepare_download(&db, &mut settings)).await;
...
#[tauri::command]
pub async fn webdav_sync_apply_download(
    ...
    let sync_result = run_with_webdav_lock(webdav_sync_service::apply_download(
        &db,
        &mut settings,
        &preview_id,
        &selections,
    ...
    if let Some(msg) = warning.as_ref() {
        log::warn!("[WebDAV] post-apply sync warning: {msg}");
    }
    result = attach_warning(result, warning);
    Ok(result)
}
