...
use super::misc::{
    ...
    PARENT_CLAUDE_SESSION_ENV_VARS, WINDOWS_BATCH_PATH_ENV,
};
...
/// Bind the path inside the new shell, not in WT's potentially stale environment.
/// Encoding the entire script also keeps literal %VAR% paths away from WT's
/// ExpandEnvironmentStringsW pass. The CMD variable is expanded only once later;
/// the generated BAT needs command extensions, without changing the host shell.
#[cfg(any(target_os = "windows", test))]
fn wt_batch_script(bat_path: &str) -> String {
    let quoted = quote_windows_batch_path_for_env(bat_path).replace('\'', "''");
    // Older WT may also retain the resident's session/color environment. Clear
    // the same variables unconditionally at the child boundary, not just spawn.
    let clear_env: String = PARENT_CLAUDE_SESSION_ENV_VARS
        .iter()
        .map(|name| format!("$env:{name} = $null; "))
        .collect();
    format!(
        "{clear_env}$env:{WINDOWS_BATCH_PATH_ENV} = '{quoted}'; try {{ & $env:ComSpec /D /E:ON /V:OFF /C '%{WINDOWS_BATCH_PATH_ENV}%' }} finally {{ $env:{WINDOWS_BATCH_PATH_ENV} = $null }}"
    )
}
...
fn run_wt_command(launcher: &str, args: &[&str]) -> Result<(), String> {
    ...
    // The path now travels in the encoded command, never in the WT resident's env.
    command.env_remove(WINDOWS_BATCH_PATH_ENV);
    command
        .args(args)
...
#[cfg(target_os = "windows")]
pub(super) fn launch_wt_terminal(bat_path: &str) -> Result<(), String> {
    ...
    let encoded_command = powershell_encoded_command(&wt_batch_script(bat_path));
...
mod tests {
    ...
    fn wt_batch_path_is_expanded_only_in_the_new_shell() {
        ...
        let hostile_dir = temp
            .path()
            .join("space & %CC_SWITCH_CMD_EXPAND% ^ !CC_SWITCH_BANG! (test) and O'Brien");
        ...
        let encoded_command = powershell_encoded_command(&format!(
            "{}; exit $LASTEXITCODE",
            wt_batch_script(&batches[1].path().to_string_lossy())
        ));
        let mut failures = Vec::new();
...
"#,
    ...
    fn wt_launch_executes_batch_in_real_terminal() {
        ...
        for (index, (profile, supports_append)) in cases.iter().enumerate() {
            ...
            if index == 0 {
                launch_wt_terminal(&bat_path).expect("WT launch should succeed");
            } else {
    ...
    fn wt_resident_launches_each_unique_batch() {
        ...
        if std::env::var_os("CC_SWITCH_WT_TEST_SEED_STALE").is_some() {
            ...
            // here: it correctly removes the private variable from WT's env.
        ...
        .enumerate()
        {
            ...
            let encoded =
                powershell_encoded_command(&wt_batch_script(&batch.path().to_string_lossy()));
