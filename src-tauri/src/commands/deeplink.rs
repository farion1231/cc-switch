use crate::bridges::deeplink as deeplink_bridge;
...
pub fn parse_deeplink(url: String) -> Result<DeepLinkImportRequest, String> {
    ...
    deeplink_bridge::parse_deeplink(&url).map_err(|e| e.to_string())
}
...
pub fn merge_deeplink_config(
    ...
    deeplink_bridge::merge_deeplink_config(request).map_err(|e| e.to_string())
}
...
pub fn import_from_deeplink(
    ...
    let _ = state;
    let provider_id = deeplink_bridge::import_provider(request).map_err(|e| e.to_string())?;
...
pub async fn import_from_deeplink_unified(
    ...
    match request.resource.as_str() {
        "provider" => {
            let _ = state;
            let provider_id =
                deeplink_bridge::import_provider(request).map_err(|e| e.to_string())?;
            Ok(serde_json::json!({
                "type": "provider",
                "id": provider_id
            }))
        ...
        "prompt" => {
            let _ = state;
            let prompt_id = deeplink_bridge::import_prompt(request).map_err(|e| e.to_string())?;
            Ok(serde_json::json!({
                "type": "prompt",
                "id": prompt_id
            }))
        ...
        "mcp" => {
            let _ = state;
            deeplink_bridge::import_mcp(request).map_err(|e| e.to_string())
        }
        "skill" => {
            let _ = state;
            let skill_key = deeplink_bridge::import_skill(request).map_err(|e| e.to_string())?;
            Ok(serde_json::json!({
                "type": "skill",
                "key": skill_key
            }))
