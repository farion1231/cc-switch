...
pub async fn install_update_and_restart(app: AppHandle) -> Result<bool, String> {
    crate::services::app_update::install_staged_or_download_and_restart(&app).await
}

/// 前端检查到新版本后的显式触发：后端后台预下载安装包，让「点升级」时
/// 无需现场下载。失败对用户无感（安装命令会回退为现场下载）。
#[tauri::command]
pub async fn prestage_app_update(app: AppHandle) -> Result<(), String> {
    crate::services::app_update::prestage_on_demand(&app).await
}
