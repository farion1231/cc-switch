...
use providers::{claude, codex, gemini, hermes, openclaw, opencode, kilo};
...
pub fn scan_sessions() -> Vec<SessionMeta> {
    let (r1, r2, r3, r4, r5, r6, r7) = std::thread::scope(|s| {
        ...
        let h7 = s.spawn(kilo::scan_sessions);
        (
...
pub fn load_messages(provider_id: &str, source_path: &str) -> Result<Vec<SessionMessage>, String> {
    ...
    if provider_id == "kilo" && source_path.starts_with("sqlite:") {
        return kilo::load_messages_sqlite(source_path);
    }
...
pub fn delete_session(
    ...
    if provider_id == "kilo" && source_path.starts_with("sqlite:") {
        return kilo::delete_session_sqlite(session_id, source_path);
    }
    ...
    for root in roots {
        ...
        if validated_source.starts_with(&validated_root) {
            return match provider_id {
                ...
                "kilo" => kilo::delete_session(&validated_root, &validated_source, session_id),
                _ => Err(format!("Unsupported provider: {provider_id}")),
            };
        }
    }
