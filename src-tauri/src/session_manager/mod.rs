...
use providers::{claude, codex, gemini, hermes, mimocode, openclaw, opencode};
...
pub fn scan_sessions() -> Vec<SessionMeta> {
    let (r1, r2, r3, r4, r5, r6, r7) = std::thread::scope(|s| {
        ...
        let h7 = s.spawn(mimocode::scan_sessions);
        (
...
pub fn load_messages(provider_id: &str, source_path: &str) -> Result<Vec<SessionMessage>, String> {
    ...
    if provider_id == "mimo" && source_path.starts_with("sqlite:") {
    ...
    match provider_id {
        ...
        "mimo" => mimocode::load_messages(path),
    ...
    for root in roots {
        ...
        if validated_source.starts_with(&validated_root) {
            return match provider_id {
                ...
                "mimo" => mimocode::delete_session(&validated_root, &validated_source, session_id),
                _ => Err(format!("Unsupported provider: {provider_id}")),
            };
        }
    }
...
fn provider_roots(provider_id: &str) -> Result<Vec<PathBuf>, String> {
    let roots = match provider_id {
        ...
        "mimo" => vec![mimocode::get_mimocode_storage_dir()],
