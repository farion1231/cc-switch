...
use providers::{
    claude, codebuddy, codex, gemini, grokbuild, hermes, openclaw, opencode, pi, workbuddy,
};
...
pub fn scan_sessions() -> Vec<SessionMeta> {
    let (r1, r2, r3, r4, r5, r6, r7, r8, r9, r10) = std::thread::scope(|s| {
        ...
        let h9 = s.spawn(codebuddy::scan_sessions);
        let h10 = s.spawn(workbuddy::scan_sessions);
        (
...
pub fn load_messages(provider_id: &str, source_path: &str) -> Result<Vec<SessionMessage>, String> {
    ...
    match provider_id {
        ...
        "codebuddy" => codebuddy::load_messages(path),
    ...
    for root in roots {
        ...
        if validated_source.starts_with(&validated_root) {
            return match provider_id {
                ...
                "codebuddy" => {
                    codebuddy::delete_session(&validated_root, &validated_source, session_id)
                }
                "workbuddy" => {
                    workbuddy::delete_session(&validated_root, &validated_source, session_id)
                }
                _ => Err(format!("Unsupported provider: {provider_id}")),
            };
        }
...
fn provider_roots(provider_id: &str) -> Result<Vec<PathBuf>, String> {
    let roots = match provider_id {
        ...
        "pi" => pi::session_roots(),
        "codebuddy" => codebuddy::session_roots(),
        "workbuddy" => workbuddy::session_roots(),
