...
use providers::{claude, codex, gemini, grokbuild, hermes, openclaw, opencode, qodercli};
...
pub fn scan_sessions() -> Vec<SessionMeta> {
    let (r1, r2, r3, r4, r5, r6, r7, r8) = std::thread::scope(|s| {
        ...
        let h8 = s.spawn(qodercli::scan_sessions);
        (
...
pub fn load_messages(provider_id: &str, source_path: &str) -> Result<Vec<SessionMessage>, String> {
    ...
    match provider_id {
        ...
        "qodercli" => qodercli::load_messages(path),
    ...
    for root in roots {
        ...
        if validated_source.starts_with(&validated_root) {
            return match provider_id {
                ...
                "qodercli" => {
                    qodercli::delete_session(&validated_root, &validated_source, session_id)
                }
                _ => Err(format!("Unsupported provider: {provider_id}")),
            };
        }
...
fn provider_roots(provider_id: &str) -> Result<Vec<PathBuf>, String> {
    let roots = match provider_id {
        ...
        "qodercli" => vec![crate::qodercli_config::get_qodercli_dir().join("projects")],
