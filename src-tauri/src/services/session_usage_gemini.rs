...
struct GeminiSessionUsagePersistence<'a> {
    ...
    current_request_ids: Vec<String>,
}
...
fn sync_single_gemini_file(db: &Database, file_path: &Path) -> Result<(u32, u32), AppError> {
    ...
    let mut current_request_ids = Vec::new();
    ...
    for msg in messages {
        ...
        current_request_ids.push(request_id.clone());
...
fn persist_gemini_session_usage(
    ...
    if let Some(session_id) = session_id.filter(|id| !id.trim().is_empty()) {
        delete_stale_gemini_session_rows_on_conn(&tx, session_id, &current_request_ids)?;
    }
...
/// A Gemini transcript is a complete session snapshot.  Remove native raw
/// rows whose request IDs disappeared from the rewritten snapshot before the
/// canonical replacement is committed.  Proxy rows and other Gemini sources
/// remain outside this source/session-owned partition.
fn delete_stale_gemini_session_rows_on_conn(
    conn: &rusqlite::Connection,
    session_id: &str,
    current_request_ids: &[String],
) -> Result<(), AppError> {
    let current_request_ids = current_request_ids
        .iter()
        .map(String::as_str)
        .collect::<HashSet<_>>();
    let stale_request_ids = conn
        .prepare(
            "SELECT request_id FROM proxy_request_logs
             WHERE app_type = ?1 AND data_source = ?2 AND session_id = ?3",
        )?
        .query_map(
            rusqlite::params![GEMINI_APP_TYPE, GEMINI_SESSION_DATA_SOURCE, session_id],
...
mod tests {
    ...
    #[test]
    fn rewritten_gemini_session_removes_obsolete_compatibility_rows() -> Result<(), AppError> {
        let db = Database::memory()?;
        let dir = tempdir().expect("tempdir");
        let path = dir.path().join("session-drop.json");
        let session_id = "gemini-drop-session";
        let retained_id = "retained-message";
        let removed_id = "removed-message";

        let message = |id: &str, input: u64| {
            serde_json::json!({
                "id": id,
                "type": "gemini",
                "model": "gemini-drop-model",
                "timestamp": "2026-08-18T00:00:30Z",
                "tokens": {"input": input, "output": 2, "cached": 1, "thoughts": 0}
            })
        };
        let initial = serde_json::json!({
            "sessionId": session_id,
            "messages": [message(retained_id, 10), message(removed_id, 20)]
        });
        fs::write(
            &path,
            serde_json::to_vec(&initial).expect("serialize initial session"),
        )
        .expect("write initial session");
        assert_eq!(sync_single_gemini_file(&db, &path)?, (2, 0));

        let rewritten = serde_json::json!({
            "sessionId": session_id,
            "messages": [message(retained_id, 30)]
        });
        fs::write(
            &path,
            serde_json::to_vec(&rewritten).expect("serialize rewritten session"),
        )
        .expect("write rewritten session");
        clear_gemini_sync_state(&db, &path)?;
        assert_eq!(sync_single_gemini_file(&db, &path)?, (1, 0));
        ...
        let raw_counts: (i64, i64) = conn.query_row(
            "SELECT
                (SELECT COUNT(*) FROM proxy_request_logs
                 WHERE data_source = 'gemini_session' AND session_id = ?1),
                (SELECT COUNT(*) FROM proxy_request_logs
                 WHERE request_id = ?2)",
            rusqlite::params![
                session_id,
                format!("gemini_session:{session_id}:{removed_id}")
            ],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )?;
        assert_eq!(raw_counts, (1, 0));
