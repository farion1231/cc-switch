...
fn sync_single_file_with_context(
    ...
    // Keep the normalized identity durable even when its transcript cursor is
    // unchanged. The cursor values themselves are written back unchanged in
    // the same node transaction, so this cannot advance an unscanned file.
