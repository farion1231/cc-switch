...
use crate::services::session_usage::{get_sync_state, metadata_modified_nanos};
use chrono::DateTime;
...
pub(crate) struct OpenClawSyncResult {
    ...
    pub(crate) source_revisions: Vec<OpenClawSourceRevision>,
    pub(crate) removed_source_paths: Vec<String>,
...
pub(crate) struct OpenClawSourceRevision {
...
fn scan_openclaw_sessions(root: impl AsRef<Path>) -> OpenClawSyncResult {
    scan_openclaw_sessions_filtered(root, None)
}

fn scan_openclaw_sessions_filtered(
    ...
    for agent_path in agent_paths {
        ...
        for path in session_paths {
            ...
            if changed_paths.is_some_and(|paths| !paths.contains(&path)) {
                continue;
            }
            match parse_session_file(&agent_id, &path, &native_metadata) {
                Ok(claim) => {
                    ...
                    if let Ok(metadata) = fs::metadata(&path) {
                        result.source_revisions.push(OpenClawSourceRevision {
                            file_path: path.to_string_lossy().to_string(),
                            last_modified: metadata_modified_nanos(&metadata),
                            last_offset: i64::try_from(metadata.len()).unwrap_or(i64::MAX),
                        });
                    }
                }
...
pub(crate) fn sync_openclaw_session_usage(
    db: &Database,
    root: impl AsRef<Path>,
) -> Result<OpenClawSyncResult, AppError> {
    let root = root.as_ref();
    let OpenClawDiscovery {
        paths,
        index_paths,
        mut errors,
        reconciliation_safe,
    } = discover_openclaw_session_files(root);
    let discovered_source_paths = paths
        .iter()
        .chain(index_paths.iter())
        .map(|path| path.to_string_lossy().to_string())
        .collect::<HashSet<_>>();
    let mut changed_paths = HashSet::new();
    let mut changed_session_dirs = HashSet::new();
    for path in &paths {
        let revision = match source_revision(path) {
            ...
            Err(_) => {
                ...
                if let Some(sessions_dir) = path.parent() {
                    changed_session_dirs.insert(sessions_dir.to_path_buf());
                }
                continue;
            }
        };
        let (saved_modified, saved_offset) = get_sync_state(db, &revision.file_path)?;
        if revision.last_modified != saved_modified || revision.last_offset != saved_offset {
            changed_paths.insert(path.clone());
            if let Some(sessions_dir) = path.parent() {
                changed_session_dirs.insert(sessions_dir.to_path_buf());
            }
    ...
    // Duplicate session IDs are resolved by stable file ordering within one
    // session directory. Reparse that bounded group together when any member
    // changes so an unchanged sibling cannot silently lose its precedence.
    for path in &paths {
        if path
            .parent()
            .is_some_and(|sessions_dir| changed_session_dirs.contains(sessions_dir))
        {
            changed_paths.insert(path.clone());
        }
    }

    // `sessions.json` supplies display names and cwd. A change to that index
    // requires all sessions in its directory to be reparsed, and its own
    // revision is committed with the node updates so idle syncs stay read-only.
    let mut changed_indexes = Vec::new();
    let mut seen_indexes = HashSet::new();
    for index_path in &index_paths {
        let Some(sessions_dir) = index_path.parent() else {
            continue;
        };
        if !seen_indexes.insert(index_path.clone()) {
            continue;
        }
        let revision = match source_revision(index_path) {
            Ok(revision) => revision,
            Err(error) => {
                errors.push(format!(
                    "OpenClaw sessions index {} is unreadable: {error}",
                    index_path.display()
                ));
                continue;
            }
        ...
        let (saved_modified, saved_offset) = get_sync_state(db, &revision.file_path)?;
        if revision.last_modified != saved_modified || revision.last_offset != saved_offset {
            changed_indexes.push(revision);
            for session_path in &paths {
                if session_path.parent() == Some(sessions_dir) {
                    changed_paths.insert(session_path.clone());
                }
            }
        }
    }

    let removed_source_paths = if reconciliation_safe {
        stale_openclaw_source_paths(db, root, &discovered_source_paths)?
    } else {
        Vec::new()
    };

    if changed_paths.is_empty() && removed_source_paths.is_empty() {
        return Ok(OpenClawSyncResult {
            files_scanned: paths.len() as u32,
            errors,
            ..OpenClawSyncResult::default()
        });
    }

    let mut result = scan_openclaw_sessions_filtered(root, Some(&changed_paths));
    result.errors.extend(errors);
    result.errors.sort();
    result.errors.dedup();
    result.removed_source_paths = removed_source_paths;
    result.source_revisions.extend(changed_indexes);
    result
        .source_revisions
...
fn source_revision(path: &Path) -> std::io::Result<OpenClawSourceRevision> {
    let metadata = fs::metadata(path)?;
    Ok(OpenClawSourceRevision {
        file_path: path.to_string_lossy().to_string(),
        last_modified: metadata_modified_nanos(&metadata),
        last_offset: i64::try_from(metadata.len()).unwrap_or(i64::MAX),
    })
...
fn discover_openclaw_session_files(root: &Path) -> OpenClawDiscovery {
    let agents_dir = root.join("agents");
    if !agents_dir.is_dir() {
        return OpenClawDiscovery::default();
    }
    let agent_paths = match sorted_child_directories(&agents_dir) {
        Ok(paths) => paths,
        Err(error) => {
            return OpenClawDiscovery {
                ...
                ..OpenClawDiscovery::default()
            }
        }
    };
    let mut discovery = OpenClawDiscovery {
    ...
    for agent_path in agent_paths {
        ...
        if sessions_dir.is_dir() {
            ...
            if index_path.is_file() {
                discovery.index_paths.push(index_path);
            }
            match sorted_jsonl_files(&sessions_dir) {
                Ok(paths) => discovery.paths.extend(paths),
                Err(error) => {
                    discovery.reconciliation_safe = false;
    ...
    discovery.index_paths.sort();
    discovery.index_paths.dedup();
    discovery
}

/// OpenClaw node source paths prove which cursors belong to this provider.
/// Once the configured root has been enumerated without directory errors,
/// paths absent from that scan are safe to reconcile. A missing or unreadable
/// root is deliberately not treated as a deletion signal.
fn stale_openclaw_source_paths(
    db: &Database,
    root: &Path,
    discovered_source_paths: &HashSet<String>,
) -> Result<Vec<String>, AppError> {
    let source_root = normalize_path_key(&root.join("agents"));
    let conn = crate::database::lock_conn!(db.conn);
    let mut statement = conn.prepare(
        "SELECT source_path FROM agent_session_nodes
         WHERE app_type = 'openclaw' AND source_path IS NOT NULL",
    )?;
    let stored_paths = statement
        .query_map([], |row| row.get::<_, Option<String>>(0))?
        .collect::<Result<Vec<_>, _>>()?;
    let discovered_source_paths = discovered_source_paths
        .iter()
        .map(|path| normalize_path_key(Path::new(path)))
        .collect::<HashSet<_>>();
    let mut stale_paths = stored_paths
        .into_iter()
        .flatten()
        .filter(|path| {
            let normalized = normalize_path_key(Path::new(path));
            normalized.starts_with(&(source_root.clone() + "/"))
                && !discovered_source_paths.contains(&normalized)
        })
        .collect::<Vec<_>>();
    stale_paths.sort();
    stale_paths.dedup();
