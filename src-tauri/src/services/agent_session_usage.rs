    ...
    let sql = format!(
        "WITH node_map AS (
             ...
             SELECT COALESCE(n.root_session_id, l.session_id) AS root_session_id,
                    ...
                    CASE WHEN COALESCE(l.data_source, 'proxy') IN
                                   ('session_log', 'codex_session', 'gemini_session',
                                    'grok_session', 'opencode_session', 'pi_session')
...
fn enrich_codex_session_costs(conn: &Connection, app_type: &str, groups: &mut [UsageGroup]) {
    ...
    for group in groups {
        ...
        // Legacy Codex facts exclude cache reads from normalized input. Total
        // facts exclude both cache reads and cache creation, so each component
        // is priced exactly once. An absent cache-creation component remains
...
fn codex_replay_hides_unpublished_snapshot_on_conn(conn: &Connection) -> Result<bool, AppError> {
    Ok(
        crate::services::session_usage_codex::codex_replay_in_progress_on_conn(conn)
            && !codex_published_snapshot_on_conn(conn)?,
    )
}
...
impl TaskSourceScope {
    fn new(
        ...
        let hide_pending_codex = codex_replay_hides_unpublished_snapshot_on_conn(conn)?;
...
pub fn get_agent_session_usage(
    ...
    let replay_hides_snapshot =
        app_type == "codex" && codex_replay_hides_unpublished_snapshot_on_conn(&conn)?;
    let groups = if replay_hides_snapshot {
        Vec::new()
    } else {
        query_usage_groups(
            &conn,
    ...
    let supports_descendants = agent_usage_capability(&app_type)
        ...
        .unwrap_or(false);
    let mut summary = summary_for_root(
        &root,
    ...
    if replay_hides_snapshot {
        summary.warnings.push(
            "Codex session usage is rebuilding; no complete canonical snapshot is available".into(),
        );
    }
    Ok(summary)
}
...
mod tests {
    ...
    #[test]
    fn unmarked_legacy_pi_zero_cost_is_unavailable() -> Result<(), AppError> {
        let db = Database::memory()?;
        db.upsert_agent_session_node(&query_node("pi", "legacy-pi", None))?;
        {
            let conn = crate::database::lock_conn!(db.conn);
            conn.execute(
                "INSERT INTO proxy_request_logs (
                    ...
                    cache_creation_tokens, total_cost_usd, latency_ms,
                    status_code, created_at, session_id, data_source
                 ) VALUES (
                    'pi-legacy-request', '_pi_session', 'pi', 'pi-model',
                    10, 5, 2, 1, '0', 0, 200, 2000,
                    'legacy-pi', 'pi_session'
                 )",
                [],
            )?;
        }
        ...
        assert_eq!(
            summary
                .total_usage
                .as_ref()
                .and_then(|usage| usage.total_cost_usd.as_deref()),
            None
        );
        assert!(summary
            .warnings
            .iter()
            .any(|warning| warning.contains("cost is unavailable")));
        Ok(())
    }
    ...
    #[test]
    fn codex_session_summary_hides_legacy_raw_during_initial_replay() -> Result<(), AppError> {
        let db = Database::memory()?;
        db.upsert_agent_session_node(&query_node("codex", "replay-session", None))?;
        {
            let conn = crate::database::lock_conn!(db.conn);
            conn.execute(
                "INSERT INTO proxy_request_logs (
                    ...
                    data_source
                 ) VALUES ('legacy-codex', '_codex_session', 'codex', 'gpt-5', 'gpt-5',
                           10, 5, 2, 0, '0', 0, 200, 2000, 'replay-session', 'codex_session')",
                [],
            )?;
            conn.execute(
                "INSERT OR REPLACE INTO settings (key, value)
        ...
        assert!(summary
            .warnings
            .iter()
            .any(|warning| warning.contains("rebuilding")));
        Ok(())
    }
