...
/// Parse all currently discovered roots. Missing roots are intentionally a
/// no-op; one unreadable root is reported without blocking other roots. The
/// caller owns the one final transaction so it can apply cross-source proxy
/// ownership before any Cowork canonical fact is visible.
pub(crate) fn collect_cowork_usage() -> Result<CoworkUsageBatch, AppError> {
    collect_cowork_usage_from_roots(&discover_cowork_roots())
}

/// Parse explicit roots. This is the fixture seam used by tests and by callers
...
pub(crate) fn collect_cowork_usage_from_roots(
    ...
    for root in roots {
        ...
        let batch = collect_single_root(root)?;
...
fn collect_single_root(root: &Path) -> Result<CoworkUsageBatch, AppError> {
