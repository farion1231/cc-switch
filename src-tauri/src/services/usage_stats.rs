...
fn provider_name_coalesce(log_alias: &str, provider_alias: &str) -> String {
    format!(
        "COALESCE({provider_alias}.name, CASE {log_alias}.provider_id \
         ...
         WHEN '_kilo_session' THEN 'Kilo (Session)' \
...
pub(crate) fn effective_usage_log_filter(log_alias: &str) -> String {
    ...
    format!(
        "NOT (
            {data_source} IN ('session_log', 'codex_session', 'gemini_session', 'opencode_session', 'kilo_session')
            AND EXISTS (
                SELECT 1
                ...
                WHERE {proxy_data_source} = 'proxy'
                  ...
                  AND (
                      ...
                      OR (
                          ...
                          AND {data_source} IN ('codex_session', 'gemini_session', 'opencode_session', 'kilo_session')
                      )
                  )
...
pub(crate) fn has_matching_proxy_usage_log(
    ...
    let allow_missing_cache_creation =
        matches!(key.app_type, "codex" | "gemini" | "opencode" | "kilo") && key.cache_creation_tokens == 0;
