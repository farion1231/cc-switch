...
pub struct RequestLogDetail {
    ...
    /// 供应商类型；路由 provider 为 "cc_switch_route"（UP 路由），前端据此标记来源。
    #[serde(skip_serializing_if = "Option::is_none")]
...
///  data_source, pricing_model, input_token_semantics, provider_type`
///
    ...
    pub fn get_request_logs(
        ...
        let sql = format!(
            "SELECT l.request_id, l.provider_id, {logs_pname} as provider_name, l.app_type, l.model,
                    ...
                    l.input_token_semantics, l.provider_type
             FROM proxy_request_logs l
             LEFT JOIN providers p ON l.provider_id = p.id AND l.app_type = p.app_type
             {where_clause}
