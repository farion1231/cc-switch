        ...
        for row in rows {
            ...
            if bucket_idx >= bucket_count {
                ...
                if let Some(existing) = map.get(&target_bucket) {
                    log::info!("→ Accumulating: existing {} req + new {} req = {} req",
                        existing.request_count, stat.request_count, existing.request_count + stat.request_count);
                    ...
                    let merged_stat = DailyStats {
                        ...
                        total_input_tokens: existing.total_input_tokens + stat.total_input_tokens,
                        total_output_tokens: existing.total_output_tokens + stat.total_output_tokens,
                        total_cache_creation_tokens: existing.total_cache_creation_tokens + stat.total_cache_creation_tokens,
                        total_cache_read_tokens: existing.total_cache_read_tokens + stat.total_cache_read_tokens,
                    };
                ...
                // 如果目标桶还没有数据，直接插入
                log::info!("→ Bucket {} has no existing data, inserting boundary data", target_bucket);
