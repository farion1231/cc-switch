...
fn sync_from_roots(
    ...
    let sync_cursors = load_sync_cursors(db)?;
    let previous_catalog_fingerprint = sync_cursors
        .get(CATALOG_SYNC_KEY)
        .map(|cursor| cursor.last_line_offset)
        .unwrap_or(0);
    ...
    for file_path in files {
        ...
        let last_modified = sync_cursors
            .get(&sync_key)
            .map(|cursor| cursor.last_modified)
            .unwrap_or(0);
