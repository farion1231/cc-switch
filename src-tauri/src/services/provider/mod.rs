...
"#;
    ...
    async fn update_current_claude_provider_syncs_live_when_proxy_takeover_detected_without_backup()
    {
        ...
        assert_eq!(
            live.get("permissions").cloned(),
            Some(json!({ "allow": ["Bash"] })),
            "provider edits must not overwrite user-owned Claude permission settings during takeover"
        );
        assert_eq!(
            live.get("env")
        ...
        assert_eq!(
            live.get("env")
                ...
                .and_then(|v| v.as_str()),
            None,
            "takeover live config should expose Claude role aliases instead of the provider fallback model"
        );
    }
        ...
        if is_current {
            ...
            if should_sync_via_proxy {
                if matches!(app_type, AppType::ClaudeDesktop) {
                    write_live_with_common_config(state.db.as_ref(), &app_type, &provider)?;
                } else if live_taken_over && !has_live_backup {
