...
pub(crate) use live::{
    backfill_live_into_provider, build_effective_provider_for_live_with_codex_oauth_manager,
    build_effective_settings_with_common_config, live_settings_have_content,
...
pub fn reapply_current_codex_official_live(state: &AppState) -> Result<bool, AppError> {
    ...
    // 接管中的 live 是代理占位配置，不能回填进 DB。只在即将整文件重写前
    // 把用户在 [mcp_servers] 里的手工编辑吸收进 DB（warn-degrade）。
    if !live_taken_over {
...
impl ProviderService {
    ...
    pub fn update(
        ...
        // read through the final provider-row commit. Codex also needs this
        // lock before reading existing/current so a concurrent unbind cannot
        // race the managed-account decision. Additive OMO updates keep their
        // existing dedicated transaction below.
    ...
    fn switch_normal(
        ...
        if use_managed_codex_transaction {
            ...
            // Codex: validate the live projection before writing live —
            ...
            // Capture raw bytes immediately before the first live write.
            // DB/provider reconstruction is not a valid rollback source because
            // live may contain newer manual edits or unknown fields that
            // backfill could not persist.
            ...
            // Sync to live first (write_gemini_live handles security flag
            // internally for Gemini). A multi-file writer can fail after
            // changing an earlier file, so use the same snapshot for write
            // failure as well as later pointer failure.
            if let Err(write_error) = Self::write_preflighted_or_current_live(
                state,
                &app_type,
                provider,
                preflighted_provider.as_ref(),
            ...
            if !app_type.is_additive_mode() {
                let live_snapshot = live_snapshot
                    .expect("exclusive provider switch must capture a live file snapshot");
