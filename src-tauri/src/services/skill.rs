...
const CODEX_UI_METADATA_MARKER: &str =
    "# Generated by CC Switch for Codex UI metadata; DO NOT EDIT.\n";
const CODEX_UI_DESCRIPTION_MIN_CHARS: usize = 25;
const CODEX_UI_DESCRIPTION_MAX_CHARS: usize = 64;
...
#[derive(Serialize)]
struct CodexUiMetadata {
    interface: CodexUiInterface,
}

#[derive(Serialize)]
struct CodexUiInterface {
    display_name: String,
...
impl SkillService {
    ...
    fn collect_files_for_hash(base: &Path, current: &Path, files: &mut Vec<PathBuf>) -> Result<()> {
        ...
        for entry in entries {
            ...
            } else if !Self::is_generated_codex_ui_metadata(base, &path)? {
                files.push(path);
            }
        }
        Ok(())
    ...
    /// CC Switch 在 Codex 同步时补齐的 UI metadata 是可重建的平台产物，
    /// 不应让远程 Skill 永久显示「有更新」。只有精确路径且带稳定 marker
    /// 的普通文件才中性；上游原生 metadata 和其他带相同文本的文件仍参与哈希。
    fn is_generated_codex_ui_metadata(base: &Path, path: &Path) -> Result<bool> {
        ...
        Ok(content.starts_with(CODEX_UI_METADATA_MARKER.as_bytes()))
    }
    ...
    fn codex_short_description(description: Option<&str>, display_name: &str) -> String {
        let source = description
            .filter(|value| !value.trim().is_empty())
            .unwrap_or(display_name);
        let mut value = source.split_whitespace().collect::<Vec<_>>().join(" ");

        if value.chars().count() < CODEX_UI_DESCRIPTION_MIN_CHARS {
            value.push_str(" and related workflows");
        }
        if value.chars().count() < CODEX_UI_DESCRIPTION_MIN_CHARS {
            value.push_str(" with practical guidance");
        }
        if value.chars().count() > CODEX_UI_DESCRIPTION_MAX_CHARS {
            value = value
                .chars()
                .take(CODEX_UI_DESCRIPTION_MAX_CHARS)
                .collect::<String>()
                .trim_end()
                .to_string();
        }
    ...
    /// 在 SSOT 中补齐 Codex TUI Picker 所需的 UI metadata。
    ///
    /// 已存在的文件无论来自上游还是用户都保持字节不变。临时文件通过
    /// `persist_noclobber` 发布，避免并发同步覆盖刚刚出现的原生 metadata。
    fn ensure_codex_ui_metadata(source: &Path, directory: &str) -> Result<()> {
        let agents_dir = source.join("agents");

        match fs::symlink_metadata(&agents_dir) {
            Ok(metadata) if metadata.file_type().is_symlink() => {
                return Err(anyhow!(
                    "Codex metadata 目录不能是 symlink: {}",
                    agents_dir.display()
                ));
            }
            Ok(metadata) if !metadata.is_dir() => {
                return Err(anyhow!(
                    "Codex metadata 路径不是目录: {}",
                    agents_dir.display()
                ));
            }
            Ok(_) => {}
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
                match fs::create_dir(&agents_dir) {
                    Ok(()) => {}
                    Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => {}
                    Err(error) => {
                        return Err(error).with_context(|| {
                            format!("Codex metadata 目录创建失败: {}", agents_dir.display())
                        });
                    }
                }
            }
            Err(error) => {
                return Err(error).with_context(|| {
                    format!("Codex metadata 目录检查失败: {}", agents_dir.display())
                });
            }
        }

        // create_dir 之后再检查一次文件类型，避免跟随现存的目录 symlink。
        let agents_metadata = fs::symlink_metadata(&agents_dir)
            .with_context(|| format!("Codex metadata 目录检查失败: {}", agents_dir.display()))?;
        if agents_metadata.file_type().is_symlink() || !agents_metadata.is_dir() {
            return Err(anyhow!(
                "Codex metadata 目录不能是 symlink 或非目录: {}",
                agents_dir.display()
            ));
        }
        ...
        match fs::symlink_metadata(&metadata_path) {
            ...
            Err(error) => {
                return Err(error).with_context(|| {
                    format!("Codex UI metadata 检查失败: {}", metadata_path.display())
                });
            }
        }

        let manifest = source.join("SKILL.md");
        let skill_metadata = Self::parse_skill_metadata_static(&manifest)
            .with_context(|| format!("解析 Skill metadata 失败: {}", manifest.display()))?;
        let name = skill_metadata
            ...
            .unwrap_or(directory)
            .to_string();
        let metadata = CodexUiMetadata {
            interface: CodexUiInterface {
                display_name: name.clone(),
                short_description: Self::codex_short_description(
                    skill_metadata.description.as_deref(),
                    &name,
                ),
                default_prompt: format!("Use ${name} to complete the requested task."),
            },
        };
        let yaml = serde_yaml::to_string(&metadata).context("序列化 Codex UI metadata 失败")?;
        let content = format!("{CODEX_UI_METADATA_MARKER}{yaml}");

        let mut temporary = tempfile::NamedTempFile::new_in(&agents_dir).with_context(|| {
            format!(
                "Codex UI metadata 临时文件创建失败: {}",
                agents_dir.display()
            )
        })?;
        std::io::Write::write_all(temporary.as_file_mut(), content.as_bytes()).with_context(
            || {
                format!(
                    "Codex UI metadata 临时文件写入失败: {}",
                    agents_dir.display()
                )
            },
        )?;
        ...
        match temporary.persist_noclobber(&metadata_path) {
            ...
            Err(error) => Err(error.error).with_context(|| {
                format!("Codex UI metadata 发布失败: {}", metadata_path.display())
            }),
        }
    }
    ...
    pub fn sync_to_app_dir(directory: &str, app: &AppType) -> Result<()> {
        ...
        if matches!(app, AppType::Codex) {
            Self::ensure_codex_ui_metadata(&source, &directory)?;
        }
...
mod tests {
    ...
    #[test]
    #[serial_test::serial]
    fn codex_sync_generates_safe_ui_metadata_and_is_idempotent() {
        let temp = tempdir().expect("tempdir");
        let _home = TestHomeGuard::set(temp.path());
        let _location = StorageLocationGuard::set(SkillStorageLocation::CcSwitch);
        let source = SkillService::get_ssot_dir()
            .expect("SSOT")
            .join("test-skill");
        fs::create_dir_all(&source).expect("create skill dir");
        fs::write(
            source.join("SKILL.md"),
            "---\nname: 'Quoted \"\u{6280}\u{80fd}\" \\ path'\ndescription: \"Line one\\nline two: \\\"safe\\\"\"\n---\n",
        )
        .expect("write SKILL.md");

        SkillService::sync_to_app_dir("test-skill", &AppType::Codex).expect("first Codex sync");

        let metadata_path = source.join("agents").join("openai.yaml");
        let first = fs::read(&metadata_path).expect("generated metadata");
        let first_text = std::str::from_utf8(&first).expect("UTF-8 metadata");
        assert!(first_text.starts_with("# Generated by CC Switch for Codex"));
        let parsed: serde_yaml::Value = serde_yaml::from_slice(&first).expect("valid YAML");
        assert_eq!(
            parsed["interface"]["display_name"].as_str(),
            Some("Quoted \"\u{6280}\u{80fd}\" \\ path")
        );
        assert_eq!(
            parsed["interface"]["short_description"].as_str(),
            Some("Line one line two: \"safe\"")
        );
        assert_eq!(
            parsed["interface"]["default_prompt"].as_str(),
            Some("Use $Quoted \"技能\" \\ path to complete the requested task.")
        );

        SkillService::sync_to_app_dir("test-skill", &AppType::Codex).expect("second Codex sync");
        assert_eq!(
            fs::read(&metadata_path).expect("metadata remains"),
            first,
            "a second sync must not rewrite generated metadata"
        );
    }

    #[test]
    fn codex_short_description_is_deterministically_bounded() {
        assert_eq!(
            SkillService::codex_short_description(Some("Brief"), "tiny"),
            "Brief and related workflows"
        );

        let long = "word ".repeat(30);
        let description = SkillService::codex_short_description(Some(&long), "verbose");
        assert_eq!(
            description,
            "word word word word word word word word word word word word word"
        );
        assert!((25..=64).contains(&description.chars().count()));
    }

    #[test]
    #[serial_test::serial]
    fn codex_sync_preserves_native_openai_metadata() {
        let temp = tempdir().expect("tempdir");
        let _home = TestHomeGuard::set(temp.path());
        let _location = StorageLocationGuard::set(SkillStorageLocation::CcSwitch);
        let source = SkillService::get_ssot_dir()
            .expect("SSOT")
            .join("test-skill");
        write_skill(&source, "native");
        let metadata_path = source.join("agents").join("openai.yaml");
        fs::create_dir_all(metadata_path.parent().expect("agents dir")).expect("create agents dir");
        let native = b"interface:\n  display_name: native\n";
        fs::write(&metadata_path, native).expect("write native metadata");

        SkillService::sync_to_app_dir("test-skill", &AppType::Codex).expect("Codex sync");

        assert_eq!(fs::read(metadata_path).expect("native metadata"), native);
    }

    #[test]
    #[serial_test::serial]
    fn non_codex_sync_does_not_generate_openai_metadata() {
        let temp = tempdir().expect("tempdir");
        let _home = TestHomeGuard::set(temp.path());
        let _location = StorageLocationGuard::set(SkillStorageLocation::CcSwitch);
        let source = SkillService::get_ssot_dir()
            .expect("SSOT")
            .join("test-skill");
        write_skill(&source, "Claude only");

        SkillService::sync_to_app_dir("test-skill", &AppType::Claude).expect("Claude sync");

        assert!(!source.join("agents").join("openai.yaml").exists());
    }

    #[test]
    fn generated_openai_metadata_is_hash_neutral_but_native_metadata_is_not() {
        let temp = tempdir().expect("tempdir");
        let skill = temp.path().join("test-skill");
        write_skill(&skill, "hash test");
        let baseline = SkillService::compute_dir_hash(&skill).expect("baseline hash");
        let metadata_path = skill.join("agents").join("openai.yaml");
        fs::create_dir_all(metadata_path.parent().expect("agents dir")).expect("create agents dir");
        fs::write(
            &metadata_path,
        ...
        assert_eq!(
            SkillService::compute_dir_hash(&skill).expect("generated hash"),
            baseline,
            "CC Switch generated metadata must not create a permanent update"
        );

        fs::write(
            &metadata_path,
            "interface:\n  display_name: \"upstream native\"\n",
        )
        .expect("write native metadata");
        assert_ne!(
            SkillService::compute_dir_hash(&skill).expect("native hash"),
            baseline,
            "upstream metadata must continue to participate in update detection"
        );
    }

    #[test]
    fn marker_does_not_hide_unrelated_files_from_hashing() {
        let temp = tempdir().expect("tempdir");
        let skill = temp.path().join("test-skill");
        write_skill(&skill, "hash test");
        let baseline = SkillService::compute_dir_hash(&skill).expect("baseline hash");
        ...
        assert_ne!(
            SkillService::compute_dir_hash(&skill).expect("updated hash"),
            baseline,
            "only agents/openai.yaml may be hash-neutral"
        );
    }
    ...
    #[test]
    #[serial_test::serial]
    fn codex_sync_rejects_a_symlinked_agents_directory() {
        use std::os::unix::fs::symlink;
        ...
        let source = SkillService::get_ssot_dir()
            .expect("SSOT")
            .join("test-skill");
        write_skill(&source, "unsafe agents");
        let outside = temp.path().join("outside");
        fs::create_dir(&outside).expect("create outside dir");
        symlink(&outside, source.join("agents")).expect("symlink agents dir");

        let error = SkillService::sync_to_app_dir("test-skill", &AppType::Codex)
            .expect_err("Codex sync must fail closed");

        assert!(
            error.to_string().contains("symlink"),
            "unexpected error: {error:#}"
        );
        assert!(!outside.join("openai.yaml").exists());
    }
