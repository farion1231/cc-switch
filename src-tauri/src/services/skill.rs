...
pub enum SyncMethod {
    ...
    #[default]
    Auto,
    /// 托管链接（Unix symlink；Windows 使用 junction）
    Symlink,
    /// 文件复制（兼容模式）
    Copy,
}
...
impl SkillService {
    ...
    pub fn restore_from_backup(
        ...
        if restore_path.exists() || Self::is_managed_link(&restore_path) {
            return Err(anyhow!(
    ...
    /// - Windows: 使用 NTFS junction（兼容 Git Bash / SSH 中的 Node.js）
    #[cfg(unix)]
    ...
    #[cfg(windows)]
    fn create_managed_link(src: &Path, dest: &Path) -> Result<()> {
        Self::create_junction(src, dest).with_context(|| {
            format!(
                "创建 junction 失败: {} -> {}",
                src.display(),
                dest.display()
    ...
    /// 检查路径是否为应用目录中的托管链接（Unix symlink / Windows junction 或 symlink）。
    ...
    fn replace_dest_with_copy(source: &Path, dest: &Path, directory: &str) -> Result<()> {
        ...
        if tmp.exists() || Self::is_managed_link(&tmp) {
...
mod tests {
    ...
    fn managed_windows_reparse_tag_rejects_non_link_tags() {
        ...
        assert!(!SkillService::is_managed_windows_reparse_tag(
    ...
    #[cfg(windows)]
    #[test]
    fn managed_windows_reparse_tag_accepts_junctions() {
        assert!(SkillService::is_managed_windows_reparse_tag(
