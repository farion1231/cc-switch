//! WorkBuddy（腾讯 WorkBuddy 桌面端）会话日志使用追踪
//!
//! 从 `~/.workbuddy/projects/` 下的 JSONL 会话文件中提取 token 使用数据，
//! 实现代理关闭时的使用统计。日志格式与 CodeBuddy Code CLI 完全同构
//! （复刻 `session_usage_codebuddy.rs`，含 `<sessionId>/subagents/agent-*.jsonl`
//! 子代理文件，落在 MAX_DIR_DEPTH=3 容忍范围内）。
//!
//! ## 数据源格式
//! WorkBuddy 的会话转录按工作目录组织：
//! ```text
//! ~/.workbuddy/projects/<workspace-dir>/<sessionId>.jsonl
//! ```
...
//! ## 数据流
//! ```text
//! ~/.workbuddy/projects/**/*.jsonl → 增量解析 → messageId 去重 → 费用计算 → proxy_request_logs 表
//! ```
...
/// 同步 WorkBuddy 会话日志到使用统计数据库。
pub fn sync_workbuddy_usage(db: &Database) -> Result<SessionSyncResult, AppError> {
    let projects_dir = get_workbuddy_config_dir().join("projects");
    if !projects_dir.exists() {
        return Ok(SessionSyncResult::default());
    }

    let mut result = SessionSyncResult::default();
    let jsonl_files = collect_workbuddy_jsonl_files(&projects_dir);
    ...
    for file_path in &jsonl_files {
        ...
        match sync_single_file(db, file_path, cursor) {
            ...
            Err(e) => {
                ...
                log::warn!("[WORKBUDDY-SYNC] 文件解析失败: {msg}");
...
fn collect_workbuddy_jsonl_files(projects_dir: &Path) -> Vec<PathBuf> {
...
fn sync_single_file(
    ...
    let start_byte = match last_byte_offset {
        Some(offset) if !(0..=file_size).contains(&offset) => {
            log::warn!(
                "[WORKBUDDY-SYNC] 文件被外部截断，游标钉至 EOF、不重放旧区间: {}",
                file_path.display()
            );
            let conn = lock_conn!(db.conn);
            update_workbuddy_sync_state_on_conn(&conn, &file_path_str, file_modified, file_size)?;
            return Ok(WorkBuddyFileSync::default());
        }
    ...
    let mut messages: HashMap<String, ParsedWorkBuddyUsage> = HashMap::new();
    ...
    let mut ordered: Vec<&ParsedWorkBuddyUsage> = messages.values().collect();
    ordered.sort_by_key(|m| m.timestamp_ms);

    for msg in ordered {
        match insert_workbuddy_entry_on_conn(&tx, msg) {
            Ok(true) => imported += 1,
            Ok(false) => skipped += 1,
            Err(e) => {
                log::warn!("[WORKBUDDY-SYNC] 插入失败 ({}): {e}", msg.message_id);
                skipped += 1;
            }
        }
    }
...
/// 末尾事件）才会返回 Some。
fn parse_usage_line(value: &serde_json::Value) -> Option<ParsedWorkBuddyUsage> {
...
/// 插入单条 WorkBuddy 会话日志到 proxy_request_logs。
/// 返回是否为新插入（false = 已存在被去重跳过）。
fn insert_workbuddy_entry_on_conn(
    conn: &rusqlite::Connection,
    msg: &ParsedWorkBuddyUsage,
) -> Result<bool, AppError> {
    let created_at = (msg.timestamp_ms / 1000).clamp(0, i64::MAX);

    let request_id = format!("workbuddy_session:{}", msg.message_id);
    ...
    let inserted_rows = conn
        .execute(
            ...
            rusqlite::params![
                ...
                "1.0",                 // cost_multiplier
                created_at,
                DATA_SOURCE,           // data_source: workbuddy_session
            ],
        )
...
mod tests {
    ...
    /// 构造一行带 usage 的 WorkBuddy JSONL（模拟 function_call 末尾事件）
        ...
        // 验证落库字段
        {
            ...
            let (input_tokens, cache_read, semantics, app_type, data_source): (
                ...
                "SELECT input_tokens, cache_read_tokens, input_token_semantics, app_type, data_source
                 FROM proxy_request_logs WHERE request_id = 'workbuddy_session:msg_a'",
                [],
                |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?, r.get(3)?, r.get(4)?)),
            )?;
            assert_eq!((input_tokens, cache_read), (21817, 12544));
            assert_eq!(semantics, INPUT_TOKEN_SEMANTICS_TOTAL);
            assert_eq!(app_type, "workbuddy");
            assert_eq!(data_source, "workbuddy_session");
        }
    ...
    fn test_collect_jsonl_files_recursively() {
        ...
        let files = collect_workbuddy_jsonl_files(&tmp);
        assert_eq!(files.len(), 2, "只收集 .jsonl，深度 2 内");

        fs::remove_dir_all(&tmp).ok();
    }

    fn sync_with_cursor(db: &Database, path: &Path) -> WorkBuddyFileSync {
