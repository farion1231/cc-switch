...
impl UsageEvent {
    fn id(&self) -> String {
        let bytes = serde_json::to_vec(self).expect("usage event is serializable");
...
fn insert_usage(conn: &Connection, event: &UsageEvent) -> Result<bool, AppError> {
    ...
    let inserted = conn.execute("INSERT OR IGNORE INTO proxy_request_logs (
        ...
        VALUES (?1,'_codex_otel','codex',?2,?2,?2,?3,?4,?5,?6,1,?7,?8,?9,?10,?11,0,200,?12,'codex_otel',1,'1',?13,'codex_otel',?14,?15,?16,2)",
