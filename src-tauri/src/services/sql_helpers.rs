...
///
/// Legacy and total-inclusive rows subtract cache reads only. Cache creation
/// is a separate nullable component and must not be deducted a second time
/// from the source's input total. Rollups normalized to fresh input are
/// returned unchanged.
///
...
mod tests {
    ...
    #[test]
    fn fresh_input_does_not_subtract_cache_creation_for_total_semantics() {
        let conn = setup_conn();
        conn.execute(
