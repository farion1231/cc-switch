        ...
        loop {
            ...
            match wire_type {
                0 => {
                    ...
                    return Some((field_num, ProtoValue::Varint(value)));
                }
                1 => {
                    // Fixed64 - skip 8 bytes
                    let next_offset = self.offset.checked_add(8)?;
                    if next_offset > self.data.len() {
                        return None;
                    }
                    self.offset = next_offset;
                    // loop to parse next field
                }
                2 => {
                    // Length-delimited
                    let length = self.decode_varint()? as usize;
                    let next_offset = self.offset.checked_add(length)?;
                    if next_offset > self.data.len() {
                        return None;
                    }
                    let blob = &self.data[self.offset..next_offset];
                    self.offset = next_offset;
                    ...
                    if let Ok(s) = std::str::from_utf8(blob) {
                        ...
                        {
                            return Some((field_num, ProtoValue::String(s.to_string())));
                        } else {
                            return Some((field_num, ProtoValue::Nested(blob.to_vec())));
                        }
                    } else {
                        return Some((field_num, ProtoValue::Nested(blob.to_vec())));
                    }
                ...
                5 => {
                    ...
                    let next_offset = self.offset.checked_add(4)?;
                    if next_offset > self.data.len() {
                        return None;
                    }
                    self.offset = next_offset;
...
pub fn sync_antigravity_usage(db: &Database) -> Result<SessionSyncResult, AppError> {
    ...
    if result.imported > 0 {
        ...
        log::debug!(
            "[ANTIGRAVITY-SYNC] 无新条目同步: 跳过 {} 条, 扫描 {} 个文件",
            result.skipped,
            result.files_scanned
        );
    }
...
fn sync_single_antigravity_db(db: &Database, db_path: &Path) -> Result<(u32, u32), AppError> {
    ...
    // 读取 gen_metadata 条目
    let gen_entries = read_gen_metadata_entries(&antigravity_conn, last_gen_idx);

    if gen_entries.is_empty() {
        update_sync_state(db, &file_path_str, file_modified, last_gen_idx)?;
        return Ok((0, 0));
    }

    // 读取 steps 表获取 per-entry 精确时间戳
    let step_timestamps = read_step_timestamps(&antigravity_conn, last_gen_idx);

    let mut imported: u32 = 0;
    let mut skipped: u32 = 0;
    let max_idx = gen_entries
        .last()
        .map(|e| e.idx + 1)
        .unwrap_or(last_gen_idx);

    // 获取会话 ID
    let session_id = trajectory_meta.as_ref().and_then(|m| m.session_id.clone());

    {
        let mut conn_guard = lock_conn!(db.conn);
        let tx = conn_guard
            .transaction()
...
fn read_gen_metadata_entries(
    ...
    last_gen_idx: i64,
) -> Vec<GenMetadataEntry> {
    let mut entries = Vec::new();

    let mut stmt =
        match conn.prepare("SELECT idx, data FROM gen_metadata WHERE idx >= ? ORDER BY idx") {
            Ok(s) => s,
            Err(_) => return entries,
        };

    let rows = stmt
        .query_map([last_gen_idx], |row| {
...
fn read_step_timestamps(
    ...
    let mut stmt =
        match conn.prepare("SELECT metadata FROM steps WHERE metadata IS NOT NULL AND idx >= ?") {
            Ok(s) => s,
            Err(_) => return map,
        };

    let rows = stmt
        .query_map([last_gen_idx], |row| row.get::<_, Vec<u8>>(0))
        .ok();
...
/// 将 Google 内部模型代号映射 to 公开名称，以匹配定价表
...
fn insert_antigravity_session_entry(
    ...
    // 取 output 与 thoughts 中的最大值，避免重复计算 thoughts 并确保安全（等同于之前的 fix 逻辑）
    let output_tokens = std::cmp::max(token_data.output_tokens, token_data.thoughts_tokens);
