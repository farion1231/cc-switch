...
#[cfg(any(not(target_os = "windows"), test))]
const UNIX_FISH_ENV_HEADER: &str =
    "# Generated by CC Switch. Sourced by fish startup config for persisted CLI env.\n";
#[cfg(any(not(target_os = "windows"), test))]
const UNIX_FISH_SOURCE_LINE: &str =
    "test -f \"$HOME/.cc-switch/env.fish\"; and source \"$HOME/.cc-switch/env.fish\" # cc-switch env";
#[cfg(any(not(target_os = "windows"), test))]
const UNIX_SHELL_STARTUP_FILES: [(&str, &str); 4] = [
    (".profile", UNIX_SHELL_SOURCE_LINE),
    (".bashrc", UNIX_SHELL_SOURCE_LINE),
    (".zshrc", UNIX_SHELL_SOURCE_LINE),
    (".config/fish/config.fish", UNIX_FISH_SOURCE_LINE),
];
...
fn persist_unix_user_env_var(name: &str, value: Option<&str>) -> Result<(), String> {
    ...
    let fish_env_file = env_dir.join("env.fish");
    let fish_current = match fs::read_to_string(&fish_env_file) {
        Ok(current) => current,
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => String::new(),
        Err(err) => {
            return Err(format!(
                "读取 fish 用户环境变量文件失败 {}: {err}",
                fish_env_file.display()
            ))
        }
    };
    fs::write(
        &fish_env_file,
        upsert_fish_env_file_content(&fish_current, name, value),
    )
    .map_err(|e| {
        format!(
            "写入 fish 用户环境变量文件失败 {}: {e}",
            fish_env_file.display()
        )
    })?;
...
#[cfg(any(not(target_os = "windows"), test))]
fn has_active_shell_source(content: &str, source_line: &str) -> bool {
...
#[cfg(any(not(target_os = "windows"), test))]
fn upsert_fish_env_file_content(current: &str, name: &str, value: Option<&str>) -> String {
    let set_prefix = format!("set -gx {name} ");
    let unset_line = format!("set -e {name}");
    let mut lines: Vec<String> = current
        .lines()
        .filter(|line| !line.starts_with(&set_prefix) && *line != unset_line)
        .filter(|line| *line != UNIX_FISH_ENV_HEADER.trim_end())
        .map(ToString::to_string)
        .collect();

    match value {
        Some(value) => {
            lines.push(format!("set -gx {name} {}", fish_single_quote(value)));
        }
        None => {
            lines.push(unset_line);
        }
    }

    let mut content = UNIX_FISH_ENV_HEADER.to_string();
    if !lines.is_empty() {
        content.push_str(&lines.join("\n"));
        content.push('\n');
    }
...
mod tests {
    ...
    #[test]
    fn fish_env_file_content_upserts_and_shell_quotes_values() {
        let content = upsert_fish_env_file_content(
            "set -gx CC_SWITCH_KEEP 'old'\nset -gx CC_SWITCH_TEST_UNIX_ENV 'old'\n",
            "CC_SWITCH_TEST_UNIX_ENV",
            Some("/tmp/claude profile's dir\\fish"),
        );

        assert!(content.contains("set -gx CC_SWITCH_KEEP 'old'"));
        assert!(content
            .contains("set -gx CC_SWITCH_TEST_UNIX_ENV '/tmp/claude profile\\'s dir\\\\fish'"));

        let content = upsert_fish_env_file_content(&content, "CC_SWITCH_TEST_UNIX_ENV", None);
        assert!(content.contains("set -gx CC_SWITCH_KEEP 'old'"));
        assert!(!content.contains("set -gx CC_SWITCH_TEST_UNIX_ENV "));
        assert!(content.contains("set -e CC_SWITCH_TEST_UNIX_ENV"));
    }

    #[test]
    fn unix_shell_startup_files_include_fish_config() {
        assert!(UNIX_SHELL_STARTUP_FILES.iter().any(|(path, source_line)| {
            *path == ".config/fish/config.fish" && *source_line == UNIX_FISH_SOURCE_LINE
        }));
    }

    #[test]
    fn shell_startup_source_detection_requires_active_exact_line() {
        assert!(!has_active_shell_source(
            "# [ -f \"$HOME/.cc-switch/env.sh\" ] && . \"$HOME/.cc-switch/env.sh\" # cc-switch env\n",
            UNIX_SHELL_SOURCE_LINE,
        ));
        assert!(!has_active_shell_source(
            "# Documentation mentions .cc-switch/env.sh but does not source it\n",
            UNIX_SHELL_SOURCE_LINE,
        ));
        assert!(has_active_shell_source(
            &format!("\n  {UNIX_SHELL_SOURCE_LINE}  \n"),
    ...
    #[test]
    fn unix_shell_startup_update_creates_fish_config_and_ignores_commented_marker() {
        let temp_home = tempfile::tempdir().expect("temp home");
        let profile = temp_home.path().join(".profile");
        fs::write(
            &profile,
            "# Documentation mentions .cc-switch/env.sh but does not source it\n",
        )
