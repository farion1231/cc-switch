...
///
/// 原因：接管模式下 `*_MODEL` 必须由 EC Switch 写成稳定的 Claude 角色别名，
...
pub struct ProxyService {
    ...
    orchestration: Arc<RwLock<Option<Arc<crate::orchestration::OrchestrationEngine>>>>,
}
...
impl ProxyService {
    ...
    pub fn set_orchestration(&self, engine: Arc<crate::orchestration::OrchestrationEngine>) {
    ...
    pub async fn update_config(&self, config: &ProxyConfig) -> Result<(), String> {
        ...
        if require_restart {
            ...
            let new_server =
                ProxyServer::new(new_config.clone(), self.db.clone(), app_handle, orchestration);
            let info = new_server
                .start()
                .await
...
"#;
        ...
        assert!(
            catalog_path.exists(),
            "ec-switch-model-catalog.json must be created on provider switch"
        );
