...
fn persist_opencode_session(
    ...
    let marked_at = SystemTime::now()
        ...
        .unwrap_or(0);
    if !has_incomplete_usage {
        delete_stale_opencode_session_rows_on_conn(&tx, &session.session_id, &current_request_ids)?;
    }
...
/// A complete OpenCode query is a replacement snapshot for this source
/// session. Delete only native compatibility rows absent from that snapshot;
/// proxy rows and other data sources retain their independent ownership.
fn delete_stale_opencode_session_rows_on_conn(
    conn: &rusqlite::Connection,
    session_id: &str,
    ...
    let mut statement = conn
        .prepare(
            "SELECT request_id FROM proxy_request_logs
             WHERE app_type = 'opencode' AND data_source = 'opencode_session' AND session_id = ?1",
        )
        .map_err(|error| {
            AppError::Database(format!("读取 OpenCode raw replacement 范围失败: {error}"))
        })?;
    let stale_request_ids = statement
        .query_map([session_id], |row| row.get::<_, String>(0))
        .map_err(|error| AppError::Database(format!("查询过期 OpenCode raw 行失败: {error}")))?
        .collect::<Result<Vec<_>, _>>()
        .map_err(|error| AppError::Database(format!("读取过期 OpenCode raw 行失败: {error}")))?
        .into_iter()
    ...
    for request_id in stale_request_ids {
        conn.execute(
            "DELETE FROM proxy_request_logs
             WHERE request_id = ?1
               AND app_type = 'opencode' AND data_source = 'opencode_session' AND session_id = ?2",
            rusqlite::params![request_id, session_id],
        )
        .map_err(|error| AppError::Database(format!("删除过期 OpenCode raw 行失败: {error}")))?;
    }
    Ok(())
}
        ...
        let remaining: (i64, i64) = conn
            .query_row(
                "SELECT
                    ...
                    (SELECT COUNT(*) FROM proxy_request_logs
                     WHERE app_type = 'opencode' AND data_source = 'opencode_session'
                       AND session_id = 'ses_rewrite')",
                [],
                |row| Ok((row.get(0)?, row.get(1)?)),
            )
            .expect("remaining canonical and raw counts");
