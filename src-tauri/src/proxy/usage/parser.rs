...
fn openai_cache_read_tokens(usage: &Value) -> u32 {
    usage
        ...
        // DeepSeek Chat 格式专有：缓存命中单列在 prompt_cache_hit_tokens。
        // prompt_tokens 本身已含命中+未命中（miss 见 prompt_cache_miss_tokens，
        // 仅作参考、无需在此扣减），故命中数直接作 cache_read 即可。
...
mod tests {
    ...
    fn test_openai_response_deepseek_cache_hit_fields() {
        // DeepSeek Chat 格式（issue #6073 关联）：缓存命中/未命中单列在
        // prompt_cache_hit_tokens / prompt_cache_miss_tokens，prompt_tokens 含两者。
        // 缺了 hit 字段的解析，缓存命中会被记 0，费用相对官网按全价虚高。
        let response = json!({
