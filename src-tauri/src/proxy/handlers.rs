...
async fn handle_responses_for_app(
    ...
    let codex_continue_enabled = app_type == AppType::Codex
        && codex_continue_native_only
    ...
    // Non-folded native Responses passthrough to a strict gateway (xAI). Folded
    // streams perform this rewrite inside codex_continue for each round's actual
    // provider. Restore flattened function-call names *and* rewrite whole-float
    // tool arguments — the integer rewrite must run even when the request had
    // no namespace tools.
...
mod tests {
    use super::{
        ...
        responses_sse_to_response_value, should_return_anthropic_transform_as_sse,
        should_return_chat_transform_as_sse, should_use_claude_transform_streaming, transform,
