...
pub fn set_proxy_port(port: u16) {
    if let Some(lock) = CC_SWITCH_PROXY_PORT.get() {
        if let Ok(mut current_port) = lock.write() {
            *current_port = port;
            log::debug!("[GlobalProxy] Updated EC Switch proxy port to {port}");
        }
    } else {
        let _ = CC_SWITCH_PROXY_PORT.set(RwLock::new(port));
        log::debug!("[GlobalProxy] Initialized EC Switch proxy port to {port}");
    }
}

/// 获取 EC Switch 代理服务器的监听端口
...
fn proxy_points_to_loopback(value: &str) -> bool {
    ...
    // 检查是否指向 EC Switch 自己的代理端口
    // 只有指向自己的代理才需要跳过，避免递归
    fn is_ec_switch_proxy_port(port: Option<u16>) -> bool {
        let ec_switch_port = get_proxy_port();
        port == Some(ec_switch_port)
    }

    if let Ok(parsed) = url::Url::parse(value) {
        if let Some(host) = parsed.host_str() {
            // 只有当主机是 loopback 且端口是 EC Switch 的端口时才返回 true
            return host_is_loopback(host) && is_ec_switch_proxy_port(parsed.port());
        }
        return false;
    }
...
mod tests {
    ...
    fn test_proxy_points_to_loopback() {
        // 设置 EC Switch 代理端口为 15721（默认值）
        set_proxy_port(15721);

        // 只有指向 EC Switch 自己端口的 loopback 地址才返回 true
