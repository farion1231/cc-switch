...
fn collect_input_declared_tools(value: &Value, context: &mut CodexToolContext) {
    match value {
        ...
        Value::Object(obj) => {
            ...
            if matches!(item_type, Some("tool_search_output" | "additional_tools")) {
                if let Some(tools) = obj.get("tools").and_then(|v| v.as_array()) {
                    for tool in tools {
                        if item_type == Some("tool_search_output") {
                            let mut normalized = tool.clone();
                            super::transform_codex_responses_namespace::normalize_response_function_tool(
                                &mut normalized,
                            );
                            context.add_response_tool(&normalized);
                        } else {
                            context.add_response_tool(tool);
                        }
                    }
...
mod tests {
    ...
    fn responses_request_to_chat_omits_missing_tool_description() {
        ...
        assert_eq!(tools.len(), 3); // includes the ToolSearch compatibility tool
    ...
    fn additional_tools_carrier_tools_dedup_against_top_level_tools() {
        ...
        let tools = result["tools"].as_array().unwrap();
        assert_eq!(tools.len(), 2, "includes the ToolSearch compatibility tool");
        assert_eq!(tools[0]["function"]["name"], "wait");
        assert_eq!(tools[0]["function"]["description"], "Top-level wait.");
        assert_eq!(tools[1]["function"]["name"], "tool_search");
    }
