...
mod tests {
    ...
    fn map_reasoning_effort_qwen_mode_table() {
        assert_eq!(map_reasoning_effort("minimal", Some("qwen"), None), Some("low"));
        assert_eq!(map_reasoning_effort("low", Some("qwen"), None), Some("low"));
        assert_eq!(map_reasoning_effort("medium", Some("qwen"), None), Some("medium"));
        assert_eq!(map_reasoning_effort("high", Some("qwen"), None), Some("xhigh"));
        assert_eq!(map_reasoning_effort("xhigh", Some("qwen"), None), Some("xhigh"));
        assert_eq!(map_reasoning_effort("max", Some("qwen"), None), Some("xhigh"));
        assert_eq!(map_reasoning_effort("garbage", Some("qwen"), None), None);
        // none/off/disabled 在 qwen 模式下不可达（关闭分支已提前处理），
        // 但函数自身的 early-return 语义保持不变
        assert_eq!(map_reasoning_effort("none", Some("qwen"), None), None);
    }
