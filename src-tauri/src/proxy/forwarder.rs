...
struct ForwardOptions {
    ...
    /// 图片分析递归请求必须保留配置的视觉模型，不能被 Codex catalog 回退覆盖。
    preserve_request_model: bool,
}

impl ForwardOptions {
    fn for_image_analysis() -> Self {
        Self {
            apply_model_mapping: false,
            image_context: false,
            media_prevention: false,
            preserve_request_model: true,
        }
    }
}
...
impl RequestForwarder {
    ...
    async fn forward(
        ...
        if is_copilot && !options.preserve_request_model {
            mapped_body =
        ...
        let copilot_optimization = if is_copilot
            && self.copilot_optimizer_config.enabled
            && !options.preserve_request_model
        {
        ...
        if matches!(app_type, AppType::Codex) && options.image_context {
            self.apply_image_context_if_configured(
                app_type,
                method,
                provider,
        ...
        let mut request_body = if codex_responses_to_chat {
            ...
            if !options.preserve_request_model {
        ...
        if matches!(app_type, AppType::Codex) && options.media_prevention {
    ...
    async fn apply_image_context_if_configured(
        ...
        if let Some(key) = cache_key.as_deref() {
            ...
            {
                super::image_context::inject_image_context_into_body(mapped_body, &analysis);
                log::info!(
        ...
        let image_forward = self.forward(
            ...
            extensions,
            adapter,
            ForwardOptions::for_image_analysis(),
        );
        let image_started_at = Instant::now();
        let (response, _claude_api_format, outbound_analysis_model) =
            match Box::pin(image_forward).await {
                Ok(response) => response,
                Err(error) => {
                    let reason = error.to_string();
                    // 识图失败时保留原图，交回原有 media 流程（受 enabled && request_media_fallback
                    // gate 约束）处理，不在此无条件替换为 marker。
                    log::warn!(
                        "[ImageContext] 图片识别失败，保留原图继续: provider={} reason={}",
                        provider.id,
                        reason
                    );
                    return Ok(());
        ...
        let analysis = match self
            .inject_image_analysis(
                ...
                app_type,
                image_started_at,
                outbound_analysis_model.as_deref(),
            )
            .await
        {
            ...
            Err(error) => {
                let reason = error.to_string();
                log::warn!(
                    "[ImageContext] 图片识别结果注入失败，保留原图继续: provider={} reason={}",
                    provider.id,
                    reason
                );
                return Ok(());
    ...
    fn cached_image_context_analysis(
        ...
        // 多图但单图缓存键两两不同，说明图片组合变化，不能拼装成一次完整分析。
        if image_count > 1
            && image_cache_keys
                .windows(2)
        ...
        // 已知折衷：单图缓存命中时无法恢复跨图关系（relation），仅注入各图描述。
        // 整体缓存（IMAGE_CONTEXT_CACHE）命中时才带 relation；多图重复请求若整体缓存
        // 已失效，第二次起会丢失关系描述，可接受（关系通常非关键上下文）。
    ...
    async fn log_image_context_usage(
        ...
        let app_type_str = app_type.as_str();
        let (multiplier, _pricing_model_source) = logger
            .resolve_pricing_config(provider_id, app_type_str)
            .await;
        // 实际出站模型优先于配置的 imageModel：上游响应不带 model 时回退到出站真值，
        // 再回退到 imageModel；pricing 与落库 model 都按实际出站模型，避免 alias 计价错配。
        let actual_model = usage
            .model
            .as_deref()
            .filter(|m| !m.is_empty())
            .or(outbound_model.filter(|m| !m.is_empty()))
...
mod tests {
    ...
    #[test]
    fn forward_options_default_enables_normal_request_processing() {
        let options = ForwardOptions::default();

        assert!(options.apply_model_mapping);
        assert!(options.image_context);
        assert!(options.media_prevention);
        assert!(!options.preserve_request_model);
    }

    #[test]
    fn image_analysis_forward_options_preserve_visual_model() {
        let options = ForwardOptions::for_image_analysis();
