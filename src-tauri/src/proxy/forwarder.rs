...
impl RequestForwarder {
    ...
    > {
        ...
        //
        // 这两个值是"静态"判定（基于 provider 配置/端点），GitHub Copilot 场景下
        // 会在下方按 copilot_protocol（按具体模型的实时能力）再做一次动态覆盖。
        let static_codex_responses_to_chat =
            matches!(app_type, AppType::Codex | AppType::GrokBuild)
                && super::providers::should_convert_codex_responses_to_chat(provider, endpoint);
        let static_codex_responses_to_anthropic =
            matches!(app_type, AppType::Codex | AppType::GrokBuild)
        ...
        // Copilot 的 endpoint 能力按具体模型实时确定（copilot_protocol），存在时优先于
        // 静态判定；不存在（非 Copilot 或未命中）时回退到上面的静态结果。
        ...
        let codex_responses_to_chat = matches!(app_type, AppType::Codex | AppType::GrokBuild)
            && match copilot_protocol {
                ...
                Some(_) => false,
                None => static_codex_responses_to_chat,
            };
