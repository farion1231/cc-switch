...
pub struct ForwardResult {
    ...
    /// Base URL selected for the successful upstream attempt.
    pub upstream_endpoint_url: String,
...
impl RequestForwarder {
    ...
    async fn forward_with_retry_inner(
        ...
        for provider in providers.iter() {
            ...
            match self
                ...
                .await
            {
                Ok((response, claude_api_format, outbound_model, upstream_endpoint_url)) => {
    ...
    /// 转发单个请求（使用适配器）
    ///
    /// 成功时返回 `(response, claude_api_format, outbound_model, base_url)`，其中
    ...
    ) -> Result<(ProxyResponse, Option<String>, Option<String>, String), ProxyError> {
        // 使用适配器提取 base_url
