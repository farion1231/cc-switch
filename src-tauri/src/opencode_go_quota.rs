...
mod tests {
    ...
    #[tokio::test]
    #[ignore = "requires OPENCODE_GO_WORKSPACE_ID and OPENCODE_GO_AUTH_COOKIE"]
    async fn live_get_quota_returns_usage_windows() {
        let workspace_id = env_or_dotenv("OPENCODE_GO_WORKSPACE_ID")
            .expect("OPENCODE_GO_WORKSPACE_ID must be set");
        let auth_cookie =
            env_or_dotenv("OPENCODE_GO_AUTH_COOKIE").expect("OPENCODE_GO_AUTH_COOKIE must be set");

        let result = get_quota(&workspace_id, &auth_cookie)
            .await
            .expect("live result");
        assert!(result.success, "live query failed: {:?}", result.error);
        let data = result.data.expect("usage data");
        assert!(!data.is_empty(), "usage data should not be empty");
        assert!(data.len() <= 3, "expected at most three usage windows");
        for item in data {
            assert!(item
                .plan_name
                .as_deref()
                .is_some_and(|name| !name.is_empty()));
            assert_eq!(item.total, Some(100.0));
            assert_eq!(item.unit.as_deref(), Some("%"));
            assert!(item.used.is_some_and(|used| (0.0..=100.0).contains(&used)));
            assert!(item
                .remaining
                .is_some_and(|remaining| (0.0..=100.0).contains(&remaining)));
            assert!(item.extra.as_deref().is_some_and(|extra| !extra.is_empty()));
        }
    }

    #[tokio::test]
    #[ignore = "requires OPENCODE_GO_WORKSPACE_ID and OPENCODE_GO_AUTH_COOKIE"]
    async fn live_get_quota_rejects_invalid_cookie() {
        let workspace_id = env_or_dotenv("OPENCODE_GO_WORKSPACE_ID")
            .expect("OPENCODE_GO_WORKSPACE_ID must be set");

        let result = get_quota(&workspace_id, "invalid-cookie")
            .await
            .expect("live result");
        assert!(!result.success);
        ...
        assert!(
            error.contains("Authentication failed")
                || error.contains("Dashboard returned HTTP")
                || error.contains("Could not parse usage data"),
    ...
    #[tokio::test]
    #[ignore = "requires OPENCODE_GO_WORKSPACE_ID and OPENCODE_GO_AUTH_COOKIE"]
    async fn live_get_quota_rejects_wrong_workspace_id() {
        let auth_cookie =
            env_or_dotenv("OPENCODE_GO_AUTH_COOKIE").expect("OPENCODE_GO_AUTH_COOKIE must be set");

        let result = get_quota("wrk_invalid", &auth_cookie)
            .await
            .expect("live result");
        assert!(!result.success);
        let error = result.error.expect("error message");
