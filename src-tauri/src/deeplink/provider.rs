...
pub fn import_provider_from_deeplink(
    ...
    // Desktop imports stay database-only until enabled explicitly requests a switch.
...
pub(crate) fn build_provider_from_request(
    ...
    if matches!(app_type, AppType::ClaudeDesktop) {
        // Reuse the Claude import boundary for role IDs, labels and [1M] suffixes.
        let routes =
            crate::commands::suggested_claude_desktop_routes(&provider).unwrap_or_default();
        let direct = routes
            .values()
            .all(|route| crate::claude_desktop_config::is_claude_safe_model_id(&route.model));
        let meta = provider.meta.get_or_insert_with(ProviderMeta::default);
        meta.claude_desktop_mode = Some(if direct {
            ClaudeDesktopMode::Direct
        } else {
            ClaudeDesktopMode::Proxy
        });
        meta.claude_desktop_model_routes = if direct {
            // Direct mode exposes the upstream IDs without a local mapping.
            routes
                .into_values()
                .map(|route| (route.model.clone(), route))
                .collect()
        } else {
            routes
        };
