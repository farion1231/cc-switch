...
fn parse_provider_deeplink(
    ...
    if !matches!(
        ...
        return Err(AppError::InvalidInput(format!(
            "Invalid app type: must be 'claude', 'codex', 'gemini', 'opencode', 'mimo', 'openclaw', or 'hermes', got '{app}'"
        )));
    }
...
fn parse_mcp_deeplink(
    ...
    for app in apps.split(',') {
        ...
        if !matches!(
            ...
            return Err(AppError::InvalidInput(format!(
                "Invalid app in 'apps': must be 'claude', 'codex', 'gemini', 'opencode', 'mimo', 'openclaw', or 'hermes', got '{trimmed}'"
            )));
        }
    }
