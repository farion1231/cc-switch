...
pub struct AppSettings {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub mimocode_config_dir: Option<String>,
    ...
    /// 当前 MimoCode 供应商 ID（本地存储，保持结构一致）
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub current_provider_mimocode: Option<String>,
...
impl Default for AppSettings {
    fn default() -> Self {
        Self {
            ...
            current_provider_mimocode: None,
...
pub fn get_mimocode_override_dir() -> Option<PathBuf> {
    let settings = settings_store().read().ok()?;
    settings
        .mimocode_config_dir
        .as_ref()
...
pub fn get_current_provider(app_type: &AppType) -> Option<String> {
    ...
    match app_type {
        ...
        AppType::MimoCode => settings.current_provider_mimocode.clone(),
    }
}
...
pub fn set_current_provider(app_type: &AppType, id: Option<&str>) -> Result<(), AppError> {
    ...
    mutate_settings(|settings| match app_type {
        ...
        AppType::MimoCode => settings.current_provider_mimocode = id_owned.clone(),
    })
}
