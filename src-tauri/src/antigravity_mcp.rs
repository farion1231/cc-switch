...
/// 获取 Antigravity MCP 配置文件路径（~/.gemini/config/mcp_config.json）
...
/// 将给定的启用 MCP 服务器映射写入到 Antigravity mcp_config.json 的 mcpServers 字段
pub fn set_mcp_servers_map(
...

    {
        let obj = root
            .as_object_mut()
            .ok_or_else(|| AppError::Config("~/.gemini/config/mcp_config.json 根必须是对象".into()))?;
