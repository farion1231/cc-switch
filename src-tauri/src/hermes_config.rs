...
///
/// Input (from EC Switch UI / database):
/// ```json
...
/// `providers:` dict. EC Switch treats those as read-only — writes have to
...
/// - `custom_providers:` list entries (writable by EC Switch)
...
/// keeping EC Switch free to edit it. Models are denormalized from the YAML
...
/// list — i.e. it is a read-only overlay EC Switch must not touch.
...
pub fn set_provider(
        ...
        .find(|p| p.get("name").and_then(|n| n.as_str()) == Some(name))
    {
        ...
        // this merge, a EC Switch edit to an unrelated field would silently
...
// EC Switch fills that gap by reading/writing the whole file as a markdown
...
";
    ...
    fn set_provider_preserves_unknown_fields_on_update() {
        ...
        // Hermes Web UI; a later EC Switch edit must not strip them — set_provider
...
";
    ...
    fn set_memory_enabled_preserves_other_fields() {
        ...
        // a EC Switch toggle would silently wipe those fields.
        with_test_home(|| {
            let yaml = "\
memory:
