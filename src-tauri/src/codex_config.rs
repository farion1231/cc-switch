...
fn codex_model_catalog_from_specs(
    ...
    codex_model_catalog_from_specs_with_capabilities(specs, template, profile, false)
}
...
fn codex_model_catalog_from_specs_with_capabilities(
    ...
    let entries: Vec<Value> = specs
        ...
        .enumerate()
        .map(|(index, spec)| {
            codex_catalog_model_entry(template, spec, index, profile, multi_agent_v2)
        })
        .collect();
...
mod tests {
    ...
    fn multi_agent_version_template_marker_is_removed_before_capability_insert() {
        ...
        for profile in [
            ...
            let catalog =
                codex_model_catalog_from_specs_with_capabilities(&specs, &template, profile, true);
            assert!(
