...
pub struct McpRoot {
    ...
    /// Cursor MCP 配置（实际使用 .cursor/mcp.json）
    #[serde(default, skip_serializing_if = "McpConfig::is_empty")]
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match normalized.as_str() {
            ...
            other => Err(AppError::localized(
                "unsupported_app",
                format!("不支持的应用标识: '{other}'。可选值: claude, claude-desktop, codex, gemini, grokbuild, opencode, openclaw, hermes, cursor。"),
                format!("Unsupported app id: '{other}'. Allowed: claude, claude-desktop, codex, gemini, grokbuild, opencode, openclaw, hermes, cursor."),
            )),
        }
    }
}
...
impl MultiAppConfig {
    ...
    fn default_with_auto_import() -> Result<Self, AppError> {
        ...
        Self::auto_import_prompt_if_exists(&mut config, AppType::Cursor)?;
    ...
    fn maybe_auto_import_prompts_for_existing_config(&mut self) -> Result<bool, AppError> {
        ...
        if !self.prompts.claude.prompts.is_empty()
            ...
            || !self.prompts.cursor.prompts.is_empty()
        {
            return Ok(false);
        }
    ...
    pub fn migrate_mcp_to_unified(&mut self) -> Result<bool, AppError> {
        ...
        for app in [
            ...
            let old_servers = match app {
                ...
                AppType::Cursor => continue,   // Cursor didn't exist in v3.6.x, skip
            };
