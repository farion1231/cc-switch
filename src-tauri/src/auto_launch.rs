...
/// 将 `/path/to/CC Switch Legacy.app/Contents/MacOS/CC Switch Legacy` 转换为 `/path/to/CC Switch Legacy.app`
#[cfg(target_os = "macos")]
...
mod tests {
    ...
    fn test_get_macos_app_bundle_path_valid() {
        let exe_path = std::path::Path::new(
            "/Applications/CC Switch Legacy.app/Contents/MacOS/CC Switch Legacy",
        );
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from("/Applications/CC Switch Legacy.app"))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_with_spaces() {
        let exe_path =
            std::path::Path::new(
                "/Users/test/My Apps/CC Switch Legacy.app/Contents/MacOS/CC Switch Legacy",
            );
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from(
                "/Users/test/My Apps/CC Switch Legacy.app"
            ))
        );
    }
