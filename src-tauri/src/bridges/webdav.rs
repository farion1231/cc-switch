...
pub async fn upload() -> Result<Value, AppError> {
    ...
    let mut settings =
        cc_switch_core::settings::get_webdav_sync_settings().ok_or_else(|| {
            AppError::Config("未配置 WebDAV 同步".to_string())
        })?;
...
pub fn save_settings_from_core(settings: cc_switch_core::WebDavSyncSettings) -> Result<(), AppError> {
