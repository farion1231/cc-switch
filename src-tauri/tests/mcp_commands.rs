...
fn import_default_config_claude_persists_provider() {
    ...
    let db_path = home.join(".ec-switch").join("ec-switch.db");
    assert!(
        db_path.exists(),
        "importing default config should persist to ec-switch.db"
    );
}
...
fn import_mcp_from_claude_creates_config_and_enables_servers() {
    ...
    assert!(
        db_path.exists(),
        "state.save should persist to ec-switch.db when changes detected"
    );
}
...
fn set_mcp_enabled_for_codex_writes_live_config() {
    ...
    let toml_path = ec_switch_lib::get_codex_config_path();
    assert!(
        toml_path.exists(),
