...
"#
    ...
    let auth_value: serde_json::Value =
        read_json_file(&ec_switch_lib::get_codex_auth_path()).expect("read auth.json");
    assert_eq!(
    ...
    let config_text =
        std::fs::read_to_string(ec_switch_lib::get_codex_config_path()).expect("read config.toml");
    assert!(
...
"#;
    ...
    let auth_after_switch: serde_json::Value =
        read_json_file(&ec_switch_lib::get_codex_auth_path()).expect("read auth after switch");
    assert_eq!(
    ...
    let config_after_switch =
        std::fs::read_to_string(ec_switch_lib::get_codex_config_path()).expect("read config");
    assert!(
    ...
    let auth_after_takeover: serde_json::Value =
        read_json_file(&ec_switch_lib::get_codex_auth_path()).expect("read auth after takeover");
    assert_eq!(
    ...
    let restored_auth: serde_json::Value =
        read_json_file(&ec_switch_lib::get_codex_auth_path()).expect("read restored auth");
    assert_eq!(
        restored_auth, oauth_auth,
        "disabling takeover should restore without replacing OAuth auth.json"
    );

    let restored_config = std::fs::read_to_string(ec_switch_lib::get_codex_config_path())
...
fn provider_service_switch_codex_official_accounts_write_auth_json() {
    ...
    let auth_b: serde_json::Value =
        read_json_file(&ec_switch_lib::get_codex_auth_path()).expect("read auth B");
    assert_eq!(
        auth_b
            .pointer("/tokens/access_token")
            .and_then(|v| v.as_str()),
    ...
    let auth_a: serde_json::Value =
        read_json_file(&ec_switch_lib::get_codex_auth_path()).expect("read auth A");
    assert_eq!(
        auth_a
            .pointer("/tokens/access_token")
            .and_then(|v| v.as_str()),
...
fn switch_packycode_gemini_updates_security_selected_type() {
    ...
    // Gemini security settings are written to ~/.gemini/settings.json, not ~/.ec-switch/settings.json
