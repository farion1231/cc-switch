...
"#
    ...
    let auth_value: serde_json::Value =
        read_json_file(&cc_doctor_lib::get_codex_auth_path()).expect("read auth.json");
    assert_eq!(
    ...
    let config_text =
        std::fs::read_to_string(cc_doctor_lib::get_codex_config_path()).expect("read config.toml");
    assert!(
...
fn switch_packycode_gemini_updates_security_selected_type() {
    ...
    // Gemini security settings are written to ~/.gemini/settings.json, not ~/.cc-doctor/settings.json
