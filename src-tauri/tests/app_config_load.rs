...
use ec_switch_lib::{AppError, MultiAppConfig};
...
fn cfg_path() -> PathBuf {
    ...
    PathBuf::from(home).join(".ec-switch").join("config.json")
}
...
fn load_v1_config_returns_error_and_does_not_write() {
    ...
    let bak = home.join(".ec-switch").join("config.json.bak");
...
fn load_valid_v2_config_succeeds() {
    ...
    assert!(loaded
        .get_manager(&ec_switch_lib::AppType::Claude)
        .is_some());
    assert!(loaded.get_manager(&ec_switch_lib::AppType::Codex).is_some());
}
