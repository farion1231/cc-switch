...
fn legacy_snapshot() -> PromptSnapshot {
    reset_test_fs();
...
fn core_snapshot() -> PromptSnapshot {
    reset_test_fs();
    let home = ensure_test_home();
    let _state = create_core_state_with_config(&MultiAppConfig::default());
    prompt_bridge::upsert_prompt(AppType::Claude, "prompt-1", prompt_fixture())
        .expect("core prompt upsert");

    let prompts = prompt_bridge::get_prompts(AppType::Claude).expect("core prompts");
    let current_file =
        prompt_bridge::get_current_prompt_file_content(AppType::Claude).expect("current file");
