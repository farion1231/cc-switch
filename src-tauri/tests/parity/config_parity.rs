...
fn config_parity_snippet_round_trip_matches_legacy() {
    ...
    let legacy =
        config_bridge::legacy_get_common_config_snippet("claude").expect("legacy get snippet");
...
fn config_parity_extract_matches_legacy() {
    ...
    let legacy =
        config_bridge::legacy_extract_common_config_snippet(AppType::Claude, input.clone())
            .expect("legacy extract");

    reset_test_fs();
    let _home = ensure_test_home();
    let core =
        config_bridge::extract_common_config_snippet(AppType::Claude, input).expect("core extract");
