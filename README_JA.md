...
デスクトップ版の WebUI は引き続き `cc-switch` から起動します。コマンドライン操作は独立した `cc-switch-cli` バイナリで提供されます:
...
`cc-switch-cli webui` はデスクトップ版と同じフロントエンドビルド成果物を再利用し、デフォルトで `127.0.0.1:9990` をリッスンします。
