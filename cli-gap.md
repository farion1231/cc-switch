...
如果后续目标是让 CLI 在进入 Tauri 迁移前承担完整的后端验证入口，那么最主要的剩余缺口已经收敛到 `Sessions`、结构化 `Settings`、以及少量宿主机动作与信息面能力。
...
| Settings 基础配置 | 完整 | 已有 | 部分对齐 | CLI 已补 language / visible-apps / terminal / startup / plugin / onboarding，剩余主要是目录管理与系统信息面 |
...
| WebDAV Sync | 完整 | 已有 | 基本对齐 | CLI 已补 show/save/test/upload/download/remote-info，剩余主要是 preset 与确认式交互 |
| 环境冲突处理 | 完整 | 已有 | 部分对齐 | CLI 已补 check/delete/restore，剩余主要是多选与来源级交互 |
| Sessions | 完整 | 已有 | 部分对齐 | CLI 已补 list/messages/resume-command，仍缺 terminal launch |
| Workspace / Daily Memory | 完整 | 已有 | 基本对齐 | CLI 已补读写、搜索和显式 path 输出，剩余主要是 open-dir 这类宿主机动作 |
| OMO / OMO Slim | 完整 | 已有 | 部分对齐 | CLI 已补 read-local/import-local/current/disable-current，剩余主要是专属模型映射体验 |
...
- `settings show`
- `settings language get/set/clear`
- `settings visible-apps get/set/clear`
- `settings terminal get/set/clear`
- `settings startup show/set`
- `settings plugin status/enable/disable`
- `settings onboarding status/skip/clear`
...
CLI 现在已经补上了大部分日常结构化设置入口，剩余主要差距集中在目录管理、rectifier / log config、auto backup 设置和少量壳层交互。
...
这个域的核心链路已经在 CLI 里跑通，剩余差距主要是 preset、保存后自动测试和确认式交互体验。
...
这个域的核心查看与 resume 信息获取已经可用，剩余差距主要是直接拉起 terminal 这类宿主机动作。
...
这个域的内容读写和路径发现都已经可用，剩余差距主要是目录打开这类宿主机动作，而不是文件工作流本身。
...
- `omo read-local/import-local/current/disable-current`
- `omo-slim read-local/import-local/current/disable-current`
- 仍未补更细的专属模型映射编辑体验

#### 结论

这个域的主工作流已经在 CLI 里跑通，剩余差距主要是更细的编辑体验，而不是“没有入口”。
...
- `portable-mode`
- `tool-versions`
- `auto-launch status/enable/disable`

#### 结论

CLI 现在已经覆盖了这部分的核心信息面和基础运维入口，剩余差距主要是 GUI 专属的下载更新与重启、open-external 交互，以及更细的 WSL 表单体验。
...
- 下载更新与重启
- open website / docs / terminal-launch 一类辅助动作
- GUI 级表单体验和多步交互反馈

### 当前收口线

- `core + CLI` 的业务能力收口已经完成，可以先停在这里。
- 剩余未补项主要是壳层交互：`sessions terminal launch`、`open-external`、`open-folder`、文件对话框。
- 这些能力目前统一后置，不作为进入后续阶段的阻塞项。
...
因此，CLI 与 GUI 的剩余差距已经不再是基础数据链，而主要集中在少数工作流和宿主机能力。后续如果要继续收敛差距，建议按以下功能层来补：

1. Provider / Proxy / Usage 高级运维层
2. Skills / OpenClaw / Sessions / Workspace 工作流层
3. Backup / Env Conflict 维护层
